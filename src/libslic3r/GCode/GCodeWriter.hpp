...
public:
    ...
    std::string set_pressure_advance(double pa, unsigned int extruder_id = 0) const;
    ...
    std::string set_speed(double F, const std::string_view comment = {},
                          const std::string_view cooling_marker = {}) const;
    ...
    std::string travel_to_xy_G2G3IJ(const Vec2d &point, const Vec2d &ij, const bool ccw,
    ...
    std::string extrude_to_xy_G2G3IJ(const Vec2d &point, const Vec2d &ij, const bool ccw, double dE,
                                     const std::string_view comment);
...
private:
    ...
    unsigned int m_last_travel_acceleration =
        (unsigned int) (-1); // only used for flavors supporting separate print/travel acc
...
//    static constexpr const int E_EXPORT_DIGITS    = 9;
    ...
    static constexpr const std::array<double, 10> pow_10_inv{1. / 1.,         1. / 10.,       1. / 100.,
                                                             1. / 1000.,      1. / 10000.,    1. / 100000.,
                                                             1. / 1000000.,   1. / 10000000., 1. / 100000000.,
                                                             1. / 1000000000.};
    ...
    {
        return {quantize(pt.x(), XYZF_EXPORT_DIGITS), quantize(pt.y(), XYZF_EXPORT_DIGITS),
                quantize(pt.z(), XYZF_EXPORT_DIGITS)};
    }
    ...
    void emit_z(const double z) { this->emit_axis('Z', z, XYZF_EXPORT_DIGITS); }

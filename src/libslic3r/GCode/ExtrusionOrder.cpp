...
int get_extruder_id(const ExtrusionEntityCollection &eec, const LayerTools &layer_tools, const PrintRegion &region,
...
ExtrusionEntitiesPtr extract_infill_extrusions(const PrintRegion &region, const ExtrusionEntityCollection &fills,
...
// Group fills by ExtrusionRole to ensure proper printing order (e.g., interlocking before regular infill)
std::vector<ExtrusionEntityReference> sort_fill_extrusions(const ExtrusionEntitiesPtr &fills, const Point *start_near)
{
    ...
    for (auto &[role, role_fills] : fills_by_role)
    {
        ...
        for (const ExtrusionEntityReference &fill : chain_extrusion_references(role_fills, current_position))
        {
            ...
            {
                for (const ExtrusionEntityReference &ee :
                     chain_extrusion_references(*eec, current_position, fill.flipped()))
                {
...
std::vector<InfillRange> extract_infill_ranges(const Print &print, const Layer &layer, const LayerIsland &island,
                                               const Point &offset, std::optional<Point> &previous_position,
                                               const ExtractEntityPredicate &should_pick_extrusion,
...
std::vector<std::reference_wrapper<const LayerIsland>> get_ordered_islands(
    const LayerSlice &lslice, const std::optional<Point> &previous_position)
{
    ...
    chain_and_reorder_layer_islands(islands_to_order,
                                    previous_position.has_value() ? std::addressof(*previous_position) : nullptr);
    return islands_to_order;
}

std::vector<IslandExtrusions> extract_island_extrusions(const LayerSlice &lslice, const Print &print,
                                                        const Layer &layer,
                                                        const ExtractEntityPredicate &should_pick_extrusion,
                                                        const PathSmoothingFunction &smooth_path, const Point &offset,
                                                        const unsigned extruder_id,
                                                        std::optional<Point> &previous_position)
{
    const auto should_pick_infill =
        [&should_pick_extrusion](const ExtrusionEntityCollection &eec, const PrintRegion &region)
    {
    ...
    for (const LayerIsland &island : ordered_islands)
    {
        ...
        {
            island_extrusions.infill_ranges = extract_infill_ranges(print, layer, island, offset, previous_position,
                                                                    should_pick_infill, smooth_path, extruder_id);

            island_extrusions.perimeters = extract_perimeter_extrusions(print, layer, island, should_pick_extrusion,
                                                                        extruder_id, offset, previous_position,
                                                                        smooth_path);
        }
        else
        {
...
std::vector<InfillRange> extract_ironing_extrusions(const LayerSlice &lslice, const Print &print, const Layer &layer,
                                                    const ExtractEntityPredicate &should_pick_extrusion,
                                                    const PathSmoothingFunction &smooth_path, const Point &offset,
                                                    const unsigned extruder_id, std::optional<Point> &previous_position)
{
    ...
    for (const LayerIsland &island : ordered_islands)
    {
        const std::vector<InfillRange> ironing_ranges{extract_infill_ranges(print, layer, island, offset,
                                                                            previous_position, should_pick_ironing,
                                                                            smooth_path, extruder_id)};
        result.insert(result.end(), ironing_ranges.begin(), ironing_ranges.end());
    }
    return result;
}

std::vector<SliceExtrusions> get_slices_extrusions(const Print &print, const Layer &layer,
                                                   const ExtractEntityPredicate &should_pick_extrusion,
                                                   const PathSmoothingFunction &smooth_path, const Point &offset,
                                                   const unsigned extruder_id, std::optional<Point> &previous_position)
{
    ...
    for (size_t idx : lslice_order)
    {
        ...
        std::vector<IslandExtrusions> island_extrusions{
            extract_island_extrusions(lslice, print, layer, should_pick_extrusion, smooth_path, offset, extruder_id,
                                      previous_position)};
        std::vector<InfillRange> ironing_extrusions{extract_ironing_extrusions(lslice, print, layer,
                                                                               should_pick_extrusion, smooth_path,
                                                                               offset, extruder_id, previous_position)};
        if (!island_extrusions.empty() || !ironing_extrusions.empty())
        {
            result.emplace_back(SliceExtrusions{std::move(island_extrusions), std::move(ironing_extrusions)});
        }
    }
    return result;
}

unsigned translate_support_extruder(const int configured_extruder, const LayerTools &layer_tools,
                                    const ConfigOptionBools &is_soluable)
{
    if (configured_extruder <= 0)
    {
        ...
        auto it_nonsoluble = std::find_if(layer_tools.extruders.begin(), layer_tools.extruders.end(),
                                          [&is_soluable](unsigned int extruder_id)
                                          { return !is_soluable.get_at(extruder_id); });
...
std::vector<SupportPath> get_support_extrusions(const unsigned int extruder_id,
                                                const GCode::ObjectLayerToPrint &layer_to_print,
                                                unsigned int support_extruder, unsigned int interface_extruder,
                                                const PathSmoothingFunction &smooth_path,
                                                std::optional<Point> &previous_position)
{
            ...
            for (const ExtrusionEntityReference &entity_reference : chain_extrusion_references(entities, support_start))
            {
                ...
                if (collection != nullptr)
                {
                    for (const ExtrusionEntity *sub_entity : *collection)
                    {
                        ...
                        auto [path, _]{smooth_path(nullptr, nullptr, {*sub_entity, entity_reference.flipped()},
                                                   extruder_id, last_position)};
                        if (!path.empty())
                        {
...
std::vector<OverridenExtrusions> get_overriden_extrusions(const Print &print, const GCode::ObjectsLayerToPrint &layers,
                                                          ...
                                                          std::optional<Point> &previous_position)
{
    ...
    for (const InstanceToPrint &instance : instances_to_print)
    {
        ...
        {
            const auto should_pick_extrusion =
                [&layer_tools, &instance, &extruder_id](const ExtrusionEntityCollection &entity_collection,
                                                        const PrintRegion &region)
            {
                ...
                if (get_extruder_id(entity_collection, layer_tools, region, instance.instance_id) !=
            ...
            std::vector<SliceExtrusions> slices_extrusions{get_slices_extrusions(print, *layer, should_pick_extrusion,
                                                                                 smooth_path, offset, extruder_id,
                                                                                 previous_position)};
...
std::vector<NormalExtrusions> get_normal_extrusions(const Print &print, const GCode::ObjectsLayerToPrint &layers,
                                                    const LayerTools &layer_tools,
                                                    const std::vector<InstanceToPrint> &instances_to_print,
                                                    const unsigned int extruder_id,
                                                    const PathSmoothingFunction &smooth_path,
    ...
    for (std::size_t i{0}; i < instances_to_print.size(); ++i)
    {
        ...
        if (layers[instance.object_layer_to_print_id].support_layer != nullptr)
        {
            result.back().support_extrusions = get_support_extrusions(
                extruder_id, layers[instance.object_layer_to_print_id],
                translate_support_extruder(instance.print_object.config().support_material_extruder.value, layer_tools,
                                           print.config().filament_soluble),
                translate_support_extruder(instance.print_object.config().support_material_interface_extruder.value,
                                           layer_tools, print.config().filament_soluble),
                smooth_path, previous_position);
        }
        ...
        if (const Layer *layer = layers[instance.object_layer_to_print_id].object_layer; layer)
        {
            ...
            result.back().slices_extrusions = get_slices_extrusions(print, *layer, should_pick_extrusion, smooth_path,
                                                                    offset, extruder_id, previous_position);
        }
    }
    return result;
...
std::vector<ExtruderExtrusions> get_extrusions(
    const Print &print, const GCode::WipeTowerIntegration *wipe_tower, const GCode::ObjectsLayerToPrint &layers,
    const bool is_first_layer, const LayerTools &layer_tools, const std::vector<InstanceToPrint> &instances_to_print,
    const std::map<unsigned int, std::pair<size_t, size_t>> &skirt_loops_per_extruder, unsigned current_extruder_id,
    const PathSmoothingFunction &smooth_path, bool get_brim, std::optional<Point> previous_position)
{
    ...
    for (const unsigned int extruder_id : layer_tools.extruders)
    {
        ...
        if (layer_tools.has_wipe_tower && wipe_tower != nullptr)
        {
            ...
            if (finish_wipe_tower ||
                is_toolchange_required(is_first_layer, layer_tools.extruders.back(), extruder_id, current_extruder_id))
            {
                ...
                if (const auto tool_change{wipe_tower->get_toolchange(toolchange_number, ignore_sparse)})
                {
                    ...
                    extruder_extrusions.wipe_tower_start = Point::new_scale(
                        wipe_tower->transform_wt_pt(tool_change->start_pos));
                }
            }
        }
        ...
        {
            extruder_extrusions.overriden_extrusions = get_overriden_extrusions(print, layers, layer_tools,
                                                                                instances_to_print, extruder_id,
                                                                                smooth_path, previous_position);
        }

        using GCode::ExtrusionOrder::get_normal_extrusions;
        extruder_extrusions.normal_extrusions = get_normal_extrusions(print, layers, layer_tools, instances_to_print,
                                                                      extruder_id, smooth_path, previous_position);
...
const PrintInstance *get_first_instance(const std::vector<ExtruderExtrusions> &extrusions,
                                        const std::vector<InstanceToPrint> &instances_to_print)
{

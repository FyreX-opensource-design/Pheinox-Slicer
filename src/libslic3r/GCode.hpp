...
///|/
#ifndef slic3r_preFlight_GCode_hpp_
#define slic3r_preFlight_GCode_hpp_
...
struct PrintObjectInstance
{
    ...
    bool operator==(const PrintObjectInstance &other) const
    {
        return print_object == other.print_object && instance_idx == other.instance_idx;
    }
...
public:
    ...
    Eigen::Matrix<double, Derived::SizeAtCompileTime, 1, Eigen::DontAlign> point_to_gcode(
        const Eigen::MatrixBase<Derived> &point) const
    {
        ...
        static_assert(int(Derived::SizeAtCompileTime) == 2 || int(Derived::SizeAtCompileTime) == 3,
    ...
    Vec2d point_to_gcode_quantized(const Eigen::MatrixBase<Derived> &point) const
    {
        static_assert(Derived::IsVectorAtCompileTime && int(Derived::SizeAtCompileTime) == 2,
                      "GCodeGenerator::point_to_gcode_quantized(): first parameter is not a 2D vector");
    ...
    std::string placeholder_parser_process(const std::string &name, const std::string &templ,
...
private:
    ...
    std::string generate_ramping_layer_change_gcode(const Polyline &xy_path, const double initial_elevation,
    ...
    LayerResult process_layer(const Print &print,
                              ...
                              const ObjectsLayerToPrint &layers, const LayerTools &layer_tools,
                              const GCode::SmoothPathCaches &smooth_path_caches, const bool last_layer,
    ...
    void process_layers(const Print &print, const ToolOrdering &tool_ordering,
                        const std::vector<const PrintInstance *> &print_object_instances_ordering,
                        const std::vector<std::pair<coordf_t, ObjectsLayerToPrint>> &layers_to_print,
                        const GCode::SmoothPathCache &smooth_path_cache_global, GCodeOutputStream &output_stream);
    // Process all layers of a single object instance (sequential mode) with a parallel pipeline:
    // Generate G-code, run the filters (vase mode, cooling buffer), run the G-code analyser
    // and export G-code into file.
    void process_layers(const Print &print, const ToolOrdering &tool_ordering, ObjectsLayerToPrint layers_to_print,
                        const size_t single_object_idx, const GCode::SmoothPathCache &smooth_path_cache_global,
                        GCodeOutputStream &output_stream);

    void set_extruders(const std::vector<unsigned int> &extruder_ids);
    std::string preamble();
    std::string change_layer(coordf_t previous_layer_z, coordf_t print_z, bool vase_mode, const Point &first_point,
                             const bool first_layer);
    std::string extrude_smooth_path(const GCode::SmoothPath &smooth_path, const bool is_loop,
    ...
    // Outputs gap fills from the region that fall within the island_bbox and haven't been output yet.
    // Tracks output entities in output_entities set to avoid duplicates.
    std::string extrude_interlocking_gap_fills(const LayerRegion *layerm, const BoundingBox &island_bbox,
                                               std::set<const ExtrusionEntity *> &output_entities);
    void initialize_instance(const InstanceToPrint &print_instance, const ObjectLayerToPrint &layer_to_print,
                             const bool is_first);
    ...
    std::string generate_travel_gcode(
        const Points3 &travel, const std::string &comment, const std::function<std::string()> &insert_gcode,
    ...
    Polyline generate_travel_xy_path(const Point &start, const Point &end, const bool needs_retraction,
                                     bool &could_be_wipe_disabled);

    std::string travel_to(const Vec3crd &start_point, const Vec3crd &end_point, ExtrusionRole role,
                          const std::string &comment, const std::function<std::string()> &insert_gcode,
                          const EnforceFirstZ enforce_first_z = EnforceFirstZ::False);

    std::string travel_to_first_position(const Vec3crd &point, const double from_z, const ExtrusionRole role,
    ...
    std::optional<int> m_current_manual_fan_speed;
    ...
    struct EmitModifiers
    {
        EmitModifiers(bool emit_fan_speed_reset, bool emit_bridge_fan_start, bool emit_bridge_fan_end)
            : emit_fan_speed_reset(emit_fan_speed_reset)
            , emit_bridge_fan_start(emit_bridge_fan_start)
            , emit_bridge_fan_end(emit_bridge_fan_end)
        {
        }

        EmitModifiers() : EmitModifiers(true, true, true) {};

        static EmitModifiers create_with_disabled_emits() { return {false, false, false}; }
    ...
    void _print_first_layer_chamber_temperature(GCodeOutputStream &file, const Print &print, const std::string &gcode,
                                                int temp, bool wait, bool accurate);
    void _print_first_layer_bed_temperature(GCodeOutputStream &file, const Print &print, const std::string &gcode,
                                            unsigned int first_printing_extruder_id, bool wait);
    void _print_first_layer_extruder_temperatures(GCodeOutputStream &file, const Print &print, const std::string &gcode,
                                                  unsigned int first_printing_extruder_id, bool wait);
    ...
    static void smooth_path_interpolate(const ObjectLayerToPrint &layers,
...
#endif // slic3r_preFlight_GCode_hpp_

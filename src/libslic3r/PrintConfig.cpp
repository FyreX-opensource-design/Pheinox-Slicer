...
// SLA removed - enums stubbed in PrintConfig.hpp
...
static t_config_enum_names enum_names_from_keys_map(const t_config_enum_values &enum_keys_map)
{
    ...
    names.assign(cnt, "");
    for (const auto &kvp : enum_keys_map)
        // Only assign if slot is empty, so first alphabetical name wins (e.g., "athena" over "classic")
...
static const t_config_enum_values s_keys_map_ArcFittingType{{"disabled", int(ArcFittingType::Disabled)},
...
static const t_config_enum_values s_keys_map_GCodeFlavor{
    {"rapid", gcfRapid},           {"reprapfirmware", gcfRepRapFirmware},
    {"reprap", gcfRepRapSprinter}, {"repetier", gcfRepetier},
    {"teacup", gcfTeacup},         {"makerware", gcfMakerWare},
    {"marlin", gcfMarlinLegacy},   {"marlin2", gcfMarlinFirmware},
    {"klipper", gcfKlipper},       {"sailfish", gcfSailfish},
    {"smoothie", gcfSmoothie},     {"mach3", gcfMach3},
    {"machinekit", gcfMachinekit}, {"no-extrusion", gcfNoExtrusion}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(GCodeFlavor)

static const t_config_enum_values s_keys_map_MachineLimitsUsage{{"emit_to_gcode", int(MachineLimitsUsage::EmitToGCode)},
                                                                {"time_estimate_only",
                                                                 int(MachineLimitsUsage::TimeEstimateOnly)},
...
static const t_config_enum_values s_keys_map_AuthorizationType{{"key", atKeyPassword}, {"user", atUserPassword}};
...
static const t_config_enum_values s_keys_map_FuzzySkinType{
    ...
    {"external_plus1", int(FuzzySkinType::ExternalPlus1)},
    {"external_plus2", int(FuzzySkinType::ExternalPlus2)},
    {"external_plus3", int(FuzzySkinType::ExternalPlus3)},
    {"all", int(FuzzySkinType::All)},
    // With holes options
    {"external_with_holes", int(FuzzySkinType::ExternalWithHoles)},
    {"external_plus1_holes", int(FuzzySkinType::ExternalPlus1WithHoles)},
    {"external_plus2_holes", int(FuzzySkinType::ExternalPlus2WithHoles)},
    {"external_plus3_holes", int(FuzzySkinType::ExternalPlus3WithHoles)},
    {"allwalls", int(FuzzySkinType::AllWalls)}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(FuzzySkinType)

static const t_config_enum_values s_keys_map_FuzzySkinNoiseType{{"classic", int(FuzzySkinNoiseType::Classic)},
...
static const t_config_enum_values s_keys_map_FuzzySkinMode{{"displacement", int(FuzzySkinMode::Displacement)},
...
static const t_config_enum_values s_keys_map_FuzzySkinPointPlacement{{"standard",
                                                                      int(FuzzySkinPointPlacement::Standard)},
                                                                     {"shapefollowing",
                                                                      int(FuzzySkinPointPlacement::ShapeFollowing)}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(FuzzySkinPointPlacement)

static const t_config_enum_values s_keys_map_FuzzySkinPaintedPerimeters{
    {"external", int(FuzzySkinPaintedPerimeters::External)},
    {"external+1", int(FuzzySkinPaintedPerimeters::ExternalPlus1)},
    {"external+2", int(FuzzySkinPaintedPerimeters::ExternalPlus2)},
    {"external+3", int(FuzzySkinPaintedPerimeters::ExternalPlus3)},
    {"all", int(FuzzySkinPaintedPerimeters::All)}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(FuzzySkinPaintedPerimeters)

static const t_config_enum_values s_keys_map_FuzzySkinVisibilityDetection{
    {"precise", int(FuzzySkinVisibilityDetection::fsvdPrecise)},
    {"standard", int(FuzzySkinVisibilityDetection::fsvdStandard)},
    {"relaxed", int(FuzzySkinVisibilityDetection::fsvdRelaxed)},
    {"minimal", int(FuzzySkinVisibilityDetection::fsvdMinimal)},
};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(FuzzySkinVisibilityDetection)

static const t_config_enum_values s_keys_map_TopSurfaceVisibilityDetection{
    {"precise", int(TopSurfaceVisibilityDetection::tsvdPrecise)},
    {"standard", int(TopSurfaceVisibilityDetection::tsvdStandard)},
    {"relaxed", int(TopSurfaceVisibilityDetection::tsvdRelaxed)},
    {"minimal", int(TopSurfaceVisibilityDetection::tsvdMinimal)},
};
...
static const t_config_enum_values s_keys_map_FanSpinupResponseType{{"axial", int(FanSpinupResponseType::fsrtAxial)},
                                                                   ...
                                                                   {"high_inertia",
                                                                    int(FanSpinupResponseType::fsrtHighInertia)}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(FanSpinupResponseType)
...
static const t_config_enum_values s_keys_map_SupportMaterialPattern{{"rectilinear", smpRectilinear},
...
static const t_config_enum_values s_keys_map_SupportBottomContactGap{{"no_gap", sbcgNoGap},
                                                                     {"half_layer", sbcgHalfLayer},
                                                                     {"full_layer", sbcgFullLayer}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SupportBottomContactGap)

static const t_config_enum_values s_keys_map_SupportTopContactGap{{"no_gap", stcgNoGap},
                                                                  {"half_layer", stcgHalfLayer},
                                                                  {"full_layer", stcgFullLayer},
                                                                  {"custom", stcgCustom}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SupportTopContactGap)

static const t_config_enum_values s_keys_map_SeamPosition{{"random", spRandom},
                                                          {"nearest", spNearest},
                                                          {"aligned", spAligned},
                                                          {"rear", spRear}};
...
static const t_config_enum_values s_keys_map_SLAPillarConnectionMode = {{"zigzag",
                                                                         int(SLAPillarConnectionMode::zigzag)},
                                                                        {"cross", int(SLAPillarConnectionMode::cross)},
                                                                        {"dynamic",
                                                                         int(SLAPillarConnectionMode::dynamic)}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SLAPillarConnectionMode)

static const t_config_enum_values s_keys_map_SLAMaterialSpeed = {{"slow", slamsSlow},
                                                                 {"fast", slamsFast},
...
static inline const t_config_enum_values s_keys_map_SLASupportTreeType = {
    {"default", int(sla::SupportTreeType::Default)}, {"branching", int(sla::SupportTreeType::Branching)},
...
static const t_config_enum_values s_keys_map_DraftShield = {{"disabled", dsDisabled},
                                                            {"limited", dsLimited},
                                                            {"enabled", dsEnabled}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(DraftShield)

static const t_config_enum_values s_keys_map_LabelObjectsStyle = {{"disabled", int(LabelObjectsStyle::Disabled)},
...
// Legacy configs may still reference "classic" - automatically upgrade to Athena (fixed-width)
static t_config_enum_values s_keys_map_PerimeterGeneratorType{
    {"arachne", int(PerimeterGeneratorType::Arachne)},
    {"athena", int(PerimeterGeneratorType::Athena)},
    {"classic", int(PerimeterGeneratorType::Athena)} // Legacy alias → Athena
};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(PerimeterGeneratorType)

static t_config_enum_values s_keys_map_TopOnePerimeterType{{"none", int(TopOnePerimeterType::None)},
...
static const t_config_enum_values s_keys_map_TowerSpeeds{
    {"layer1", tsLayer1},   {"layer2", tsLayer2},   {"layer3", tsLayer3},   {"layer4", tsLayer4},
    {"layer5", tsLayer5},   {"layer8", tsLayer8},   {"layer11", tsLayer11}, {"layer14", tsLayer14},
    {"layer18", tsLayer18}, {"layer22", tsLayer22}, {"layer24", tsLayer24},
};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(TowerSpeeds)

static const t_config_enum_values s_keys_map_TiltSpeeds{
    {"move120", tsMove120},     {"layer200", tsLayer200},   {"move300", tsMove300},     {"layer400", tsLayer400},
    {"layer600", tsLayer600},   {"layer800", tsLayer800},   {"layer1000", tsLayer1000}, {"layer1250", tsLayer1250},
    {"layer1500", tsLayer1500}, {"layer1750", tsLayer1750}, {"layer2000", tsLayer2000}, {"layer2250", tsLayer2250},
    {"move5120", tsMove5120},   {"move8000", tsMove8000},
};
...
static const t_config_enum_values s_keys_map_InterlockFlowDetection{
...
void PrintConfigDef::init_common_params()
{
    ...
    def->set_enum<AuthorizationType>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"key", L("API key")},
                                                                             {"user", L("HTTP digest")}});
    def->mode = comAdvanced;
    def->cli = ConfigOptionDef::nocli;
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->set_enum<ArcFittingType>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"disabled", "Disabled"},
    ...
    def->tooltip = L(
        "This feature automatically combines infill of several layers and speeds up your print by extruding thicker "
        "infill layers while preserving thin perimeters, thus maintaining accuracy. "
        "Note: Combined layer height cannot exceed the nozzle diameter.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Infill");
    def->tooltip = L(
        "Maximum layer height for combining infill when automatic infill combining is enabled. "
    ...
    def->tooltip = L("Fan speed for bridge infill extrusions. Used in both auto cooling and manual fan control modes.");
    def->sidetext = L("%");
    def->min = 0;
    def->max = 100;
    def->mode = comExpert;
    ...
    def->max_literal = 50;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(0.5, false));

    def = this->add("top_one_perimeter_type", coEnum);
    def->label = L("Single perimeter on top surfaces");
    def->category = L("Layers and Perimeters");
    def->tooltip = L(
        "Use only one perimeter on flat top surface, to give more space to the top infill pattern. Could be applied on topmost surface or all top surfaces.");
    def->mode = comExpert;
    def->set_enum<TopOnePerimeterType>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"none", L("Disabled")}, {"top", L("All top surfaces")}, {"topmost", L("Topmost surface only")}});
    ...
    def->set_enum<CoolingSlowdownLogicType>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
    ...
    def->tooltip = L("This end procedure is inserted at the end of the output file. "
                     "Note that you can use placeholder variables for all preFlight settings.");
    def->multiline = true;
    def->full_width = true;
    def->height = 12;
    def->mode = comExpert;
    ...
    def->set_enum<EnsureVerticalShellThickness>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"disabled", L("Disabled")},
    ...
    def->label = L("Top solid fill pattern");
    def->category = L("Infill");
    def->tooltip = L(
    ...
    def->set_enum<InfillPattern>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
    ...
    def->label = L("Bottom solid fill pattern");
    def->category = L("Infill");
    def->tooltip = L(
    ...
    def->label = L("Height");
    def->tooltip = L(
        "Only used when 'Print Settings -> Complete individual objects' is active. Set this to the vertical "
        ...
        "The value is ignored for printers that come with more detailed extruder models.");
    def->sidetext = L("mm");
    def->min = 0;
    def->mode = comExpert;
    ...
    def->label = L("Radius");
    def->tooltip = L(
        "Only used when 'Print Settings -> Complete individual objects' is active. Set this to a radius "
    ...
    def = this->add("fan_spinup_time", coInts);
    def->label = L("Spin-up time");
    def->tooltip = L("Time in milliseconds for the part cooling fan to spin up from 0% to 100%. "
                     "This enables physics-based fan speed compensation: fan commands are issued earlier "
                     "so the fan reaches target speed exactly when needed (e.g., at overhang/bridge start). "
                     "The actual lead time is calculated proportionally based on the speed change magnitude. "
                     "Set to 0 to disable this feature. "
                     "Typical values: 300-1000ms for small blower fans, check your fan's datasheet or measure.");
    def->sidetext = L("ms");
    def->min = 0;
    def->max = 5000;
    def->step = 100;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInts{500});

    def = this->add("fan_spinup_response_type", coEnums);
    def->label = L("Fan response type");
    def->tooltip = L("Acceleration profile of your part cooling fan. Affects how early the fan command is issued.\n"
                     "• Axial fan: Fast response, typical small axial fans\n"
                     "• Blower: Moderate response, typical small radial blowers\n"
                     "• High-inertia: Slow response, high-static-pressure blowers");
    def->set_enum<FanSpinupResponseType>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"axial", L("Axial fan (fast)")},
                                                                             {"blower", L("Blower (moderate)")},
                                                                             {"high_inertia",
                                                                              L("High-inertia (slow)")}});
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionEnums<FanSpinupResponseType>{FanSpinupResponseType::fsrtBlower});

    def = this->add("fan_spinup_bridge_infill", coBools);
    def->label = L("Bridge infill");
    def->tooltip = L("Enable early fan spin-up before bridge infill. "
                     "When enabled, the fan command is issued early enough for the fan to reach target speed "
                     "before the bridge begins. Requires Spin-up time > 0.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBools{true});
    ...
    def->tooltip = L(
        "If set to nonzero value, filament is moved toward the nozzle between the individual cooling moves (\"stamping\"). "
    ...
    def->tooltip = L(
        "Perform ramming when using multitool printer (i.e. when the 'Single Extruder Multimaterial' in Printer Settings is unchecked). "
    ...
    def->tooltip = L("Enter your filament shrinkage percentage for the X axis here to apply scaling of the object to "
    ...
    def->tooltip = L("Enter your filament shrinkage percentage for the Y axis here to apply scaling of the object to "
    ...
    def->gui_flags = "show_value";
    def->label = L("Sparse fill density");
    def->category = L("Infill");
    def->tooltip = L("Density of sparse infill, expressed in the range 0% - 100%.");
    def->sidetext = L("%");
    def->min = 0;
    def->max = 100;
    def->set_enum_values(ConfigOptionDef::GUIType::f_enum_open,
                         std::initializer_list<std::pair<std::string_view, std::string_view>>{{"0", "0%"},
                                                                                              {"5", "5%"},
                                                                                              {"10", "10%"},
                                                                                              {"15", "15%"},
                                                                                              {"20", "20%"},
    ...
    def->label = L("Sparse fill pattern");
    def->category = L("Infill");
    def->tooltip = L("Fill pattern for sparse infill.");
    def->set_enum<InfillPattern>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"rectilinear", L("Rectilinear")},
        {"alignedrectilinear", L("Aligned Rectilinear")},
        {"grid", L("Grid")},
    ...
    def->category = L("Infill");
    def->tooltip = L(
        "Fill pattern for solid infill (internal solid layers and vertical shells). Does not affect bridges.");
    // Use same enum values as top_fill_pattern (solid-appropriate patterns only)
    def->set_enum<InfillPattern>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"rectilinear", L("Rectilinear")},
        {"monotonic", L("Monotonic")},
        {"monotoniclines", L("Monotonic Lines")},
    ...
    def->tooltip = L(
        "If expressed as absolute value in mm/s, this speed will be applied to the solid infill print moves "
    ...
    def->tooltip = L("If set to a non-zero value, this speed will be used for travel moves on the first layer. "
                     "If expressed as a percentage, it will be calculated over the travel speed. "
                     "Set to 0 to use the normal travel speed on the first layer.");
    def->sidetext = L("mm/s or %");
    def->min = 0;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(0, false));
    ...
    def->label = L("Fuzzy skin (global)");
    def->category = L("Fuzzy skin");
    def->tooltip = L("Applies fuzzy skin to ALL surfaces of the model. "
                     "Set to 'None' if you only want fuzzy skin on painted areas.");
    def->set_enum<FuzzySkinType>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"none", L("None")},
        // No holes options
        {"external", L("External perimeters (no holes)")},
        {"external_plus1", L("External perimeters +1 (no holes)")},
        {"external_plus2", L("External perimeters +2 (no holes)")},
        {"external_plus3", L("External perimeters +3 (no holes)")},
        {"all", L("All perimeters (no holes)")},
        // With holes options
        {"external_with_holes", L("External perimeters (incl holes)")},
        {"external_plus1_holes", L("External perimeters +1 (incl holes)")},
        {"external_plus2_holes", L("External perimeters +2 (incl holes)")},
        {"external_plus3_holes", L("External perimeters +3 (incl holes)")},
        {"allwalls", L("All perimeters (incl holes)")}});
    def->mode = comSimple;
    def->width = 20;
    ...
    def->label = L("Fuzzy skin on bottom layer");
    def->category = L("Fuzzy skin");
    def->tooltip = L("When enabled, fuzzy skin is applied to all layers including the first layer (layer 0). "
                     "When disabled, fuzzy skin is skipped on the first layer, giving a smooth bottom surface "
                     "for better bed adhesion while maintaining the fuzzy texture on all other layers.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Fuzzy skin");
    def->tooltip = L("Type of noise algorithm to use for fuzzy skin displacement. "
                     "Classic uses random noise (original behavior). "
                     "Perlin and Billow create spatially coherent textures. "
                     "Ridged creates sharp, jagged features. "
                     "Voronoi creates cell-based patchwork patterns.");
    def->set_enum<FuzzySkinNoiseType>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"classic", L("Classic (Random)")},
                                                                             {"perlin", L("Perlin")},
                                                                             {"billow", L("Billow")},
    ...
    def->category = L("Fuzzy skin");
    def->tooltip = L("How fuzzy skin effect is applied. "
                     "Displacement offsets the path perpendicular to the surface. "
                     "Extrusion varies the extrusion width instead. "
                     "Combined uses both displacement and extrusion variation.");
    def->set_enum<FuzzySkinMode>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"displacement", L("Displacement")}, {"extrusion", L("Extrusion")}, {"combined", L("Combined")}});
    def->mode = comAdvanced;
    ...
    def->category = L("Fuzzy skin");
    def->tooltip = L("The base size of coherent noise features in mm. "
                     "Higher values create larger texture features. "
                     "Only applies to Perlin, Billow, Ridged, and Voronoi noise types.");
    def->sidetext = L("mm");
    def->min = 0.1;
    def->max = 100;
    def->mode = comAdvanced;
    ...
    def->category = L("Fuzzy skin");
    def->tooltip = L("Number of noise instances with decreasing feature size that are summed together. "
                     "More octaves add finer detail but increase computation time. "
                     "Only applies to Perlin, Billow, and Ridged noise types.");
    def->min = 1;
    def->max = 10;
    def->mode = comExpert;
    ...
    def->category = L("Fuzzy skin");
    def->tooltip = L("Controls how quickly the amplitude decreases for each successive octave. "
                     "Lower values result in smoother noise, higher values preserve more fine detail. "
                     "Only applies to Perlin and Billow noise types.");
    def->min = 0.0;
    def->max = 1.0;
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionFloat(0.5));

    def = this->add("fuzzy_skin_point_placement", coEnum);
    def->label = L("Point placement");
    def->category = L("Fuzzy skin");
    def->tooltip = L(
        "Algorithm used for placing fuzzy skin points along perimeters.\n\n"
        "• Standard: Original algorithm, faster but may cut corners at larger point distances.\n"
        "• Shape-following: Preserves corner vertices for better accuracy, especially at larger point distances.");
    def->set_enum<FuzzySkinPointPlacement>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"standard", L("Standard")},
                                                                             {"shapefollowing", L("Shape-following")}});
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionEnum<FuzzySkinPointPlacement>(FuzzySkinPointPlacement::Standard));

    def = this->add("fuzzy_skin_painted_perimeters", coEnum);
    def->label = L("Painted fuzzy skin");
    def->category = L("Fuzzy skin");
    def->tooltip = L("Controls which perimeters are affected in PAINTED fuzzy skin areas. "
                     "This setting only applies when using the paint-on fuzzy skin tool.\n\n"
                     "• External perimeters: Only the outermost perimeter (perimeter 0).\n"
                     "• External perimeters +1: External plus 1 inner perimeter (perimeters 0-1).\n"
                     "• External perimeters +2: External plus 2 inner perimeters (perimeters 0-2).\n"
                     "• External perimeters +3: External plus 3 inner perimeters (perimeters 0-3).\n"
                     "• All perimeters: All perimeters regardless of depth.");
    def->set_enum<FuzzySkinPaintedPerimeters>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"external", L("External perimeters")},
        {"external+1", L("External perimeters +1")},
        {"external+2", L("External perimeters +2")},
        {"external+3", L("External perimeters +3")},
        {"all", L("All perimeters")}});
    def->mode = comAdvanced;
    def->width = 20;
    def->set_default_value(new ConfigOptionEnum<FuzzySkinPaintedPerimeters>(FuzzySkinPaintedPerimeters::External));

    def = this->add("fuzzy_skin_on_top", coBool);
    def->label = L("Fuzzy skin on top surfaces");
    def->category = L("Fuzzy skin");
    def->tooltip = L(
        "When enabled, fuzzy skin is applied to all painted areas including top surfaces visible from above. "
        "When disabled, fuzzy skin is skipped on perimeters that are visible from the top of the print, "
        "giving a smooth finish on top layers while maintaining the fuzzy texture on vertical walls.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));

    def = this->add("fuzzy_skin_visibility_detection", coEnum);
    def->label = L("Visibility detection");
    def->category = L("Fuzzy skin");
    def->tooltip = L("Controls the sampling interval used to detect visibility boundaries on perimeters. "
                     "When 'Fuzzy skin on top/bottom surfaces' is disabled, the slicer samples along each perimeter "
                     "to find where visibility changes (e.g., where a wall meets an internal shelf).\n\n"
                     ...
                     "Lower values increase slicing time but improve accuracy at visibility boundaries.");
    def->set_enum<FuzzySkinVisibilityDetection>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"precise", L("Precise")}, {"standard", L("Standard")}, {"relaxed", L("Relaxed")}, {"minimal", L("Minimal")}});
    def->mode = comAdvanced;
    ...
    def->tooltip = L("Some G/M-code commands, including temperature control and others, are not universal. "
                     ...
                     "The \"No extrusion\" flavor prevents preFlight from exporting any extrusion value at all.");
    ...
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionEnum<GCodeFlavor>(gcfRapid));
    ...
    def->set_enum<LabelObjectsStyle>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"disabled", L("Disabled")}, {"octoprint", L("OctoPrint comments")}, {"firmware", L("Firmware-specific")}});
    def->mode = comAdvanced;
    ...
    def->tooltip = L("This feature allows to combine infill and speed up your print by extruding thicker "
                     ...
                     "Note: Combined layer height cannot exceed the nozzle diameter (e.g., with a 0.4mm nozzle and "
                     "0.2mm layer height, you can combine up to 2 layers).");
    def->sidetext = L("layers");
    ...
    def->tooltip = L(
        ...
        "preFlight tries to connect two close infill lines to a short perimeter segment. If no such perimeter segment "
    ...
    def->max_literal = 1000;
    def->set_enum_values(
        ConfigOptionDef::GUIType::f_enum_open,
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"0", L("0 (no open anchors)")},
                                                                             {"1", L("1 mm")},
                                                                             {"2", L("2 mm")},
                                                                             {"5", L("5 mm")},
                                                                             {"10", L("10 mm")},
    ...
    def->set_enum_values(
        ConfigOptionDef::GUIType::f_enum_open,
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"0", L("0 (not anchored)")},
                                                                             {"1", L("1 mm")},
                                                                             {"2", L("2 mm")},
                                                                             {"5", L("5 mm")},
                                                                             {"10", L("10 mm")},
    ...
    def->category = L("Advanced");
    def->tooltip = L("Controls overlap between infill and the innermost perimeter. "
                     "Extruded beads have rounded edges that must overlap for proper bonding. "
                     "The default 21.46% is derived from the geometric constant (1 - π/4), "
                     "which accounts for the semicircular bead cross-section. The overlap amount "
                     "is calculated from layer height. Higher values may cause over-extrusion artifacts.");
    def->sidetext = L("mm or %");
    def->ratio_over = "layer_height";
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionFloatOrPercent(21.46, true));
    ...
    def->ratio_over = "layer_height";
    def->min = -100;
    // Note: max_literal intentionally NOT set (uses default of 1) so UI prompts
    // "Do you mean X%?" for values > 1 or < -1, matching other overlap settings.
    // Actual clamping to -100% to 80% range happens in Fill.cpp.
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionFloatOrPercent(0, true));

    def = this->add("bridge_infill_perimeter_overlap", coFloatOrPercent);
    def->label = L("Bridge infill/perimeters overlap");
    def->category = L("Advanced");
    def->tooltip = L("Controls overlap between bridge infill and the innermost perimeter. "
                     "Separate from 'Infill/perimeters overlap' which affects normal infill. "
                     "Extruded beads have rounded edges that must overlap for proper bonding. "
                     "The default 21.46% is derived from the geometric constant (1 - π/4), "
                     "which accounts for the semicircular bead cross-section. The overlap amount "
                     "is calculated from layer height. At 0%, edges touch. Negative values create gaps.");
    def->sidetext = L("mm or %");
    def->ratio_over = "layer_height";
    def->min = -100;
    // Note: max_literal NOT set (uses default 1) for "Do you mean %?" prompt
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionFloatOrPercent(21.46, true));
    ...
    def->set_enum<IroningType>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"top", L("All top surfaces")}, {"topmost", L("Topmost surface only")}, {"solid", L("All solid surfaces")}});
    def->mode = comAdvanced;
    ...
    def->tooltip = L("Emit M73 P[percent printed] R[remaining time in minutes] at 1 minute"
                     ...
                     " Some firmware (like Marlin) recognizes M73 for progress reporting."
    ...
    def->tooltip = L(
        "Enable, if the firmware supports binary G-code format (bgcode). "
    ...
    def->set_enum<MachineLimitsUsage>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"emit_to_gcode", L("Emit to G-code")},
                                                                             {"time_estimate_only",
    ...
    // M205 J... [mm] - Junction Deviation for Marlin 2.x firmware
    def = this->add("machine_max_junction_deviation", coFloats);
    def->full_label = L("Junction deviation");
    def->category = L("Machine limits");
    def->tooltip = L("Junction deviation for Marlin 2.x firmware (M205 J).\n\n"
                     "When set to a value greater than 0, the slicer will use the junction deviation "
                     "algorithm instead of classic jerk for time estimation and will output M205 J in the G-code.\n\n"
                     "Junction deviation provides more accurate cornering speed calculations by allowing "
                     "the toolhead to deviate from the ideal path by this distance at corners.\n\n"
                     "Typical values: 0.01 - 0.08 mm\n"
                     "Set to 0 to use classic jerk (M205 XYZE).");
    def->sidetext = L("mm");
    def->min = 0;
    def->max = 1;
    def->mode = comAdvanced;
    ...
    // Each field holds a single M-code line from the user's config.g

    def = this->add("machine_rrf_m566", coString);
    def->label = L("M566 (Jerk)");
    def->category = L("Machine limits");
    def->tooltip = L("Enter your M566 command from config.g.\n\n"
                     "Sets jerk (instantaneous speed change) limits in mm/min.\n"
    ...
    def->category = L("Machine limits");
    def->tooltip = L("Enter your M204 command from config.g (optional).\n\n"
    ...
    def = this->add("machine_rrf_m207", coString);
    def->label = L("M207 (Retract)");
    def->category = L("Machine limits");
    def->tooltip = L("Enter your M207 command from config.g (optional).\n\n"
                     "Sets firmware retraction parameters:\n"
                     "  S = Retract length (mm)\n"
                     "  F = Retract feedrate (mm/min)\n"
    ...
    def->category = L("Machine limits");
    def->tooltip = L("Adjusts time estimates to better match real-world print times.\n\n"
                     "Negative values increase the estimate (more conservative).\n"
                     "Positive values decrease the estimate (more aggressive).\n\n"
                     "Examples:\n"
                     "  -15% = Estimate will be 15% longer (recommended starting point)\n"
                     "  0% = No adjustment (pure physics calculation)\n"
                     "  +10% = Estimate will be 10% shorter\n\n"
                     "Adjust based on how your actual prints compare to estimates.");
    def->sidetext = L("%");
    def->min = -50;
    def->max = 50;
    def->mode = comAdvanced;
    ...
    const std::string custom_parameter_tooltip_templ = L(
        "JSON-encoded string defining extra parameters, which can later be expanded in Custom G-code macro language. "
    ...
    def = this->add("print_nozzle_diameters", coFloats);
    def->label = L("Print nozzle diameters");
    def->tooltip = L(
        "Nozzle diameters for this print profile. When set, these values override the printer's nozzle diameters.");
    def->sidetext = L("mm");
    def->min = 0.1;
    def->max = 2;
    def->mode = comAdvanced;
    ...
    def = this->add("print_high_flow_nozzle", coBools);
    def->label = L("High flow nozzle");
    def->tooltip = L("Print preset-specific high flow nozzle settings");
    def->mode = comAdvanced;
    ...
    def->set_enum<InterlockFlowDetection>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"precise", L("Precise")}, {"standard", L("Standard")}, {"relaxed", L("Relaxed")}, {"minimal", L("Minimal")}});
    ...
    def = this->add("wipe_extend", coBools);
    def->label = L("Extend wipe");
    def->tooltip = L("When enabled, the wipe move continues in the same direction as the last extrusion move. "
                     "When disabled (default), the wipe follows the perimeter loop for the duration of the move.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBools{false});

    def = this->add("wipe_length", coFloats);
    def->label = L("Wipe length");
    def->tooltip = L("Length of the wipe move. When using firmware retraction or without "
                     "'Wipe while retracting', the wipe will be a pure travel move (no retraction). "
                     "Set to 0 to use the default wipe behavior based on retraction length.");
    def->sidetext = L("mm");
    def->min = 0;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloats{0.});
    ...
    def->tooltip = L("If enabled, preFlight detects obstacles along the travel path and makes the slope steeper "
    ...
    def->set_enum<SeamPosition>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"random", L("Random")}, {"nearest", L("Nearest")}, {"aligned", L("Aligned")}, {"rear", L("Rear")}});
    def->mode = comSimple;
    ...
    def->set_enum<ScarfSeamPlacement>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
...
//    def->gui_type = ConfigOptionDef::GUIType::slider;
    ...
    def->set_enum<DraftShield>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"disabled", L("Disabled")}, {"limited", L("Limited")}, {"enabled", L("Enabled")}});
    def->mode = comAdvanced;
    ...
    def->tooltip = L("When enabled, preFlight will check whether your custom Start G-Code contains G-codes to set "
                     ...
                     "placeholder variables for all preFlight settings, so you can put "
                     "a \"M109 S[first_layer_temperature]\" command wherever you want.\n"
                     "If your custom Start G-Code does NOT contain these G-codes, "
                     "preFlight will execute the Start G-Code after heated chamber was set to its temperature, "
                     "bed reached its target temperature and extruder just started heating.\n\n"
                     "When disabled, preFlight will NOT emit commands to heat up extruder, bed or chamber, "
    ...
    def->tooltip = L("This start procedure is inserted at the beginning, after any printer start gcode (and "
                     ...
                     "This is used to override settings for a specific filament. If preFlight detects "
                     ...
                     "for all preFlight settings, so you can put a \"M109 S[first_layer_temperature]\" command "
    ...
    def->set_enum<SlicingMode>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"regular", L("Regular")}, {"even_odd", L("Even-odd")}, {"close_holes", L("Close holes")}});
    def->mode = comAdvanced;
    ...
    def->tooltip = L("The vertical distance between object and support material interface. "
                     "Setting this to No gap will also prevent preFlight from using bridge flow and speed "
                     "for the first object layer.\n\n"
                     "Without interface layers, only No gap or Full layer height are effective. "
                     "Half layer and Custom values require interface layers for precise gap control.");
    def->set_enum<SupportTopContactGap>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"no_gap", L("No gap")},
                                                                             {"half_layer", L("Half layer height")},
                                                                             {"full_layer", L("Full layer height")},
                                                                             {"custom", L("Custom")}});
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionEnum<SupportTopContactGap>(stcgNoGap));

    def = this->add("support_material_contact_distance_custom", coFloat);
    def->label = L("Custom top contact gap");
    def->category = L("Support material");
    def->tooltip = L("Custom vertical distance between object and support material interface. "
                     "Only used when Top contact Z distance is set to Custom. "
                     "The gap is achieved by adjusting interface layer heights. "
                     "With very few interface layers (0-1), the exact gap may be approximated.");
    def->sidetext = L("mm");
    def->min = 0;
    def->max = 2;
    def->mode = comAdvanced;
    ...
    def = this->add("support_material_top_contact_extrusion_width", coPercent);
    def->label = L("Top contact extrusion width");
    def->category = L("Support material");
    def->tooltip = L("Width of extrusions for the top support interface layer, as a percentage of the interface width. "
                     "Lower values create thinner extrusions for easier separation from the supported surface. "
                     "100% uses the full interface width.\n\n"
                     "The actual width will never go below the layer height to prevent extrusion errors.\n\n"
                     "Without interface layers, this setting only applies when "
                     "Top contact Z distance is set to 0 (no gap).");
    def->sidetext = L("%");
    def->min = 50;
    def->max = 100;
    def->mode = comAdvanced;
    ...
    def = this->add("support_material_bottom_contact_distance", coEnum);
    ...
    def->tooltip = L("The vertical distance between the object top surface and the support material interface. "
                     "Options are relative to layer height to ensure proper alignment when supports merge.");
    def->set_enum<SupportBottomContactGap>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"no_gap", L("No gap")}, {"half_layer", L("Half layer height")}, {"full_layer", L("Full layer height")}});
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionEnum<SupportBottomContactGap>(sbcgNoGap));

    def = this->add("support_material_bottom_contact_extrusion_width", coPercent);
    def->label = L("Bottom contact extrusion width");
    def->category = L("Support material");
    def->tooltip = L("Width of extrusions for the bottom contact layer when using half-layer gap, "
                     "as a percentage of the interface width. "
                     "Lower values create a larger gap for easier support removal. "
                     "Height is always 50% of layer height; this controls the width.\n\n"
                     "The actual width will never go below the extrusion height to prevent extrusion errors.\n");
    def->sidetext = L("%");
    def->min = 50;
    def->max = 100;
    def->mode = comAdvanced;
    ...
    def->label = L("Support material interface extruder");
    def->category = L("Extruders");
    ...
    def->category = L("Support material");
    def->tooltip = L("Number of solid interface layers between the support structure and the object. "
                     "These layers distribute the gap specified in 'Top contact Z distance' - more layers "
                     "allow finer gap control. With fewer layers, the exact gap may not be achievable and "
                     "will be approximated within safe extrusion limits (25-75% of nozzle diameter per layer).");
    def->sidetext = L("layers");
    def->min = 0;
    def->set_enum_values(ConfigOptionDef::GUIType::i_enum_open,
                         std::initializer_list<std::pair<std::string_view, std::string_view>>{{"0", L("0 (none)")},
                                                                                              {"1", L("1 (sparse)")},
                                                                                              {"2", L("2 (moderate)")},
                                                                                              {"3", L("3 (default)")},
                                                                                              {"4", L("4 (dense)")}});
    ...
    def->tooltip = L("Number of interface layers to insert between the object(s) and support material. "
                     "Set to -1 to use the same value as 'Top interface layers'.");
    def->sidetext = L("layers");
    def->min = -1;
    def->set_enum_values(ConfigOptionDef::GUIType::i_enum_open,
    ...
    def = this->add("support_material_min_area", coFloat);
    def->label = L("Minimum support area");
    def->category = L("Support material");
    def->tooltip = L("Paint-on support regions smaller than this area will be filtered out. "
                     "This prevents accidental tiny paint specs from generating support structures.\n\n"
                     "Reference - circle diameter to area:\n"
                     "• 0.5mm diameter = 0.2 mm²\n"
                     "• 1.0mm diameter = 0.79 mm²\n"
                     "• 1.5mm diameter = 1.77 mm²\n"
                     "• 2.0mm diameter = 3.14 mm²\n\n"
    ...
    def->set_enum<SupportMaterialPattern>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"rectilinear", L("Rectilinear")}, {"rectilinear-grid", L("Rectilinear grid")}, {"honeycomb", L("Honeycomb")}});
    def->mode = comAdvanced;
    ...
    def->set_enum<SupportMaterialInterfacePattern>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"auto", L("Default")}, {"rectilinear", L("Rectilinear")}, {"concentric", L("Concentric")}});
    def->mode = comAdvanced;
    ...
    // Setting is kept for config compatibility but forced to false in Preset::normalize().
    def = this->add("thick_bridges", coBool);
    def->label = L("Thick bridges");
    def->category = L("Layers and Perimeters");
    def->tooltip = L("Deprecated: This setting is disabled in preFlight. "
                     "Thick bridges interfere with support interface gap calculations.");
    def->mode = comExpert; // Hide from normal UI
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->tooltip = L(
        "This custom code is inserted before every toolchange. Placeholder variables for all preFlight settings "
        "as well as {toolchange_z}, {previous_extruder} and {next_extruder} can be used. When a tool-changing command "
        "which changes to the correct extruder is included (such as T{next_extruder}), preFlight will emit no other such command. "
    ...
    def = this->add("top_surface_flow_reduction", coPercent);
    def->label = L("Top surface flow reduction");
    def->category = L("Layers and Perimeters");
    def->tooltip = L("Reduces extrusion flow for visible top surfaces (perimeters and solid infill). "
                     "This helps prevent overextrusion artifacts on visible surfaces. "
                     "The percentage is subtracted from 100% (e.g., 3% means 97% flow). "
                     "Set to 0% to disable. Top solid infill always uses reduced flow when enabled; "
                     "perimeters use visibility detection to only reduce flow on truly visible segments.");
    def->sidetext = L("%");
    def->min = 0;
    def->max = 20;
    def->mode = comAdvanced;
    ...
    def = this->add("top_surface_visibility_detection", coEnum);
    def->label = L("Visibility detection");
    def->category = L("Layers and Perimeters");
    def->tooltip = L("Controls how precisely the boundary between visible and hidden perimeter segments is detected. "
                     "Higher precision means more accurate boundaries but slower slicing.\n\n"
                     "• Precise (1mm): Highest accuracy, samples every 1mm along perimeters\n"
                     "• Standard (2mm): Good balance of accuracy and speed (recommended)\n"
                     "• Relaxed (4mm): Faster slicing with slightly less precise boundaries\n"
                     "• Minimal (8mm): Fastest, may miss some visibility transitions");
    def->set_enum<TopSurfaceVisibilityDetection>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"precise", L("Precise (1mm)")},
                                                                             {"standard", L("Standard (2mm)")},
                                                                             {"relaxed", L("Relaxed (4mm)")},
                                                                             {"minimal", L("Minimal (8mm)")}});
    def->mode = comAdvanced;
    def->set_default_value(
        new ConfigOptionEnum<TopSurfaceVisibilityDetection>(TopSurfaceVisibilityDetection::tsvdStandard));

    def = this->add("merge_top_solid_infills", coBool);
    def->label = L("Merge top solid infills");
    def->category = L("Layers and Perimeters");
    def->tooltip = L("When enabled, top solid infills will be merged with adjacent internal solid infills "
                     "to improve print efficiency. WARNING: On certain objects, this feature may incorrectly "
                     "convert solid infill into top solid infill, or cause slight misalignment of top surface "
                     "patterns. Disable if precise top surface alignment is critical.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->set_default_value(new ConfigOptionFloats{0.,   140., 140., 140., 140., 140., 0.,   140., 140.,
                                                  140., 140., 140., 0.,   140., 140., 140., 140., 140.,
                                                  0.,   140., 140., 140., 140., 140., 0.});
    ...
    def->tooltip = L(
        "Extra flow used for the purging lines on the wipe tower. This makes the purging lines thicker or narrower "
    ...
    def->category = L("Layers and Perimeters");
    def->tooltip = L(
        "Athena (Precise) produces precise perimeters with exact user-specified widths for maximum dimensional accuracy. "
        "Arachne (Variable) produces perimeters with variable extrusion width for better handling of thin walls and complex details. "
        "This setting also affects the Concentric infill.");
    def->set_enum<PerimeterGeneratorType>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"athena", L("Athena (Precise)")},
                                                                             {"arachne", L("Arachne (Variable)")}});
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionEnum<PerimeterGeneratorType>(PerimeterGeneratorType::Athena));
    ...
    for (const char *opt_key :
         {// floats
          ...
          // bools
          "retract_layer_change", "wipe", "wipe_extend", "travel_lift_before_obstacle", "travel_ramping_lift",
          "wipe_length",
          // percents
          "retract_before_wipe", "travel_slope"})
    {
        ...
        case coFloats:
            ...
            break;
        case coPercents:
            def->set_default_value(new ConfigOptionPercentsNullable(
                static_cast<const ConfigOptionPercents *>(it_opt->second.default_value.get())->values));
            break;
        case coBools:
            def->set_default_value(new ConfigOptionBoolsNullable(
                static_cast<const ConfigOptionBools *>(it_opt->second.default_value.get())->values));
            break;
        default:
            assert(false);
        }
    ...
    for (const char *opt_key : {// Floats or Percents
                                "seam_gap_distance"})
    {
        auto it_opt = options.find(opt_key);
        assert(it_opt != options.end());
        ...
        {
            const auto &default_value = *static_cast<const ConfigOptionFloatOrPercent *>(
                it_opt->second.default_value.get());
            def->set_default_value(
                new ConfigOptionFloatsOrPercentsNullable{{default_value.value, default_value.percent}});
            break;
        }
        default:
        {
...
void PrintConfigDef::init_extruder_option_keys()
{
    ...
    m_extruder_retract_keys = {"deretract_speed",
                               "retract_before_travel",
                               "retract_before_wipe",
                               "retract_layer_change",
                               "retract_length",
...
void PrintConfigDef::init_sla_support_params(const std::string &prefix)
{
    ...
    def->tooltip = L("Maximum number of bridges that can be placed on a pillar. Bridges "
    ...
    def->set_enum<SLAPillarConnectionMode>(ConfigOptionEnum<SLAPillarConnectionMode>::get_enum_names(),
...
void PrintConfigDef::init_sla_params()
{
    ...
    def->set_enum<SLADisplayOrientation>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{{"landscape", L("Landscape")},
                                                                             {"portrait", L("Portrait")}});
    def->mode = comExpert;
    ...
    def->tooltip = L(
        "The value is expressed as a percentage of the bed area. If the area of a particular layer "
    ...
    def->full_width = true;
    def->height = 13;
    // Notes field is useful for passing data to post-processing scripts
    def->mode = comSimple;
    def->set_default_value(new ConfigOptionString(""));
    ...
    def->set_enum<sla::SupportTreeType>(ConfigOptionEnum<sla::SupportTreeType>::get_enum_names(),
                                        {L("Default"),
    ...
    def->category = L("Pad");
    def->tooltip = L("How much should the tiny connectors penetrate into the model body.");
    def->sidetext = L("mm");
    def->min = 0;
    def->mode = comExpert;
    def->set_default_value(new ConfigOptionFloat(0.3));
    ...
    def->tooltip = L("A slower printing profile might be necessary when using materials with higher viscosity "
                     "or with some hollowed parts. It slows down the tilt movement and adds a delay before exposure.");
    def->set_enum<SLAMaterialSpeed>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"slow", L("Slow")}, {"fast", L("Fast")}, {"high_viscosity", L("High viscosity")}});
    def->mode = comAdvanced;
    ...
    for (const char *opt_key : {// float
                                "support_head_front_diameter", "branchingsupport_head_front_diameter",
                                "support_head_penetration", "branchingsupport_head_penetration", "support_head_width",
                                "branchingsupport_head_width", "support_pillar_diameter",
                                "branchingsupport_pillar_diameter", "elefant_foot_compensation", "absolute_correction",
                                // int
                                "support_points_density_relative"})
    {
        ...
        {
        case coFloat:
            def->set_default_value(new ConfigOptionFloatNullable{it_opt->second.default_value->getFloat()});
            break;
        case coInt:
            def->set_default_value(new ConfigOptionIntNullable{it_opt->second.default_value->getInt()});
            break;
        default:
            assert(false);
        }
...
void PrintConfigDef::init_sla_tilt_params()
{
    ...
    def->mode = comExpert;
    def->sidetext = L("mm/s");
    def->set_enum<TowerSpeeds>(std::initializer_list<std::pair<std::string_view, std::string_view>>{
        {"layer1", "1"},
        {"layer2", "2"},
        {"layer3", "3"},
        {"layer4", "4"},
    ...
    const std::initializer_list<std::pair<std::string_view, std::string_view>> tilt_speeds_il = {
        {"move120", "120"},    {"layer200", "200"},   {"move300", "300"},    {"layer400", "400"},
        {"layer600", "600"},   {"layer800", "800"},   {"layer1000", "1000"}, {"layer1250", "1250"},
        {"layer1500", "1500"}, {"layer1750", "1750"}, {"layer2000", "2000"}, {"layer2250", "2250"},
        {"move5120", "5120"},  {"move8000", "8000"},
    };
    ...
    def->tooltip = L(
        "Number of steps to move down from the calibrated (horizontal) position with 'tilt_down_initial_speed'.");
...
void PrintConfigDef::handle_legacy(t_config_option_key &opt_key, std::string &value)
{
    ...
    {
        // skirt_height == -1 for draft shield is no longer accepted.
    ...
    {
        // Legacy: draft_shield converted from bool to enum.
    ...
    else if (opt_key == "ensure_vertical_shell_thickness")
    {
        ...
        else if (const t_config_enum_values &enum_keys_map =
                     ConfigOptionEnum<EnsureVerticalShellThickness>::get_enum_values();
                 enum_keys_map.find(value) == enum_keys_map.end())
        {
    ...
    // Legacy: "monotonous" infill renamed to "monotonic".
    if (value == "monotonous" &&
        (opt_key == "top_fill_pattern" || opt_key == "bottom_fill_pattern" || opt_key == "fill_pattern"))
        value = "monotonic";
    ...
    if (opt_key == "support_material_contact_distance")
    {
        try
        {
            ...
            // Map old float/marker values to new enum keys
            if (v == 0.0)
                value = "no_gap";
            else if (v < -0.75)
        ...
        {
            // Value is already a string enum key, leave it as is
        }
    }

    // Old configs have float values like "0", "0.1", "0.2" - convert to new enum values
    if (opt_key == "support_material_bottom_contact_distance")
    {
        try
        {
            double v = boost::lexical_cast<double>(value);
            // Map old float values to new enum keys
            if (v <= 0.05)
                value = "no_gap";
            else if (v <= 0.15)
                value = "half_layer"; // Approximate - will be recalculated based on layer height
            else
                value = "full_layer"; // Approximate - will be recalculated based on layer height
        }
        catch (boost::bad_lexical_cast &)
        {
            // Value is already a string enum key, leave it as is
...
double min_object_distance(const ConfigBase &cfg)
{
    ...
    else
    {
        ...
        else
        {
            ...
            ret = (co_opt->value && ecr_opt->value > dd_opt->value) ? ecr_opt->value : dd_opt->value;
        }
    }
...
void DynamicPrintConfig::normalize_fdm()
{
    ...
    if (!this->has("interlocking_perimeter_extruder") && this->has("perimeter_extruder"))
        this->option("interlocking_perimeter_extruder", true)->setInt(this->option("perimeter_extruder")->getInt());
    ...
    if (auto *opt_wall_transition_length = this->opt<ConfigOptionFloat>("wall_transition_length", false);
        opt_wall_transition_length)
...
const std::map<std::string, ConfigOptionEnums<TowerSpeeds>> tower_tilt_options_enums_defs = {
    {"tower_speed", ConfigOptionEnums<TowerSpeeds>(
                        {tsLayer22, tsLayer22, tsLayer22, tsLayer22, tsLayer2, tsLayer2, tsLayer1, tsLayer1})},
};

const std::map<std::string, ConfigOptionEnums<TiltSpeeds>> tilt_options_enums_defs = {
    {"tilt_down_initial_speed", ConfigOptionEnums<TiltSpeeds>({tsLayer1750, tsLayer1750, tsLayer1750, tsLayer1750,
                                                               tsLayer800, tsLayer800, tsMove120, tsMove120})},
    {"tilt_down_finish_speed", ConfigOptionEnums<TiltSpeeds>({tsLayer1750, tsLayer1750, tsMove8000, tsLayer1750,
                                                              tsLayer1750, tsLayer1750, tsMove120, tsMove120})},
    {"tilt_up_initial_speed", ConfigOptionEnums<TiltSpeeds>({tsMove8000, tsMove8000, tsMove8000, tsMove8000,
                                                             tsLayer1750, tsLayer1750, tsMove120, tsMove120})},
    {"tilt_up_finish_speed", ConfigOptionEnums<TiltSpeeds>({tsLayer1750, tsLayer1750, tsLayer1750, tsLayer1750,
                                                            tsLayer800, tsLayer800, tsMove120, tsMove120})},
};
...
const std::map<std::string, ConfigOptionEnums<TiltSpeeds>> tilt_options_enums_sl1_defs = {
    {"tilt_down_initial_speed", ConfigOptionEnums<TiltSpeeds>({tsLayer400, tsLayer400, tsLayer400, tsLayer400,
                                                               tsLayer600, tsLayer600, tsMove120, tsMove120})},
    {"tilt_down_finish_speed", ConfigOptionEnums<TiltSpeeds>({tsLayer1500, tsLayer1500, tsLayer1750, tsLayer1500,
                                                              tsLayer1500, tsLayer1500, tsMove120, tsMove120})},
    {"tilt_up_initial_speed", ConfigOptionEnums<TiltSpeeds>({tsMove5120, tsMove5120, tsMove5120, tsMove5120,
                                                             tsLayer1500, tsLayer1500, tsMove120, tsMove120})},
    {"tilt_up_finish_speed", ConfigOptionEnums<TiltSpeeds>({tsLayer400, tsLayer400, tsLayer400, tsLayer400, tsLayer600,
                                                            tsLayer600, tsMove120, tsMove120})},
};
...
{
    const std::map<std::string, ConfigOptionFloats> floats_defs = is_sl1_model ? tilt_options_floats_sl1_defs
                                                                               : tilt_options_floats_defs;
    const std::map<std::string, ConfigOptionInts> ints_defs = is_sl1_model ? tilt_options_ints_sl1_defs
                                                                           : tilt_options_ints_defs;
    const std::map<std::string, ConfigOptionBools> bools_defs = is_sl1_model ? tilt_options_bools_sl1_defs
                                                                             : tilt_options_bools_defs;
    const std::map<std::string, ConfigOptionEnums<TowerSpeeds>> tower_enums_defs =
        is_sl1_model ? tower_tilt_options_enums_sl1_defs : tower_tilt_options_enums_defs;
    const std::map<std::string, ConfigOptionEnums<TiltSpeeds>> tilt_enums_defs = is_sl1_model
                                                                                     ? tilt_options_enums_sl1_defs
                                                                                     : tilt_options_enums_defs;
...
std::string DynamicPrintConfig::validate()
{
    ...
    auto printer_technology = (opt == nullptr) ? ptFFF
                                               : static_cast<PrinterTechnology>(
                                                     dynamic_cast<const ConfigOptionEnumGeneric *>(opt)->value);
...
std::string validate(const FullPrintConfig &cfg)
{
    ...
    if (cfg.use_firmware_retraction.value && cfg.gcode_flavor.value != gcfSmoothie &&
        cfg.gcode_flavor.value != gcfRapid && cfg.gcode_flavor.value != gcfRepRapSprinter &&
        cfg.gcode_flavor.value != gcfRepRapFirmware && cfg.gcode_flavor.value != gcfMarlinLegacy &&
        cfg.gcode_flavor.value != gcfMarlinFirmware && cfg.gcode_flavor.value != gcfMachinekit &&
        cfg.gcode_flavor.value != gcfRepetier && cfg.gcode_flavor.value != gcfKlipper)
        return "--use-firmware-retraction is only supported by oozeBot Rapid, Marlin, Klipper, Smoothie, RepRapFirmware, Repetier and Machinekit firmware";
    ...
    // The following test was commented out.
    ...
    // extrusion widths
    {
        ...
        const char *widths[] = {"external_perimeter", "perimeter",        "infill",     "solid_infill",
                                "top_infill",         "support_material", "first_layer"};
...
PRINT_CONFIG_CACHE_INITIALIZE((PrintObjectConfig, PrintRegionConfig, MachineEnvelopeConfig, GCodeConfig, PrintConfig,
                               FullPrintConfig, SLAMaterialConfig, SLAPrintConfig, SLAPrintObjectConfig,
                               SLAPrinterConfig, SLAFullPrintConfig))
...
CLIActionsConfigDef::CLIActionsConfigDef()
{
    ...
    def->tooltip =
        ("Get list of installed printer models into JSON.\n"
         "Note:\n"
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    def->tooltip = L(
        "This version of preFlight may not understand configurations produced by the newest preFlight versions. "
        "For example, newer preFlight may extend the list of supported firmware flavors. One may decide to "
    ...
    def->set_enum<ForwardCompatibilitySubstitutionRule>(
        std::initializer_list<std::pair<std::string_view, std::string_view>>{
            ...
            {"enable_silent",
             L("Enable reading unknown configuration values by silently substituting them with defaults.")}});
    def->set_default_value(
        new ConfigOptionEnum<ForwardCompatibilitySubstitutionRule>(ForwardCompatibilitySubstitutionRule::Enable));
    ...
    def->tooltip = L(
        "Sets the maximum number of threads the slicing process will use. If not defined, it will be decided automatically.");
    def->min = 1;
    ...
    def->tooltip = L("If enabled, the command line arguments are sent to an existing instance of GUI preFlight, "
                     "or an existing preFlight window is activated. "
    ...
    def->label = "Single instance mode for preflight url"; // Not translated on purpose - for internal use only.
    def->tooltip = "Works as single_instance but only if preflight url is present.";
...
ReadWriteSlicingStatesConfigDef::ReadWriteSlicingStatesConfigDef()
{
    ...
    def->tooltip = L(
        "Position of the extruder at the beginning of the custom G-code block. If the custom G-code travels somewhere else, "
        "it should write to this variable so preFlight knows where it travels from when it gets control back.");
    ...
    def->tooltip = L(
        ...
        "it should write to this variable so preFlight deretracts correctly when it gets control back.");
...
PrintStatisticsConfigDef::PrintStatisticsConfigDef()
{
    ...
    def->tooltip = L(
        "Weight per extruder extruded during the entire print. Calculated from density in Filament Settings.");
...
DimensionsConfigDef::DimensionsConfigDef()
{
    ...
    const std::string bb_size_tooltip = L(
...
CustomGcodeSpecificConfigDef::CustomGcodeSpecificConfigDef()
{
    ...
    def->tooltip = L(
        "Height above the print bed when the toolchange takes place. Usually the same as layer_z, but can be different.");
    ...
    def->tooltip = L(
        "Index of the extruder for which color change will be performed. The index is zero based (first extruder has index 0).");
}
...
{
    return boost::algorithm::contains(printer_notes, "PRINTER_VENDOR_OOZEBOT") &&

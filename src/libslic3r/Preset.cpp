...
ConfigFileType guess_config_file_type(const ptree &tree)
{
    ...
    for (const ptree::value_type &v : tree)
    {
        if (v.second.empty())
        {
            if (v.first == "background_processing" || v.first == "last_output_path" || v.first == "no_controller" ||
                v.first == "no_defaults")
                ++app_config;
...
VendorProfile VendorProfile::from_ini(const ptree &tree, const boost::filesystem::path &path, bool load_all)
{
    ...
    if (!boost::filesystem::exists(path))
    {
        throw Slic3r::RuntimeError(
            (boost::format("Cannot load Vendor Config Bundle `%1%`: File not found: `%2%`.") % id % path).str());
    }
    ...
    auto get_or_throw = [&](const ptree &tree, const std::string &key) -> ptree::const_assoc_iterator
    {
        ...
        {
            throw Slic3r::RuntimeError(
                (boost::format("Vendor Config Bundle `%1%` is not valid: Missing secion or key: `%2%`.") % id % key)
                    .str());
        }
        return res;
    };
    ...
    {
        throw Slic3r::RuntimeError(
            (boost::format("Vendor Config Bundle `%1%` is not valid: Cannot parse config_version: `%2%`.") % id %
             config_version_str)
                .str());
    }
    else
        ...
        if (boost::starts_with(section.first, printer_model_key))
        {
            ...
            if (!ConfigOptionEnum<PrinterTechnology>::from_string(technology_field, model.technology))
            {
                BOOST_LOG_TRIVIAL(error) << boost::format(
                                                "Vendor bundle: `%1%`: Invalid printer technology field: `%2%`") %
                                                id % technology_field;
                model.technology = ptFFF;
            }

            model.family = section.second.get<std::string>("family", std::string());
            if (model.family.empty() && res.name == "oozeBot")
            {
...
{
    return boost::algorithm::ends_with(name, g_suffix_modified) ? name.substr(0, name.size() - g_suffix_modified.size())
                                                                : name;
}
                              ...
                              const PresetWithVendorProfile &active_printer)
{
    ...
    auto *compatible_prints = dynamic_cast<const ConfigOptionStrings *>(
        preset.preset.config.option("compatible_prints"));
                                ...
                                const DynamicPrintConfig *extra_config)
{
    ...
    auto *compatible_printers = dynamic_cast<const ConfigOptionStrings *>(
        preset.preset.config.option("compatible_printers"));
    bool has_compatible_printers = compatible_printers != nullptr && !compatible_printers->values.empty();
    if (!has_compatible_printers && !condition.empty())
    {
        try
        {
            return PlaceholderParser::evaluate_boolean_expression(condition, active_printer.preset.config,
                                                                  extra_config);
        }
        catch (const std::runtime_error &err)
        {
    ...
    return preset.preset.is_default || active_printer.preset.name.empty() || !has_compatible_printers ||
           std::find(compatible_printers->values.begin(), compatible_printers->values.end(),
...
static std::vector<std::string> s_Preset_print_options{
    ...
    "infill_every_layers",
    /*"infill_only_where_needed",*/ "solid_infill_every_layers",
    "solid_fill_pattern",
    "narrow_to_athena",
    ...
    "fuzzy_skin_point_placement",
    "fuzzy_skin_painted_perimeters",
    "fuzzy_skin_on_top",
    ...
    "support_baobab_angle",
    "support_baobab_angle_slow",
    "support_baobab_canopy_density",
    "support_baobab_max_canopy_angle",
    "support_baobab_plant_on_model",
    "support_baobab_trunk_diameter",
    "support_baobab_trunk_diameter_angle",
    "support_baobab_trunk_distance",
    "support_tree_angle",
    ...
    "interface_shells",
    "extrusion_width_percent_of_nozzle",
    "extrusion_width",
    ...
    "bridge_infill_perimeter_overlap",
    "infill_anchor",
    "infill_anchor_max",
    "bridge_flow_ratio",
...
static std::vector<std::string> s_Preset_filament_options{
    ...
    "filament_retract_before_travel", "filament_retract_layer_change", "filament_wipe", "filament_retract_before_wipe",
    "filament_wipe_extend", "filament_wipe_length", "filament_retract_length_toolchange",
    "filament_retract_restart_extra_toolchange", "filament_travel_ramping_lift", "filament_travel_slope",
    "filament_travel_max_lift", "filament_travel_lift_before_obstacle",
    // Profile compatibility
    "filament_vendor", "compatible_prints", "compatible_prints_condition", "compatible_printers",
...
static std::vector<std::string> s_Preset_printer_options{
    ...
    "use_volumetric_e", "variable_layer_height", "prefer_clockwise_movements", "time_cost", "currency_symbol",
    //FIXME the print host keys are left here just for conversion from the Printer preset to Physical Printer preset.
    "host_type", "print_host", "printhost_apikey", "printhost_cafile", "single_extruder_multi_material",
    "custom_parameters_printer", "start_gcode", "end_gcode", "before_layer_gcode", "layer_gcode", "toolchange_gcode",
    "color_change_gcode", "pause_print_gcode", "template_custom_gcode", "between_objects_gcode", "printer_vendor",
...
static std::vector<std::string> s_Preset_sla_print_options{
    ...
    "support_head_front_diameter", "support_head_penetration", "support_head_width", "support_pillar_diameter",
    "support_small_pillar_diameter_percent", "support_max_bridges_on_pillar", "support_max_weight_on_model",
    "support_pillar_connection_mode", "support_buildplate_only", "support_enforcers_only",
    "support_pillar_widening_factor", "support_base_diameter", "support_base_height", "support_base_safety_distance",
    "support_critical_angle", "support_max_bridge_length", "support_max_pillar_link_distance",
    ...
    "branchingsupport_pillar_widening_factor", "branchingsupport_base_diameter", "branchingsupport_base_height",
    "branchingsupport_base_safety_distance", "branchingsupport_critical_angle", "branchingsupport_max_bridge_length",
    "branchingsupport_max_pillar_link_distance", "branchingsupport_object_elevation",

    "support_points_density_relative", "slice_closing_radius", "slicing_mode", "pad_enable", "pad_wall_thickness",
    "pad_wall_height", "pad_brim_size", "pad_max_merge_distance",
    // "pad_edge_radius",
    "pad_wall_slope", "pad_object_gap", "pad_around_object", "pad_around_object_everywhere",
    "pad_object_connector_stride", "pad_object_connector_width", "pad_object_connector_penetration", "hollowing_enable",
    "hollowing_min_thickness", "hollowing_quality", "hollowing_closing_distance", "output_filename_format",
    "default_sla_print_profile", "compatible_printers", "compatible_printers_condition", "inherits"};

static std::vector<std::string> s_Preset_sla_material_options{
    "material_colour", "material_type", "initial_layer_height", "bottle_cost", "bottle_volume", "bottle_weight",
    "material_density", "exposure_time", "initial_exposure_time", "material_correction", "material_correction_x",
    "material_correction_y", "material_correction_z", "material_notes", "material_vendor", "material_print_speed",
    "area_fill", "default_sla_material_profile", "zcorrection_layers", "compatible_prints",
    "compatible_prints_condition", "compatible_printers", "compatible_printers_condition", "inherits",

    // overriden options
    "material_ow_support_head_front_diameter", "material_ow_support_head_penetration", "material_ow_support_head_width",
    ...
    "material_ow_branchingsupport_head_width", "material_ow_branchingsupport_pillar_diameter",

    "material_ow_support_points_density_relative", "material_ow_absolute_correction",
    "material_ow_elefant_foot_compensation"};

static std::vector<std::string> s_Preset_sla_tilt_options{"delay_before_exposure",
...
{
    return s_Preset_sla_tilt_options;
}

static std::vector<std::string> s_Preset_sla_material_options_all = boost::copy_range<std::vector<std::string>>(
    boost::join(s_Preset_sla_material_options, s_Preset_sla_tilt_options));

static std::vector<std::string> s_Preset_sla_printer_options{
    "printer_technology", "bed_shape", "bed_custom_texture", "bed_custom_model", "max_print_height", "display_width",
    "display_height", "display_pixels_x", "display_pixels_y", "display_mirror_x", "display_mirror_y",
    "display_orientation", "fast_tilt_time", "slow_tilt_time", "high_viscosity_tilt_time", //"area_fill",
    "relative_correction", "relative_correction_x", "relative_correction_y", "relative_correction_z",
    "absolute_correction", "elefant_foot_compensation", "elefant_foot_min_width", "gamma_correction",
    "min_exposure_time", "max_exposure_time", "min_initial_exposure_time", "max_initial_exposure_time",
    "sla_archive_format", "sla_output_precision",
    //FIXME the print host keys are left here just for conversion from the Printer preset to Physical Printer preset.
    "print_host", "printhost_apikey", "printhost_cafile", "printer_notes", "inherits"};

const std::vector<std::string> &Preset::print_options()
{
    return s_Preset_print_options;
}
const std::vector<std::string> &Preset::filament_options()
{
    return s_Preset_filament_options;
}
const std::vector<std::string> &Preset::machine_limits_options()
{
    return s_Preset_machine_limits_options;
}
...
PresetCollection::PresetCollection(Preset::Type type, const std::vector<std::string> &keys,
                                   const Slic3r::StaticPrintConfig &defaults, const std::string &default_name)
...
void PresetCollection::add_default_preset(const std::vector<std::string> &keys,
                                          const Slic3r::StaticPrintConfig &defaults, const std::string &preset_name)
{
    // Insert just the default preset.
                                    ...
                                    ForwardCompatibilitySubstitutionRule substitution_rule)
{
    ...
    // Fix for preset modification issue
    boost::filesystem::path dir =
        boost::filesystem::absolute(boost::filesystem::path(dir_path) / subdir).make_preferred();
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!config_substitutions.empty())
                        substitutions.push_back({preset.name, m_type, PresetConfigSubstitutions::Source::UserFile,
                                                 preset.file, std::move(config_substitutions)});
...
Preset &PresetCollection::load_preset(const std::string &path, const std::string &name,
                                      const DynamicPrintConfig &config, bool select)
{
...
static bool profile_print_params_same(const DynamicPrintConfig &cfg_old, const DynamicPrintConfig &cfg_new)
{
    ...
    for (const char *key :
         {"compatible_prints", "compatible_prints_condition", "compatible_printers", "compatible_printers_condition",
          ...
          "printer_settings_id", "filament_vendor", "printer_model", "printer_variant", "default_print_profile",
          "default_filament_profile", "default_sla_print_profile", "default_sla_material_profile",
    ...
    if (found)
    {
        if (select != LoadAndSelect::Never)
        {
            ...
            if (!profile_print_params_same(it->config, cfg))
            {
                ...
                for (const char *key :
                     {"print_settings_id", "filament_settings_id", "sla_print_settings_id", "sla_material_settings_id",
                      "printer_settings_id", "filament_vendor", "printer_model", "printer_variant",
                      "default_print_profile", "default_filament_profile", "default_sla_print_profile",
...
Preset &PresetCollection::load_preset(const std::string &path, const std::string &name, DynamicPrintConfig &&config,
                                      bool select)
{
...
const Preset *PresetCollection::get_selected_preset_parent() const
{
    ...
    if (inherits.empty())
    {
        ...
        preset = &this->default_preset(
            m_type == Preset::Type::TYPE_PRINTER && edited_preset.printer_technology() == ptSLA ? 1 : 0);
    }
    else
...
const std::string &PresetCollection::get_preset_name_by_alias(const std::string &alias) const
{
    for (
        ...
        auto it =
            Slic3r::lower_bound_by_predicate(m_map_alias_to_profile_name.begin(), m_map_alias_to_profile_name.end(),
        ...
        if (auto it_preset = this->find_preset_internal(it->second);
            it_preset != m_presets.end() && it_preset->name == it->second && it_preset->is_visible &&
...
const std::string &PresetCollection::get_preset_name_by_alias_invisible(const std::string &alias) const
{
    for (
        ...
        it != m_map_alias_to_profile_name.end() && it->first == alias; ++it)
        if (auto it_preset = this->find_preset_internal(it->second);
            it_preset != m_presets.end() && it_preset->name == it->second && it_preset->is_compatible)
            return it_preset->name;
    return alias;
}
                                      ...
                                      bool respect_active_preset /*= true*/)
{
    ...
    return (it != m_presets.end() && it->name == key.name)
               ? &this->preset(it - m_presets.begin(), respect_active_preset)
                                                    ...
                                                    PresetSelectCompatibleType unselect_if_incompatible)
{
    ...
    for (size_t idx_preset = m_num_default_presets; idx_preset < m_presets.size(); ++idx_preset)
    {
        ...
        const PresetWithVendorProfile this_preset_with_vendor_profile = this->get_preset_with_vendor_profile(
            preset_edited);
        bool was_compatible = preset_edited.is_compatible;
        preset_edited.is_compatible = is_compatible_with_printer(this_preset_with_vendor_profile, active_printer,
                                                                 &config);
...
template<class T>
void add_correct_opts_to_diff(const std::string &opt_key, t_config_option_keys &vec, const ConfigBase &other,
                              const ConfigBase &this_c)
{
    ...
    // Special handling for print_nozzle_diameters which might have empty values
    if (opt_key == "print_nozzle_diameters")
    {
        if (opt_init->values.empty() && opt_cur->values.empty())
        {
        ...
        if (opt_init->values.empty() != opt_cur->values.empty())
        {
            // One is empty, one isn't - they're different
    ...
    for (int i = 0; i < int(opt_cur->values.size()); i++)
    {
        ...
        if (opt_init->values.empty() || opt_cur->values[i] != opt_init->values[init_id])
    ...
    for (const t_config_option_key &opt_key : config_this.keys())
    {
        ...
        if (this_opt != nullptr && other_opt != nullptr && *this_opt != *other_opt)
        {
            ...
            else
            {
                ...
                {
                case coInts:
                    add_correct_opts_to_diff<ConfigOptionInts>(opt_key, diff, config_other, config_this);
                    break;
                case coBools:
                    add_correct_opts_to_diff<ConfigOptionBools>(opt_key, diff, config_other, config_this);
                    break;
                case coFloats:
                    add_correct_opts_to_diff<ConfigOptionFloats>(opt_key, diff, config_other, config_this);
                    break;
                case coStrings:
                    add_correct_opts_to_diff<ConfigOptionStrings>(opt_key, diff, config_other, config_this);
                    break;
                case coPercents:
                    add_correct_opts_to_diff<ConfigOptionPercents>(opt_key, diff, config_other, config_this);
                    break;
                case coPoints:
                    add_correct_opts_to_diff<ConfigOptionPoints>(opt_key, diff, config_other, config_this);
                    break;
                case coFloatsOrPercents:
                    add_correct_opts_to_diff<ConfigOptionFloatsOrPercents>(opt_key, diff, config_other, config_this);
                    break;
                case coEnums:
                    add_correct_opts_to_diff<ConfigOptionEnumsGeneric>(opt_key, diff, config_other, config_this);
                    break;
                default:
                    diff.emplace_back(opt_key);
                    break;
                ...
                if (opt_key == "nozzle_diameter" && static_cast<const ConfigOptionFloats *>(this_opt)->size() !=
                                                        static_cast<const ConfigOptionFloats *>(other_opt)->size())
                    diff.emplace_back(opt_key);
            }
        }
    }
...
bool PresetCollection::is_dirty(const Preset *edited, const Preset *reference)
{
    ...
    {
        // Special handling for print presets with print_nozzle_diameters
        if (edited->type == Preset::TYPE_PRINT)
        {
            // Ensure both configs have print_nozzle_diameters before comparison
            bool edited_has = edited->config.has("print_nozzle_diameters");
            bool ref_has = reference->config.has("print_nozzle_diameters");

            if (edited_has != ref_has)
            {
                // One has it, one doesn't - configs are different
                return true;
            }
        }
        ...
        {
            return true; // Consider it dirty if comparison fails
        }
        catch (...)
        {
            return true; // Consider it dirty if comparison fails
                                                         ...
                                                         const bool deep_compare /*= false*/)
{
    ...
    if (edited != nullptr && reference != nullptr)
    {
        ...
        // For printer presets, exclude nozzle_diameter from the dirty list since it's controlled by print presets
        if (edited != nullptr && edited->type == Preset::TYPE_PRINTER)
        {
            changed.erase(std::remove(changed.begin(), changed.end(), "nozzle_diameter"), changed.end());
            changed.erase(std::remove(changed.begin(), changed.end(), "nozzle_high_flow"), changed.end());
        }
    }
    return changed;
}
...
void PresetCollection::update_map_alias_to_profile_name()
{
    ...
    std::sort(m_map_alias_to_profile_name.begin(), m_map_alias_to_profile_name.end(),
...
void PresetCollection::update_map_system_profile_renamed()
{
    ...
    for (Preset &preset : m_presets)
        ...
        {
            const auto [it, success] = m_map_system_profile_renamed.insert(
                std::pair<std::string, std::string>(renamed_from, preset.name));
            if (!success)
                BOOST_LOG_TRIVIAL(error)
                    << boost::format(
                           "Preset name \"%1%\" was marked as renamed from \"%2%\", though preset name \"%3%\" was marked as renamed from \"%2%\" as well.") %
...
const Preset *PrinterPresetCollection::find_system_preset_by_model_and_variant(const std::string &model_id,
...
static std::vector<std::string> s_PhysicalPrinter_opts{
    ...
    "preset_names", "printer_technology", "host_type", "print_host", "printhost_apikey", "printhost_cafile",
...
std::vector<std::string> PhysicalPrinter::presets_with_print_host_information(
...
{
    return config.opt_string("print_host").empty() && config.opt_string("printhost_apikey").empty() &&
           config.opt_string("printhost_cafile").empty() && config.opt_string("printhost_port").empty() &&
           config.opt_string("printhost_user").empty() && config.opt_string("printhost_password").empty();
}
...
static void update_preset_names_if_were_renamed(std::set<std::string> &preset_names,
                                                const PrinterPresetCollection &printer_presets)
{
        ...
        else
        {
            if (const std::string *new_name = printer_presets.get_preset_name_renamed(preset_name))
            {
                BOOST_LOG_TRIVIAL(warning)
                    << "Printer preset present " << preset_name << "was renamed to: " << *new_name << std::endl;
                                              ...
                                              ForwardCompatibilitySubstitutionRule substitution_rule)
{
        ...
        if (Slic3r::is_ini_file(dir_entry))
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!config_substitutions.empty())
                        substitutions.push_back({name, Preset::TYPE_PHYSICAL_PRINTER,
                                                 PresetConfigSubstitutions::Source::UserFile, printer.file,
    ...
    m_printers.insert(m_printers.end(), std::make_move_iterator(printers_loaded.begin()),
                      std::make_move_iterator(printers_loaded.end()));
    std::sort(m_printers.begin(), m_printers.end());
    if (!errors_cummulative.empty())
        throw Slic3r::RuntimeError(errors_cummulative);
}

void PhysicalPrinterCollection::load_printer(const std::string &path, const std::string &name,
                                             DynamicPrintConfig &&config, bool select, bool save /* = false*/)
{
...
std::deque<PhysicalPrinter>::iterator PhysicalPrinterCollection::find_printer_internal(
    const std::string &name, bool case_sensitive_search /* = true*/)
{
    if (case_sensitive_search)
        return Slic3r::lower_bound_by_predicate(m_printers.begin(), m_printers.end(),
...
{
    return (m_idx_selected == size_t(-1)) ? std::string()
                                          : this->get_selected_printer().get_full_name(m_selected_preset);
}
...
bool PhysicalPrinterCollection::is_selected(PhysicalPrinterCollection::ConstIterator it,
                                            const std::string &preset_name) const
{
    return m_idx_selected == size_t(it - m_printers.begin()) && m_selected_preset == preset_name;
}

ExtruderFilaments::ExtruderFilaments(PresetCollection *filaments_collection, size_t extruder_id,
                                     std::string selected_name /* = std::string()*/)
    : m_filaments(filaments_collection), m_extruder_id(extruder_id)
{
...
const std::string &ExtruderFilaments::get_preset_name_by_alias(const std::string &alias) const
{
    ...
    for (
        // Find the 1st profile name with the alias.
        auto it = Slic3r::lower_bound_by_predicate(aliases_map.begin(), aliases_map.end(),
                                                   [&alias](auto &l) { return l.first < alias; });
        // Continue over all profile names with the same alias.
        ...
        if (auto it_filament = find_filament_internal(it->second);
            ...
            it_filament->preset->is_visible &&
            (it_filament->is_compatible || size_t(it_filament - m_extr_filaments.begin()) == m_idx_selected))
                                                     ...
                                                     PresetSelectCompatibleType unselect_if_incompatible)
{
    ...
    for (size_t idx_preset = num_default_presets; idx_preset < m_extr_filaments.size(); ++idx_preset)
    {
        ...
        const PresetWithVendorProfile this_preset_with_vendor_profile = m_filaments->get_preset_with_vendor_profile(
            *preset);
        bool was_compatible = extr_filament.is_compatible;
        extr_filament.is_compatible = is_compatible_with_printer(this_preset_with_vendor_profile,
                                                                 active_printer_adjusted, &config);
        if (active_print != nullptr)
            extr_filament.is_compatible &= is_compatible_with_print(this_preset_with_vendor_profile, *active_print,
                                                                    active_printer_adjusted);
    ...
    if (preset.vendor != nullptr)
    {
        ...
        if (printer_model != nullptr && !printer_model->value.empty())
        {
            auto it = std::find_if(preset.vendor->models.begin(), preset.vendor->models.end(),
                                   [printer_model](const VendorProfile::PrinterModel &pm)
                                   { return pm.id == printer_model->value; });
...
bool compare_vendor_profile_printers(const VendorProfile &vp_old, const VendorProfile &vp_new,
                                     std::vector<std::string> &new_printers)
{
    for (const VendorProfile::PrinterModel &model : vp_new.models)
    {
        if (std::find_if(vp_old.models.begin(), vp_old.models.end(), [model](const VendorProfile::PrinterModel &pm)
                         { return pm.id == model.id; }) == vp_old.models.end())

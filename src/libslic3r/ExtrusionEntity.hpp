...
public:
    ...
    {
        m_extrusion_entity = rhs.m_extrusion_entity;
    ...
    {
        return dynamic_cast<const Type *>(m_extrusion_entity);
    }
...
struct ExtrusionFlow
{
    ...
    ExtrusionFlow(double mm3_per_mm, float width, float height) : mm3_per_mm{mm3_per_mm}, width{width}, height{height}
    {
    }
    ExtrusionFlow(const Flow &flow) : mm3_per_mm(flow.mm3_per_mm()), width(flow.width()), height(flow.height()) {}
...
{
    if (std::abs(lhs.start_distance_from_prev_layer - rhs.start_distance_from_prev_layer) >
        std::numeric_limits<float>::epsilon())
    {
        return false;
    }
    if (std::abs(lhs.end_distance_from_prev_layer - rhs.end_distance_from_prev_layer) >
        std::numeric_limits<float>::epsilon())
    {
        return false;
    }
...
struct ExtrusionAttributes : ExtrusionFlow
{
    ...
    ExtrusionAttributes(ExtrusionRole role, const ExtrusionFlow &flow, const uint16_t perimeter_index)
        : role{role}, ExtrusionFlow{flow}, perimeter_index{perimeter_index}
    {
    }
...
inline bool operator==(const ExtrusionAttributes &lhs, const ExtrusionAttributes &rhs)
{
    ...
    return static_cast<const ExtrusionFlow &>(lhs) == static_cast<const ExtrusionFlow &>(rhs) && lhs.role == rhs.role &&
           lhs.overhang_attributes == rhs.overhang_attributes;
}
...
public:
    ...
    ExtrusionPath(const Polyline &polyline, const ExtrusionAttributes &attribs)
        : polyline(polyline), m_attributes(attribs)
    {
    }
    ExtrusionPath(Polyline &&polyline, const ExtrusionAttributes &attribs)
        : polyline(std::move(polyline)), m_attributes(attribs)
    {
    }
    ...
    void set_mm3_per_mm(double value) { m_attributes.mm3_per_mm = value; }
    // Minimum volumetric velocity of this extrusion entity. Used by the constant nozzle pressure algorithm.
...
public:
    ...
    ExtrusionPathOriented(const Polyline &polyline, const ExtrusionAttributes &attribs)
        : ExtrusionPath(polyline, attribs)
    {
    }
    ExtrusionPathOriented(Polyline &&polyline, const ExtrusionAttributes &attribs)
        : ExtrusionPath(std::move(polyline), attribs)
    {
    }
...
public:
    ...
    {
        auto &path = this->paths[this->paths.size() / 2];
        return path.polyline.points[path.polyline.size() / 2];
    }
...
inline void extrusion_entities_append_paths(ExtrusionEntitiesPtr &dst, const Polylines &polylines,
                                            const ExtrusionAttributes &attributes, bool can_reverse = true)
{
    dst.reserve(dst.size() + polylines.size());
    for (const Polyline &polyline : polylines)
        if (polyline.is_valid())
            dst.emplace_back(can_reverse ? new ExtrusionPath(polyline, attributes)
                                         : new ExtrusionPathOriented(polyline, attributes));
}

inline void extrusion_entities_append_paths(ExtrusionEntitiesPtr &dst, Polylines &&polylines,
                                            const ExtrusionAttributes &attributes, bool can_reverse = true)
{
    dst.reserve(dst.size() + polylines.size());
    for (Polyline &polyline : polylines)
        if (polyline.is_valid())
            dst.emplace_back(can_reverse ? new ExtrusionPath(std::move(polyline), attributes)
                                         : new ExtrusionPathOriented(std::move(polyline), attributes));
    polylines.clear();
}

inline void extrusion_entities_append_loops(ExtrusionEntitiesPtr &dst, Polygons &&loops,
...
inline void extrusion_entities_append_loops_and_paths(ExtrusionEntitiesPtr &dst, Polylines &&polylines,
                                                      const ExtrusionAttributes &attributes)
{
    dst.reserve(dst.size() + polylines.size());
    for (Polyline &polyline : polylines)

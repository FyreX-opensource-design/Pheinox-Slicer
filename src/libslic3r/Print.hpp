...
public:
    ...
    static void collect_object_printing_extruders(const PrintConfig &print_config,
                                                  const PrintRegionConfig &region_config, const bool has_brim,
...
{
    return lhs.config_hash() == rhs.config_hash() && lhs.config() == rhs.config();
}
inline bool operator!=(const PrintRegion &lhs, const PrintRegion &rhs)
{
    return !(lhs == rhs);
...
public:
    ...
    static_assert(sizeof(Eigen::AlignedBox<float, 3>) == 24,
                  "Eigen::AlignedBox<float, 3> is not being vectorized, thus it does not need to be aligned");
    ...
    struct LayerRangeRegions
    {
        ...
        bool has_volume(const ObjectID id) const
        {
            auto it = lower_bound_by_predicate(this->volumes.begin(), this->volumes.end(),
                                               [id](const VolumeExtents &l) { return l.volume_id < id; });
...
public:
    ...
    {
        return SpanOfConstPtrs<SupportLayer>(const_cast<const SupportLayer *const *const>(m_support_layers.data()),
    ...
    {
        return BoundingBox(Point(-m_size.x() / coord_t(2), -m_size.y() / coord_t(2)),
                           Point(m_size.x() / coord_t(2), m_size.y() / coord_t(2)));
    }
    ...
    SupportLayerPtrs::iterator insert_support_layer(SupportLayerPtrs::iterator pos, size_t id, size_t interface_id,
    ...
    static SlicingParameters slicing_parameters(const DynamicPrintConfig &full_config, const ModelObject &model_object,
                                                float object_max_z, const Vec3d &object_shrinkage_compensation);
    ...
    bool has_painted_enforcers(TriangleStateType type) const;
...
private:
    ...
    bool invalidate_state_by_config_options(const ConfigOptionResolver &old_config,
    ...
    static PrintObjectConfig object_config_from_model_object(const PrintObjectConfig &default_object_config,
...
public:
    ...
    ApplyStatus apply(const Model &model, DynamicPrintConfig config,
                      std::vector<std::string> *warnings = nullptr) override;
    ...
    {
        return SpanOfConstPtrs<PrintObject>(const_cast<const PrintObject *const *const>(m_objects.data()),
                                            m_objects.size());
    }
...
private:
    bool invalidate_state_by_config_options(const ConfigOptionResolver &new_config,
                                            const std::vector<t_config_option_key> &opt_keys);
    ...
    static bool is_shared_print_object_step_valid_unguarded(SpanOfConstPtrs<PrintObject> print_objects,

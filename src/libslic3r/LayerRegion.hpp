...
public:
    ...
    // and their bounding boxes
    [[nodiscard]] const BoundingBoxes &fill_expolygons_composite_bboxes() const
    {
    ...
    // Remove fill surfaces too narrow for useful extrusion (slivers from geometry operations)
    void remove_narrow_fill_surfaces();
    ...
    [[nodiscard]] const ExtrusionEntityCollection &interlocking_gap_fills() const { return m_interlocking_gap_fills; }
    ...
    void elephant_foot_compensation_step(const float elephant_foot_compensation_perimeter_step,
    ...
    void set_num_interlocking_shells(int count) { m_num_interlocking_shells = count; }
    int num_interlocking_shells() const { return m_num_interlocking_shells; }
...
private:
    ...
    // These CANNOT be stored in m_thin_fills or m_fills without causing global corruption.
    ExtrusionEntityCollection m_interlocking_gap_fills;

    int m_num_interlocking_shells = 0;

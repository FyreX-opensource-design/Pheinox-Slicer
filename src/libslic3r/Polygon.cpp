...
{
    return Clipper2Lib::IsPositive(Slic3rPoints_to_ClipperPath(this->points));
}
...
Points filter_convex_concave_points_by_angle_threshold(const Points &poly, double angle_threshold,
                                                       ConvexConcaveFilterFn convex_concave_filter)
{
    ...
    if (angle_threshold < EPSILON)
    {
        ...
        return filter_points_by_vectors(
            poly, [convex_concave_filter, cos_angle](const Vec2d &v1, const Vec2d &v2)
    ...
    if (!this->points.empty())
    {
        for (size_t i = 0; i < this->points.size(); ++i)
        {
            ...
            if (div > 0.)
            {
                ...
                if (t > 0. && t < 1.)
                {
                    Point foot(coord_t(floor(coordf_t(pt0(0)) + t * v1(0) + 0.5)),
                               coord_t(floor(coordf_t(pt0(1)) + t * v1(1) + 0.5)));
...
bool has_duplicate_points(const Polygons &polys)
{
    ...
    struct PointHash
    {
        uint64_t operator()(const Point &p) const noexcept
        {
            ...
            memcpy(h, &p, sizeof(p));
            return ankerl::unordered_dense::detail::wyhash::hash(h[0]) ^
                   ankerl::unordered_dense::detail::wyhash::hash(h[1]);
        }
    };
...
bool remove_sticks(Polygon &poly)
{
    ...
    while (poly.points.size() >= 3 &&
           is_stick(poly.points[poly.points.size() - 2], poly.points.back(), poly.points.front()))
    {
...
static inline void simplify_polygon_impl(const Points &points, double tolerance, bool strictly_simple, Polygons &out)
{
    ...
    simplified.pop_back();
    // Simplify the decimated contour by Clipper2.
    Clipper2Lib::Path64 clipper_path = Slic3rPoints_to_ClipperPath(simplified);
    bool ccw = Clipper2Lib::Area(clipper_path) > 0.;
    Clipper2Lib::Paths64 clipper_paths;
    clipper_paths.push_back(clipper_path);
    Clipper2Lib::Paths64 simplified_paths = Clipper2Lib::SimplifyPaths(clipper_paths, 2.0);
...
{
    Clipper2Lib::Point64 clipper_pt(p.x(), p.y());
    Clipper2Lib::Path64 clipper_path = Slic3rPoints_to_ClipperPath(polygon.points);
    Clipper2Lib::PointInPolygonResult result = Clipper2Lib::PointInPolygon(clipper_pt, clipper_path);

    if (result == Clipper2Lib::PointInPolygonResult::IsOn)
...
bool contains(const Polygons &polygons, const Point &p, bool border_result)
{
    ...
    {
        Clipper2Lib::Path64 clipper_path = Slic3rPoints_to_ClipperPath(poly.points);
        Clipper2Lib::PointInPolygonResult result = Clipper2Lib::PointInPolygon(clipper_pt, clipper_path);

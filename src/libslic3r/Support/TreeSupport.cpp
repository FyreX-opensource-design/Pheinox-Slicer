...
static inline void validate_range(const Point &pt)
{
    ...
    if (pt.x() > hi || pt.y() > hi || -pt.x() > hi || -pt.y() > hi)
        throw Clipper2Lib::Clipper2Exception("Coordinate outside allowed range");
}
...
static std::vector<std::pair<TreeSupportSettings, std::vector<size_t>>> group_meshes(
    const Print &print, const std::vector<size_t> &print_object_ids)
{
    ...
    for (size_t object_id : print_object_ids)
    {
        ...
        if (object_config.support_material_contact_distance.value == stcgNoGap)
    ...
    for (size_t object_id : print_object_ids)
    {
        ...
        // In mixed mode, organic paint regions generate tree supports even when primary style is Snug
        ...
        if (print.get_object(largest_printed_mesh_idx)->layers().back()->print_z <
...
[[nodiscard]] static const std::vector<Polygons> generate_overhangs(const TreeSupportSettings &settings,
            ...
            for (LayerIndex layer_id = range.begin(); layer_id < range.end(); ++layer_id)
            {
                ...
                if (!enforcers_layers.empty() && !enforcers_layers[layer_id].empty())
                {
                        ...
                        !enforced_overhangs.empty())
                    {
                        ...
                        // otherwise tiny specs get expanded to larger areas and bypass the filter.
                        if (config.support_material_min_area.value > 0)
                        {
                            const double min_enforcer_area = scaled<double>(
                                scaled<double>(config.support_material_min_area.value));
                            remove_small(enforced_overhangs, min_enforcer_area);
                        ...
                        if (!enforced_overhangs.empty())
                        {
                            ...
                            //                    if (! intersecting_edges(enforced_overhangs).empty())
                            {
                                ...
                                SVG::export_expolygons(
                                    debug_out_path("treesupport-self-intersections-%d.svg", ++irun),
                                    {{{current_layer.lslices}, {"current_layer.lslices", "yellow", 0.5f}},
                                     {{lower_layer.lslices}, {"lower_layer.lslices", "gray", 0.5f}},
                                     {{union_ex(enforced_overhangs)},
                                      {"enforced_overhangs", "red", "black", "", scaled<coord_t>(0.1f), 0.5f}}});
                            }
#endif // TREESUPPORT_DEBUG_SVG
                            //check_self_intersections(enforced_overhangs, "generate_overhangs - enforced overhangs2");
                            overhangs = overhangs.empty() ? std::move(enforced_overhangs)
                                                          : union_(overhangs, enforced_overhangs);
 ...
 */
[[nodiscard]] static LayerIndex precalculate(const Print &print, const std::vector<Polygons> &overhangs,
                                             const TreeSupportSettings &config, const std::vector<size_t> &object_ids,
...
[[nodiscard]] static LineInformations convert_lines_to_internal(const TreeModelVolumes &volumes,
                                                                ...
                                                                const Polylines &polylines, LayerIndex layer_idx)
{
    ...
    for (const Polyline &line : polylines)
    {
        ...
        for (Point p : line)
        {
            if (!contains(volumes.getAvoidance(config.getRadius(0), layer_idx,
                                               TreeModelVolumes::AvoidanceType::FastSafe, false, min_xy_dist),
                          p))
                res_line.emplace_back(p, LineStatus::TO_BP_SAFE);
            else if (!contains(volumes.getAvoidance(config.getRadius(0), layer_idx,
                ...
                res_line.emplace_back(p, LineStatus::TO_BP);
                     ...
                     !contains(volumes.getAvoidance(config.getRadius(0), layer_idx,
                                                    TreeModelVolumes::AvoidanceType::FastSafe, true, min_xy_dist),
                               p))
                res_line.emplace_back(p, LineStatus::TO_MODEL_GRACIOUS_SAFE);
            else if (config.support_rests_on_model &&
                     !contains(volumes.getAvoidance(config.getRadius(0), layer_idx,
                                                    TreeModelVolumes::AvoidanceType::Fast, true, min_xy_dist),
                               p))
                res_line.emplace_back(p, LineStatus::TO_MODEL_GRACIOUS);
            else if (config.support_rests_on_model &&
                     !contains(volumes.getCollision(config.getRadius(0), layer_idx, min_xy_dist), p))
 ...
 */
[[nodiscard]] static bool evaluate_point_for_next_layer_function(const TreeModelVolumes &volumes,
                                                                 const TreeSupportSettings &config,
                                                                 ...
                                                                 const std::pair<Point, LineStatus> &p)
{
    ...
    const bool min_xy_dist = config.xy_distance > config.xy_min_distance;
    if (!contains(volumes.getAvoidance(config.getRadius(0), current_layer - 1,
                                       p.second == LineStatus::TO_BP_SAFE ? AvoidanceType::FastSafe
                                                                          : AvoidanceType::Fast,
                                       false, min_xy_dist),
                  p.first))
        return true;
    if (config.support_rests_on_model && (p.second != LineStatus::TO_BP && p.second != LineStatus::TO_BP_SAFE))
        return !contains(p.second == LineStatus::TO_MODEL_GRACIOUS || p.second == LineStatus::TO_MODEL_GRACIOUS_SAFE
                             ? volumes.getAvoidance(config.getRadius(0), current_layer - 1,
...
[[nodiscard]] static std::pair<LineInformations, LineInformations> split_lines(const LineInformations &lines,
                                                                               EvaluatePointFn evaluatePoint)
{
    ...
    return std::pair<std::vector<std::vector<std::pair<Point, LineStatus>>>,
                     std::vector<std::vector<std::pair<Point, LineStatus>>>>(keep, set_free);
}
...
static std::optional<std::pair<Point, size_t>> polyline_sample_next_point_at_distance(const Points &polyline,
                                                                                      const Point &start_pt,
                                                                                      size_t start_idx, double dist)
{
    ...
    for (size_t i = start_idx + 1; i < polyline.size(); ++i)
    {
        ...
        if ((p1 - start_pt).cast<int64_t>().squaredNorm() >= dist2i)
        {
            ...
            if (double l2_intersection = dist2 - l2_from_line; l2_intersection > -SCALED_EPSILON)
            {
 ...
 */
[[nodiscard]] static Polylines ensure_maximum_distance_polyline(const Polylines &input, double distance,
                                                                size_t min_points)
{
    Polylines result;
    for (Polyline part : input)
    {
        ...
        else
        {
            ...
            while (line.size() < min_points && current_distance >= scaled<double>(0.1))
            {
                ...
                while ((next_point = polyline_sample_next_point_at_distance(part.points, current_point, current_index,
                                                                            next_distance)))
                {
                    ...
                    for (Point p : line)
                        min_distance_to_existing_point = std::min(min_distance_to_existing_point,
                                                                  (p - next_point->first).cast<double>().norm());
                    ...
                    else
                    {
                        if (current_point == next_point->first)
                        {
                            ...
                            BOOST_LOG_TRIVIAL(warning)
                                << "Tree Support: Encountered a fixpoint in polyline_sample_next_point_at_distance. This is expected to happen if the distance (currently "
                                << next_distance << ") is smaller than 100";
                            tree_supports_show_error(
                                "Encountered issue while placing tips. Some tips may be missing."sv, true);
                        ...
                        next_distance = std::max(current_distance - min_distance_to_existing_point,
                                                 scaled<double>(0.1));
                        current_point = next_point->first;
                        current_index = next_point->second;
                    }
...
[[nodiscard]] static Polylines generate_support_infill_lines(
    ...
    const Polygons &polygon, const SupportParameters &support_params, bool roof, LayerIndex layer_idx,
...
//    const bool zig_zaggify_infill = roof ? pattern == EFillMethod::ZIG_ZAG : config.zig_zaggify_support;
    ...
    std::unique_ptr<Fill> filler = std::unique_ptr<Fill>(
        Fill::new_from_type(roof ? support_params.interface_fill_pattern : support_params.base_fill_pattern));
    FillParams fill_params;

    filler->layer_id = layer_idx;
    filler->spacing = flow.spacing();
    filler->bounding_width = filler->spacing;
    filler->angle = roof ?
    ...
    fill_params.density = float(roof ? support_params.interface_density
                                     : scaled<float>(filler->spacing) /
                                           (scaled<float>(filler->spacing) + float(support_infill_distance)));
    ...
    for (ExPolygon &expoly : union_ex(polygon))
    {
...
#ifdef TREE_SUPPORT_SHOW_ERRORS_WIN32
        if (area(expoly) <= 0.)
            ::MessageBoxA(nullptr, "TreeSupport infill negative area", "Bug detected!",
                          MB_OK | MB_SYSTEMMODAL | MB_SETFOREGROUND | MB_ICONWARNING);
#endif // TREE_SUPPORT_SHOW_ERRORS_WIN32
        ...
        try
        {
            ...
            if (!pl.empty() && !get_extents(surface.expolygon).inflated(SCALED_EPSILON).contains(get_extents(pl)))
                ::MessageBoxA(nullptr, "TreeSupport infill failure", "Bug detected!",
                              MB_OK | MB_SYSTEMMODAL | MB_SETFOREGROUND | MB_ICONWARNING);
#endif // TREE_SUPPORT_SHOW_ERRORS_WIN32
 ...
 */
[[nodiscard]] static Polygons safe_offset_inc(const Polygons &me, coord_t distance, const Polygons &collision,
                                              ...
                                              size_t min_amount_offset)
{
    ...
    {
        ret = diff(offset(ret, step_size, JoinType::Round, scaled<float>(0.01)), collision_trimmed());
    ...
    if (last_offset > SCALED_EPSILON)
        ret = offset(ret, distance - steps * step_size, JoinType::Round, scaled<float>(0.01));
    ret = polygons_simplify(ret, scaled<double>(0.015), polygons_strictly_simple);
...
{
public:
    RichInterfacePlacer(const InterfacePlacer &interface_placer, const TreeModelVolumes &volumes,
                        bool force_tip_to_roof, size_t num_support_layers, std::vector<SupportElements> &move_bounds)
        : InterfacePlacer(interface_placer)
        , volumes(volumes)
        , force_tip_to_roof(force_tip_to_roof)
...
public:
        ...
        size_t dont_move_until)
    {
        ...
        for (const LineInformation &line : lines)
        {
            ...
            bool disable_ovalistation = config.min_radius < 3 * config.support_line_width && roof_tip_layers == 0 &&
                                        dtt_roof_tip == 0 && line.size() > 5;
            for (const std::pair<Point, LineStatus> &point_data : line)
                add_point_as_influence_area(point_data, insert_layer_idx - dtt_roof_tip,
                                            ...
                                            // supports roof
                                            dtt_roof_tip + supports_roof_layers > 0, disable_ovalistation);
        }
    }

private:
    // called by this->add_points_along_lines()
    void add_point_as_influence_area(std::pair<Point, LineStatus> p, LayerIndex insert_layer, size_t dont_move_until,
                                     bool roof, bool skip_ovalisation)
    {
        ...
        {
            Point hash_pos = p.first / coord_t((config.min_radius + coord_t(1)) / coord_t(10));
...
int generate_raft_contact(const PrintObject &print_object, const TreeSupportSettings &config,
                          InterfacePlacer &interface_placer)
{
    ...
    if (print_object.has_raft() && print_object.layer_count() > 0)
    {
        ...
        while (raft_contact_layer_idx > 0 && config.raft_layers[raft_contact_layer_idx] >
                                                 print_object.slicing_parameters().raft_contact_top_z + EPSILON)
        ...
        interface_placer.add_roof_unguarded(expansion > 0 ? expand(lslices, scaled<float>(expansion))
                                                          : to_polygons(lslices),
                                            raft_contact_layer_idx, 0);
    }
    return raft_contact_layer_idx;
}

void finalize_raft_contact(const PrintObject &print_object, const int raft_contact_layer_idx,
                           SupportGeneratorLayersPtr &top_contacts, std::vector<SupportElements> &move_bounds)
{
    if (raft_contact_layer_idx >= 0)
    {
        ...
        if (raft_contact_layer_idx >= 0 && print_object.config().raft_expansion.value > 0)
        {
            ...
            first_layer_move_bounds.erase(std::remove_if(first_layer_move_bounds.begin(), first_layer_move_bounds.end(),
                                                         ...
                                                         {
                                                             coordf_t dist;
                                                             if (grid.signed_distance_edges(el.state.result_on_layer,
                                                                                            threshold, dist))
                                                             {
    ...
    RichInterfacePlacer &interface_placer)
{
    ...
    auto generate_roof_lines = [&interface_placer, &mesh_group_settings](const Polygons &area,
                                                                         LayerIndex layer_idx) -> Polylines
    {
    ...
    if (large_horizontal_roof)
    {
            ...
            Polygons forbidden_next;
            {
                ...
                const Polygons &forbidden_next_raw =
                    interface_placer.config.support_rests_on_model
                        ? interface_placer.volumes.getCollision(interface_placer.config.getRadius(0),
                                                                layer_idx - (dtt_roof + 1), min_xy_dist)
                        : interface_placer.volumes.getAvoidance(interface_placer.config.getRadius(0),
                                                                layer_idx - (dtt_roof + 1),
                                                                TreeModelVolumes::AvoidanceType::Fast, false,
                                                                min_xy_dist);
        ...
        layer_generation_dtt =
            std::max(dtt_roof, size_t(1)) -
            1; // 1 inside max and -1 outside to avoid underflow. layer_generation_dtt=dtt_roof-1 if dtt_roof!=0;
    ...
    if (overhang_lines.empty())
    {
        ...
        if (point_count <= min_support_points)
        {
            ...
            Polygons reduced_overhang_area = offset(union_ex(overhang_area),
                                                    -interface_placer.config.support_line_width / 2.2, jtMiter, 1.2);
            polylines = ensure_maximum_distance_polyline(
                to_polylines(!reduced_overhang_area.empty() &&
                                     area(offset(diff_ex(overhang_area, reduced_overhang_area),
                                                 std::max(interface_placer.config.support_line_width, connect_length),
 ...
 */
static void generate_initial_areas(const PrintObject &print_object, const TreeModelVolumes &volumes,
                                   const TreeSupportSettings &config, const std::vector<Polygons> &overhangs,
                                   std::vector<SupportElements> &move_bounds, InterfacePlacer &interface_placer,
                                   std::function<void()> throw_on_cancel)
{
    using AvoidanceType = TreeModelVolumes::AvoidanceType;
    ...
    const coord_t circle_length_to_half_linewidth_change =
        config.min_radius < config.support_line_width
            ? config.min_radius / 2
            : scale_(sqrt(sqr(unscale<double>(config.min_radius)) -
                          sqr(unscale<double>(config.min_radius - config.support_line_width / 2))));
    ...
    const coord_t extra_outset = std::max(coord_t(0), config.min_radius - config.support_line_width / 2) +
                                 (min_xy_dist ? config.support_line_width / 2 : 0)
    ...
    const bool force_tip_to_roof = roof_enabled &&
                                   (interface_placer.support_parameters.soluble_interface ||
    ...
    if (config.z_distance_top_layers > 0)
    {
        ...
        if (mesh_group_settings.support_angle > EPSILON && mesh_group_settings.support_angle < 0.5 * M_PI - EPSILON)
        {
            ...
            max_overhang_insert_lag = std::max(max_overhang_insert_lag,
                                               round_up_divide(config.xy_distance, max_overhang_speed / 2));
        }
    }
    ...
    RichInterfacePlacer rich_interface_placer{interface_placer, volumes, force_tip_to_roof, num_support_layers,
                                              move_bounds};
            ...
            for (size_t raw_overhang_idx = range.begin(); raw_overhang_idx < range.end(); ++raw_overhang_idx)
            {
                ...
                Polygons relevant_forbidden;
                {
                    const Polygons &relevant_forbidden_raw = config.support_rests_on_model
                                                                 ? volumes.getCollision(config.getRadius(0), layer_idx,
                                                                                        min_xy_dist)
                                                                 : volumes.getAvoidance(config.getRadius(0), layer_idx,
                                                                                        AvoidanceType::Fast, false,
                                                                                        min_xy_dist);
                ...
                Polygons overhang_regular;
                {
                    ...
                    for (coord_t extra_total_offset_acc = 0;
                         !remaining_overhang.empty() &&
                         extra_total_offset_acc + config.support_line_width / 8 < extra_outset;)
                    {
                        const coord_t offset_current_step = std::min(extra_total_offset_acc +
                                                                                 2 * config.support_line_width >
                                                                             config.min_radius
                        ...
                        remaining_overhang = diff(remaining_overhang,
                                                  safe_offset_inc(overhang_regular, 1.5 * extra_total_offset_acc,
                                                                  raw_collision, offset_step, 0, 1));
                        // Extending the overhangs by the inflated remaining overhangs.
                        overhang_regular = union_(overhang_regular,
                                                  diff(safe_offset_inc(remaining_overhang, extra_total_offset_acc,
                                                                       raw_collision, offset_step, 0, 1),
                                                       relevant_forbidden));
...
static unsigned int move_inside(const Polygons &polygons, Point &from, int distance = 0,
                                int64_t maxDist2 = std::numeric_limits<int64_t>::max())
{
    ...
    for (unsigned int poly_idx = 0; poly_idx < polygons.size(); ++poly_idx)
    {
        ...
        bool projected_p_beyond_prev_segment = (p1 - p0).cast<int64_t>().dot((from - p0).cast<int64_t>()) >=
                                               (p1 - p0).cast<int64_t>().squaredNorm();
        for (const Point &p2 : poly)
        {
            ...
            if (ab_length2 <= 0)
            {            //A = B, i.e. the input polygon had two adjacent points on top of each other.
                ...
                if (projected_p_beyond_prev_segment)
                {
                    ...
                    if (dist2 < bestDist2)
                    {
                        ...
                        else
                        {
                            ...
                            auto inward_dir = perp(abd * (scaled<double>(10.0) / lab) +
                                                   p1p2 * (scaled<double>(10.0) / lp1p2));
                            // MM2INT(10.0) to retain precision for the eventual normalization
                            ret = x + (inward_dir * (distance / inward_dir.norm())).cast<coord_t>();
                            is_already_on_correct_side_of_boundary = inward_dir.dot((p - x).cast<double>()) *
                                                                         distance >=
                                                                     0;
                        }
                    }
            ...
            else
            {
                ...
                if (dist2 < bestDist2)
                {
                    ...
                    {
                        Vec2d abd = ab.cast<double>();
                        Vec2d inward_dir = perp(
                            abd * (distance / abd.norm())); // inward or outward depending on the sign of [distance]
...
static Point move_inside_if_outside(const Polygons &polygons, Point from, int distance = 0,
                                    int64_t maxDist2 = std::numeric_limits<int64_t>::max())
{
...
[[nodiscard]] static std::optional<SupportElementState> increase_single_area(
    const TreeModelVolumes &volumes, const TreeSupportSettings &config, const AreaIncreaseSettings &settings,
    const LayerIndex layer_idx, const SupportElement &parent, const Polygons &relevant_offset, Polygons &to_bp_data,
    Polygons &to_model_data, Polygons &increased, const coord_t overspeed, const bool mergelayer)
{
    ...
    if (settings.move)
    {
        ...
        if (overspeed > 0)
        {
            const coord_t safe_movement_distance =
                ...
                (std::min(config.z_distance_top_layers, config.z_distance_bottom_layers) > 0 ? config.min_feature_size
                                                                                             : 0);
            ...
            increased = safe_offset_inc(increased, overspeed,
                                        volumes.getWallRestriction(support_element_collision_radius(config,
                                                                                                    parent.state),
                                                                   layer_idx, parent.state.use_min_xy_dist),
                                        safe_movement_distance, safe_movement_distance + radius, 1);
        }
        if (settings.no_error && settings.move)
            // as JoinType::Round has to be used for offsets this simplify is VERY important for performance.
    ...
    {
        to_bp_data = safe_union(diff_clipped(increased, volumes.getAvoidance(radius, layer_idx - 1, settings.type,
                                                                             false, settings.use_min_distance)));
        if (!current_elem.to_buildplate && area(to_bp_data) > tiny_area_threshold)
        {
            ...
            current_elem.to_buildplate =
                true; // sometimes nodes that can reach the buildplate are marked as cant reach, tainting subtrees. This corrects it.
            BOOST_LOG_TRIVIAL(debug) << "Corrected taint leading to a wrong to model value on layer " << layer_idx - 1
                                     << " targeting " << current_elem.target_height << " with radius " << radius;
        }
    }
    if (config.support_rests_on_model)
    {
        if (mergelayer || current_elem.to_model_gracious)
            to_model_data = safe_union(
                diff_clipped(increased, volumes.getAvoidance(radius, layer_idx - 1, settings.type, true,
        ...
        if (!current_elem.to_model_gracious)
        {
            if (mergelayer && area(to_model_data) >= tiny_area_threshold)
            {
                ...
                BOOST_LOG_TRIVIAL(debug) << "Corrected taint leading to a wrong non gracious value on layer "
                                         << layer_idx - 1 << " targeting " << current_elem.target_height
                                         << " with radius " << radius;
            }
            else
                // Cannot route to gracious areas. Push the tree away from object and route it down anyways.
                to_model_data = safe_union(
                    diff_clipped(increased, volumes.getCollision(radius, layer_idx - 1, settings.use_min_distance)));
        }
    }
    ...
    if (settings.increase_radius && area(check_layer_data) > tiny_area_threshold)
    {
        ...
        {
            if (volumes.ceilRadius(next_radius, settings.use_min_distance) <=
                volumes.ceilRadius(radius, settings.use_min_distance))
                return true;
            ...
            if (current_elem.to_buildplate)
                ...
                to_bp_data_2 = diff_clipped(increased, volumes.getAvoidance(next_radius, layer_idx - 1, settings.type,
                                                                            false, settings.use_min_distance));
            ...
            if (config.support_rests_on_model && !current_elem.to_buildplate)
                to_model_data_2 = diff_clipped(increased,
                                               current_elem.to_model_gracious
                                                   ? volumes.getAvoidance(next_radius, layer_idx - 1, settings.type,
                                                                          true, settings.use_min_distance)
                                                   : volumes.getCollision(next_radius, layer_idx - 1,
                                                                          settings.use_min_distance));
        ...
        if (support_element_collision_radius(config, current_elem) < config.increase_radius_until_radius &&
            support_element_collision_radius(config, current_elem) < support_element_radius(config, current_elem))
        {
            coord_t target_radius = std::min(support_element_radius(config, current_elem),
                                             config.increase_radius_until_radius);
            ...
            while (current_ceil_radius < target_radius &&
                   validWithRadius(volumes.getRadiusNextCeil(current_ceil_radius + 1, settings.use_min_distance)))
            ...
            while (resulting_eff_dtt + 1 < current_elem.distance_to_top &&
                   config.getRadius(resulting_eff_dtt + 1, current_elem.elephant_foot_increases) <=
                       current_ceil_radius &&
                   config.getRadius(resulting_eff_dtt + 1, current_elem.elephant_foot_increases) <=
        ...
        const coord_t foot_radius_increase = std::max(config.bp_radius_increase_per_layer -
                                                          config.branch_radius_increase_per_layer,
                                                      0.0);
        ...
        double planned_foot_increase = std::min(1.0, double(config.recommendedMinRadius(layer_idx - 1) -
                                                            support_element_radius(config, current_elem)) /
        ...
        if (increase_bp_foot && support_element_radius(config, current_elem) >= config.branch_radius &&
            ...
            if (validWithRadius(config.getRadius(current_elem.effective_radius_height,
        ...
        if (ceil_radius_before != volumes.ceilRadius(radius, settings.use_min_distance))
        {
            if (current_elem.to_buildplate)
                to_bp_data = safe_union(
                    diff_clipped(increased, volumes.getAvoidance(radius, layer_idx - 1, settings.type, false,
                                                                 settings.use_min_distance)));
            if (config.support_rests_on_model && (!current_elem.to_buildplate || mergelayer))
                to_model_data = safe_union(
                    diff_clipped(increased,
                                 current_elem.to_model_gracious
                                     ? volumes.getAvoidance(radius, layer_idx - 1, settings.type, true,
                                                            settings.use_min_distance)
            ...
            if (area(check_layer_data) < tiny_area_threshold)
            {
                BOOST_LOG_TRIVIAL(error) << "Lost area by doing catch up from " << ceil_radius_before << " to radius "
                                         << volumes.ceilRadius(support_element_collision_radius(config, current_elem),
                                                               settings.use_min_distance);
    ...
    return area(check_layer_data) > tiny_area_threshold ? std::optional<SupportElementState>(current_elem)
                                                        : std::optional<SupportElementState>();
}
...
struct SupportElementMerging
{
    ...
    const Point centroid() const { return (bbox_data.min() + bbox_data.max()) / coord_t(2); }
    void set_bbox(const BoundingBox &abbox)
    {
        Point eps{coord_t(SCALED_EPSILON), coord_t(SCALED_EPSILON)};
        bbox_data = {abbox.min - eps, abbox.max + eps};
    }
 ...
 * \param mergelayer[in] Will the merge method be called on this layer. This information is required as some calculation can be avoided if they are not required for merging.
 */
static void increase_areas_one_layer(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                     ...
                                     const bool mergelayer, std::function<void()> throw_on_cancel)
{
    using AvoidanceType = TreeModelVolumes::AvoidanceType;
        ...
        [&](const tbb::blocked_range<size_t> &range)
        {
            for (size_t merging_area_idx = range.begin(); merging_area_idx < range.end(); ++merging_area_idx)
            {
                ...
                const Polygons &wall_restriction =
                    ...
                    volumes.getWallRestriction(support_element_collision_radius(config, parent.state), layer_idx,
                                               parent.state.use_min_xy_dist);

#ifdef TREESUPPORT_DEBUG_SVG
                SVG::export_expolygons(
                    debug_out_path("treesupport-increase_areas_one_layer-%d-%ld.svg", layer_idx, int(merging_area_idx)),
                ...
                coord_t extra_speed =
                    5; // The extra speed is added to both movement distances. Also move 5 microns faster than allowed to avoid rounding errors, this may cause issues at VERY VERY small layer heights.
                coord_t extra_slow_speed = 0; // Only added to the slow movement distance.
                const coord_t ceiled_parent_radius = volumes.ceilRadius(support_element_collision_radius(config,
                                                                                                         parent.state),
                                                                        parent.state.use_min_xy_dist);
                coord_t projected_radius_increased = config.getRadius(parent.state.effective_radius_height + 1,
                                                                      parent.state.elephant_foot_increases);
                coord_t projected_radius_delta = projected_radius_increased -
             ...
             */
                const coord_t safe_movement_distance = (elem.use_min_xy_dist ? config.xy_min_distance
                                                                             : config.xy_distance) +
                                                       (std::min(config.z_distance_top_layers,
                                                                 config.z_distance_bottom_layers) > 0
                                                            ? config.min_feature_size
                                                            : 0);
                if (ceiled_parent_radius ==
                        volumes.ceilRadius(projected_radius_increased, parent.state.use_min_xy_dist) ||
                    ...
                    extra_slow_speed += std::min(projected_radius_delta,
                ...
                if (config.layer_start_bp_radius > layer_idx &&
                    config.recommendedMinRadius(layer_idx - 1) <
                        config.getRadius(elem.effective_radius_height + 1, elem.elephant_foot_increases))
                {
                    // can guarantee elephant foot radius increase
                    if (ceiled_parent_radius ==
                        volumes.ceilRadius(config.getRadius(parent.state.effective_radius_height + 1,
                                                            parent.state.elephant_foot_increases + 1),
                                           parent.state.use_min_xy_dist))
                ...
                constexpr bool increase_radius = true, no_error = true, use_min_radius = true,
                               move = true; // aliases for better readability
                ...
                const bool avoidance_speed_mismatch = parent_moved_slow &&
                                                      elem.last_area_increase.type != AvoidanceType::Slow;
                if (elem.last_area_increase.move && elem.last_area_increase.no_error && elem.can_use_safe_radius &&
                    !mergelayer && !avoidance_speed_mismatch &&
                    (elem.distance_to_top >= config.tip_layers || parent_moved_slow))
                {
                    ...
                    insertSetting({elem.last_area_increase.type,
                                   elem.last_area_increase.increase_speed < config.maximum_move_distance ? slow_speed
                                                                                                         : fast_speed,
                                   increase_radius, elem.last_area_increase.no_error, !use_min_radius,
                ...
                if (!elem.can_use_safe_radius)
                {
                    ...
                    insertSetting({AvoidanceType::Slow, slow_speed, increase_radius, no_error, !use_min_radius, !move},
                    ...
                    if (elem.distance_to_top < round_up_divide(config.tip_layers, size_t(2)))
                        insertSetting({AvoidanceType::Fast, slow_speed, increase_radius, no_error, !use_min_radius,
                                       !move},
                                      true);
                    insertSetting({AvoidanceType::FastSafe, fast_speed, increase_radius, no_error, !use_min_radius,
                                   !move},
                                  true); // did we manage to avoid the hole
                    insertSetting({AvoidanceType::FastSafe, fast_speed, !increase_radius, no_error, !use_min_radius,
                                   move},
                                  true);
                    insertSetting({AvoidanceType::Fast, fast_speed, !increase_radius, no_error, !use_min_radius, move},
                                  true);
                }
                else
                {
                    ...
                    if (elem.distance_to_top < config.tip_layers)
                        insertSetting({AvoidanceType::FastSafe, slow_speed, increase_radius, no_error, !use_min_radius,
                                       move},
                                      true);
                    insertSetting({AvoidanceType::FastSafe, fast_speed, increase_radius, no_error, !use_min_radius,
                                   move},
                ...
                if (elem.use_min_xy_dist)
                {
                    ...
                    for (AreaIncreaseSettings settings : order)
                    {
                        ...
                        new_order.push_back({settings.type, settings.increase_speed, settings.increase_radius,
                                             settings.no_error, use_min_radius, settings.move});
                    }
                    order = new_order;
                }
                if (elem.to_buildplate ||
                    (elem.to_model_gracious &&
                     intersection(parent.influence_area, volumes.getPlaceableAreas(radius, layer_idx, throw_on_cancel))
                         .empty()))
                {
                    // error case
                    ...
                    insertSetting({AvoidanceType::Fast, fast_speed, !increase_radius, !no_error, elem.use_min_xy_dist,
                                   move},
                                  true);
                }
                if (elem.distance_to_top < elem.dont_move_until &&
                    elem.can_use_safe_radius) // only do not move when holes would be avoided in every case.
                ...
                bool offset_independant_faster =
                    radius / safe_movement_distance -
                        int(config.maximum_move_distance + extra_speed < radius + safe_movement_distance) >
                    round_up_divide((extra_speed + extra_slow_speed + config.maximum_move_distance_slow),
                ...
                for (const AreaIncreaseSettings &settings : order)
                {
                    if (settings.move)
                    {
                            ...
                            (settings.increase_speed == slow_speed || !offset_independant_faster))
                        {
                            ...
                            offset_slow =
                                safe_offset_inc(parent.influence_area,
                                                extra_speed + extra_slow_speed + config.maximum_move_distance_slow,
                                                wall_restriction, safe_movement_distance,
                                                offset_independant_faster ? safe_movement_distance + radius : 0, 2);
#ifdef TREESUPPORT_DEBUG_SVG
                            SVG::export_expolygons(
                                debug_out_path("treesupport-increase_areas_one_layer-slow-%d-%ld.svg", layer_idx,
                                               int(merging_area_idx)),
                                {{{union_ex(wall_restriction)}, {"wall_restricrictions", "gray", 0.5f}},
                                 {{union_ex(offset_slow)},
                                  {"offset_slow", "red", "black", "", scaled<coord_t>(0.1f), 0.5f}}});
#endif // TREESUPPORT_DEBUG_SVG
                        }
                        if (offset_fast.empty() && settings.increase_speed != slow_speed)
                        {
                            if (offset_independant_faster)
                                offset_fast =
                                    safe_offset_inc(parent.influence_area, extra_speed + config.maximum_move_distance,
                                                    wall_restriction, safe_movement_distance,
                                                    offset_independant_faster ? safe_movement_distance + radius : 0, 1);
                            else
                            {
                                const coord_t delta_slow_fast = config.maximum_move_distance -
                                                                (config.maximum_move_distance_slow + extra_slow_speed);
                                offset_fast = safe_offset_inc(offset_slow, delta_slow_fast, wall_restriction,
                                                              safe_movement_distance, safe_movement_distance + radius,
                                                              offset_independant_faster ? 2 : 1);
                            }
#ifdef TREESUPPORT_DEBUG_SVG
                            SVG::export_expolygons(
                                debug_out_path("treesupport-increase_areas_one_layer-fast-%d-%ld.svg", layer_idx,
                                               int(merging_area_idx)),
                                {{{union_ex(wall_restriction)}, {"wall_restricrictions", "gray", 0.5f}},
                                 {{union_ex(offset_fast)},
                                  {"offset_fast", "red", "black", "", scaled<coord_t>(0.1f), 0.5f}}});
#endif // TREESUPPORT_DEBUG_SVG
                        }
                    }
                    ...
                    if (!settings.no_error)
                    {
                        ...
                        Polygons lines_offset = offset(to_polylines(parent.influence_area), scaled<float>(0.005),
                                                       jtMiter, 1.2);
                        Polygons base_error_area = union_(parent.influence_area, lines_offset);
                        result = increase_single_area(volumes, config, settings, layer_idx, parent, base_error_area,
                                                      to_bp_data, to_model_data, inc_wo_collision,
                        ...
                        BOOST_LOG_TRIVIAL(warning)
                            ...
                            << "Influence area could not be increased! Data about the Influence area: "
                               "Radius: "
                            << radius << " at layer: " << layer_idx - 1 << " NextTarget: " << elem.layer_idx
                            << " Distance to top: " << elem.distance_to_top << " Elephant foot increases "
                            << elem.elephant_foot_increases << " use_min_xy_dist " << elem.use_min_xy_dist
                            << " to buildplate " << elem.to_buildplate << " gracious " << elem.to_model_gracious
                            << " safe " << elem.can_use_safe_radius << " until move " << elem.dont_move_until
                            << " \n "
                               "Parent "
                            << &parent << ": Radius: " << support_element_collision_radius(config, parent.state)
                            << " at layer: " << layer_idx << " NextTarget: " << parent.state.layer_idx
                            << " Distance to top: " << parent.state.distance_to_top << " Elephant foot increases "
                            << parent.state.elephant_foot_increases << "  use_min_xy_dist "
                            << parent.state.use_min_xy_dist << " to buildplate " << parent.state.to_buildplate
                            << " gracious " << parent.state.to_model_gracious << " safe "
                            << parent.state.can_use_safe_radius << " until move " << parent.state.dont_move_until;
                        ...
                        result = increase_single_area(volumes, config, settings, layer_idx, parent,
                                                      ...
                                                      to_bp_data, to_model_data, inc_wo_collision, 0, mergelayer);
                    ...
                    if (result)
                    {
                        ...
                        BOOST_LOG_TRIVIAL(error)
                            << "Trying to keep area by moving faster than intended: FAILURE! WRONG BRANCHES LIKLY!";
                }
                ...
                if (add)
                {
                    ...
                    Polygons max_influence_area = safe_union(diff_clipped(inc_wo_collision,
                                                                          volumes.getCollision(radius, layer_idx - 1,
...
[[nodiscard]] static SupportElementState merge_support_element_states(const SupportElementState &first,
                                                                      ...
                                                                      const TreeSupportSettings &config)
{
    ...
    out.to_model_gracious = first.to_model_gracious &&
                            second.to_model_gracious; // valid as we do not merge non-gracious with gracious

    out.elephant_foot_increases = 0;
    if (config.bp_radius_increase_per_layer > 0)
    {
        coord_t foot_increase_radius = std::abs(std::max(support_element_collision_radius(config, second),
                                                         support_element_collision_radius(config, first)) -
        ...
        out.elephant_foot_increases = foot_increase_radius /
    ...
    out.last_area_increase = {std::min(first.last_area_increase.type, second.last_area_increase.type),
                              std::min(first.last_area_increase.increase_speed,
                                       second.last_area_increase.increase_speed),
...
static bool merge_influence_areas_two_elements(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                               const LayerIndex layer_idx, SupportElementMerging &dst,
                                               SupportElementMerging &src)
{
    ...
    const coord_t real_radius_delta = std::abs(support_element_radius(config, bigger_rad.state) -
    ...
    auto intersect_small_with_bigger = [real_radius_delta, smaller_collision_radius, &collision,
                                        &config](const Polygons &small, const Polygons &bigger)
    {
        return intersection(safe_offset_inc(small, real_radius_delta, collision,
...
#ifdef TREES_MERGE_RATHER_LATER
    intersect = intersect_small_with_bigger(merging_to_bp ? smaller_rad.areas.to_bp_areas
                                                          : smaller_rad.areas.to_model_areas,
                                            merging_to_bp ? bigger_rad.areas.to_bp_areas
    ...
    new_state.increased_to_model_radius = increased_to_model_radius == 0
                                              ...
                                              std::max(dst.state.increased_to_model_radius,
                                                       src.state.increased_to_model_radius)
                                              : increased_to_model_radius;

    // Rather unioning with "intersect" due to some rounding errors.
    Polygons influence_areas = safe_union(intersect_small_with_bigger(smaller_rad.areas.influence_areas,
    ...
    if (merging_to_bp && config.support_rests_on_model)
        to_model_areas = new_state.to_model_gracious ?
                                                     // Rather unioning with "intersect" due to some rounding errors.
                             safe_union(intersect_small_with_bigger(smaller_rad.areas.to_model_areas,
                                                                    bigger_rad.areas.to_model_areas),
                                        intersect)
                                                     : influence_areas;
...
static SupportElementMerging *merge_influence_areas_leaves(const TreeModelVolumes &volumes,
                                                           const TreeSupportSettings &config,
                                                           const LayerIndex layer_idx,
...
static SupportElementMerging *merge_influence_areas_two_sets(
    ...
    SupportElementMerging *const dst_begin, SupportElementMerging *dst_end, SupportElementMerging *src_begin,
 ...
 * \param layer_idx[in] The current layer.
 */
static void merge_influence_areas(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                  const LayerIndex layer_idx, std::vector<SupportElementMerging> &influence_areas,
                                  std::function<void()> throw_on_cancel)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, num_buckets_initial),
                      [&](const tbb::blocked_range<size_t> &range)
                      {
                          for (size_t idx = range.begin(); idx < range.end(); ++idx)
                          {
                              ...
                              buckets[idx].second = merge_influence_areas_leaves(volumes, config, layer_idx,
                                                                                 buckets[idx].first,
                                                                                 buckets[idx].second);
                              throw_on_cancel();
                          }
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, buckets.size() / 2),
                          [&](const tbb::blocked_range<size_t> &range)
                          {
                              for (size_t idx = range.begin(); idx < range.end(); ++idx)
                              {
                                  ...
                                  // Merge bucket_count adjacent to each other, merging uneven bucket numbers into even buckets
                                  buckets[bucket_pair_idx].second = merge_influence_areas_two_sets(
                                      volumes, config, layer_idx, buckets[bucket_pair_idx].first,
                                      buckets[bucket_pair_idx].second, buckets[bucket_pair_idx + 1].first,
                                      buckets[bucket_pair_idx + 1].second);
                                  throw_on_cancel();
                              }
                          });
 ...
 */
static void create_layer_pathing(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                 std::vector<SupportElements> &move_bounds, std::function<void()> throw_on_cancel)
{
    ...
    size_t max_merge_every_x_layers = std::min(std::min(5000 / (std::max(config.maximum_move_distance, coord_t(100))),
                                                        1000 /
                                                            std::max(config.maximum_move_distance_slow, coord_t(20))),
    ...
    for (int layer_idx = int(move_bounds.size()) - 1; layer_idx > 0; --layer_idx)
        if (SupportElements &prev_layer = move_bounds[layer_idx]; !prev_layer.empty())
        {
            ...
            const bool merge_this_layer = had_new_element ||
                                          size_t(last_merge_layer_idx - layer_idx) >= merge_every_x_layers;
            if (had_new_element)
            ...
            influence_areas.erase(
                std::remove_if(influence_areas.begin(), influence_areas.end(),
                               [&this_layer, layer_idx](SupportElementMerging &elem)
                               {
                                   ...
                                   if (elem.areas.to_bp_areas.empty() && elem.areas.to_model_areas.empty())
                                   {
                                       ...
                                       {
                                           BOOST_LOG_TRIVIAL(error)
                                               << "Insert Error of Influence area bypass on layer " << layer_idx - 1;
                                           tree_supports_show_error("Insert error of area after bypassing merge.\n"sv,
                                                                    true);
                                       }
                                       // Move the area to output.
                                       this_layer.emplace_back(elem.state, std::move(elem.parents),
                                                               std::move(elem.areas.influence_areas));
                                       return true;
                                   }
                                   // Keep the area.
                                   return false;
            ...
            for (SupportElementMerging &elem : influence_areas)
                if (!elem.areas.influence_areas.empty())
                {
                    ...
                    {
                        BOOST_LOG_TRIVIAL(error)
                            << "Insert Error of Influence area on layer " << layer_idx - 1 << ". Origin of "
                            << elem.parents.size() << " areas. Was to bp " << elem.state.to_buildplate;
            ...
            Progress::messageProgress(Progress::Stage::SUPPORT,
                                      progress_total * m_progress_multiplier + m_progress_offset, TREE_PROGRESS_TOTAL);
#endif
            throw_on_cancel();
        }

    BOOST_LOG_TRIVIAL(info) << "Time spent with creating influence areas' subtasks: Increasing areas "
    ...
    if (layer_above)
        for (int32_t next_elem_idx : elem.parents)
        {
            ...
            if (!next_elem.state.result_on_layer_is_set())
            {
                ...
                next_elem.state.result_on_layer = move_inside_if_outside(next_elem.influence_area,
                                                                         elem.state.result_on_layer);
...
static void set_to_model_contact_simple(SupportElement &elem)
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Added NON gracious Support On Model Point (" << best.x() << "," << best.y()
                             << "). The current layer is " << elem.state.layer_idx;
}
 ...
 * \param layer_idx[in] The current layer.
 */
static void set_to_model_contact_to_model_gracious(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                                   std::vector<SupportElements> &move_bounds,
                                                   SupportElement &first_elem, std::function<void()> throw_on_cancel)
{
    ...
    // check for every layer upwards, up to the point where this influence area was created (either by initial insert or merge) if the branch could be placed on it, and highest up layer index.
    {
        ...
        for (LayerIndex layer_check = elem->state.layer_idx;
             !intersection(elem->influence_area,
                           volumes.getPlaceableAreas(support_element_collision_radius(config, elem->state), layer_check,
                                                     throw_on_cancel))
                  .empty();
    ...
    {
        BOOST_LOG_TRIVIAL(warning) << "No valid placement found for to model gracious element on layer "
                                   << first_elem.state.layer_idx;
        tree_supports_show_error(
            "Could not fine valid placement on model! Just placing it down anyway. Could cause floating branches."sv,
            true);
    ...
    else
    {
        // Found a gracious area above first_elem. Remove all below last_successfull_layer.
        {
            ...
            for (SupportElement *elem = &first_elem; elem != last_successfull_layer;
                 elem = &move_bounds[++parent_layer_idx][elem->parents.front()])
            {
                assert(!elem->state.deleted);
        ...
        BOOST_LOG_TRIVIAL(debug) << "Added gracious Support On Model Point (" << best.x() << "," << best.y()
                                 << "). The current layer is " << last_successfull_layer;
    }
}
 ...
 * \param move_bounds[in,out] All currently existing influence areas
 */
static void create_nodes_from_area(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                   std::vector<SupportElements> &move_bounds, std::function<void()> throw_on_cancel)
{
        ...
        for (SupportElement &elem : layer)
        {
            ...
            if (!elem.state.result_on_layer_is_set())
            {
                if (elem.state.to_buildplate ||
                    (elem.state.distance_to_top < config.min_dtt_to_model && !elem.state.supports_roof))
                {
                    if (elem.state.to_buildplate)
                    {
                        BOOST_LOG_TRIVIAL(error)
                            << "Uninitialized Influence area targeting " << elem.state.target_position.x() << ","
                            << elem.state.target_position.y()
                            << ") "
                               "at target_height: "
                            << elem.state.target_height << " layer: " << layer_idx;
                        tree_supports_show_error(
                            "Uninitialized support element! A branch could be missing or exist partially."sv, true);
                    }
        ...
        for (SupportElement &elem : layer)
            if (!elem.state.deleted)
            {
                for (int32_t iparent : elem.parents)
                {
                    ...
                    if (elem.state.result_on_layer_is_set())
                    {
                        double radius_increase = support_element_radius(config, elem) -
                                                 support_element_radius(config, parent);
 ...
 */
static void generate_branch_areas(const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                                  const std::vector<SupportElements> &move_bounds, std::vector<DrawArea> &linear_data,
                                  std::function<void()> throw_on_cancel)
{
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
    double progress_total = TREE_PROGRESS_PRECALC_AVO + TREE_PROGRESS_PRECALC_COLL + TREE_PROGRESS_GENERATE_NODES +
                            TREE_PROGRESS_AREA_CALC;
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, linear_data.size()),
        [&volumes, &config, &move_bounds, &linear_data, &branch_circle,
         &throw_on_cancel](const tbb::blocked_range<size_t> &range)
        {
            for (size_t idx = range.begin(); idx < range.end(); ++idx)
            {
                ...
                std::vector<std::pair<Point, coord_t>> movement_directions{
                    std::pair<Point, coord_t>(Point(0, 0), radius)};
                if (!draw_area.element->state.skip_ovalisation)
                {
                    if (draw_area.child_element != nullptr)
                    {
                        const Point movement = draw_area.child_element->state.result_on_layer -
                                               draw_area.element->state.result_on_layer;
                        movement_directions.emplace_back(movement, radius);
                    }
                    const SupportElements *layer_above = layer_idx + 1 < LayerIndex(move_bounds.size())
                                                             ? &move_bounds[layer_idx + 1]
                                                             : nullptr;
                    for (int32_t parent_idx : draw_area.element->parents)
                    {
                        ...
                        movement_directions.emplace_back(movement, std::max(support_element_radius(config, parent),
                                                                            config.support_line_width));
                        parent_uses_min |= parent.state.use_min_xy_dist;
                    }
                }

                const Polygons &collision = volumes.getCollision(0, layer_idx,
                                                                 parent_uses_min ||
                                                                     draw_area.element->state.use_min_xy_dist);
                auto generateArea = [&collision, &draw_area, &branch_circle, branch_radius = config.branch_radius,
                                     support_line_width = config.support_line_width,
                                     &movement_directions](coord_t aoffset, double &max_speed)
                {
                    Polygons poly;
                    max_speed = 0;
                    for (std::pair<Point, coord_t> movement : movement_directions)
                    {
                        ...
                        Point center_position = draw_area.element->state.result_on_layer + movement.first / coord_t(2);
                        ...
                        for (Point vertex : branch_circle)
                            circle.points.emplace_back(center_position +
                                                       Point(matrix[0] * vertex.x() + matrix[1] * vertex.y(),
                    ...
                    poly = diff_clipped(offset(union_(poly), std::min(coord_t(50), support_line_width / 4), jtMiter,
                                               1.2),
                                        collision);
                    return poly;
                };
                ...
                if (fast_relative_movement ||
                    support_element_radius(config, *draw_area.element) -
                            support_element_collision_radius(config, draw_area.element->state) >
                        config.support_line_width)
                {
                        ...
                        if (nozzle_path.size() > 1)
                        {
                            ...
                            for (ExPolygon &part : nozzle_path)
                            {
                                ...
                                if (!part.contains(draw_area.element->state.result_on_layer))
                                {
                                    ...
                                    drop = (draw_area.element->state.result_on_layer - pt).cast<double>().norm() >=
                                           scaled<double>(0.025);
                                }
                                if (!drop)
                            ...
                            polygons = diff_clipped(
                                offset(polygons_with_correct_center, config.support_line_width / 2, jtMiter, 1.2),
                ...
                if (idx % progress_inserts_check_interval == 0)
                {
                    ...
                    Progress::messageProgress(Progress::Stage::SUPPORT,
                                              progress_total * m_progress_multiplier + m_progress_offset,
                                              TREE_PROGRESS_TOTAL);
                }
#endif
                throw_on_cancel();
 ...
 */
static void smooth_branch_areas(const TreeSupportSettings &config, std::vector<SupportElements> &move_bounds,
                                std::vector<DrawArea> &linear_data, const std::vector<size_t> &linear_data_layers,
                                std::function<void()> throw_on_cancel)
{
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
    double progress_total = TREE_PROGRESS_PRECALC_AVO + TREE_PROGRESS_PRECALC_COLL + TREE_PROGRESS_GENERATE_NODES +
                            TREE_PROGRESS_AREA_CALC + TREE_PROGRESS_GENERATE_BRANCH_AREAS;
#endif // SLIC3R_TREESUPPORTS_PROGRESS

    const coord_t max_radius_change_per_layer =
        1 + config.support_line_width /
                2; // this is the upper limit a radius may change per layer. +1 to avoid rounding errors
    ...
    for (LayerIndex layer_idx = 0; layer_idx < LayerIndex(move_bounds.size()) - 1; ++layer_idx)
    {
        ...
        tbb::parallel_for(
            tbb::blocked_range<size_t>(0, processing_base_above - processing_base),
            [&](const tbb::blocked_range<size_t> &range)
            {
                for (size_t processing_idx = range.begin(); processing_idx < range.end(); ++processing_idx)
                {
                    ...
                    if (do_something)
                    {
                        ...
                        Polygons max_allowed_area = offset(draw_area.polygons, float(max_outer_wall_distance), jtMiter,
                                                           1.2);
                        for (int32_t parent_idx : draw_area.element->parents)
                        {
                            const SupportElement &parent = layer_above[parent_idx];
#ifndef NDEBUG
                            assert(parent.state.layer_idx == layer_idx + 1);
                            assert(contains(linear_data[processing_base_above + parent_idx].polygons,
                                            parent.state.result_on_layer));
                            double radius_increase = support_element_radius(config, *draw_area.element) -
                                                     support_element_radius(config, parent);
                            assert(radius_increase >= 0);
                            double shift = (draw_area.element->state.result_on_layer - parent.state.result_on_layer)
                                               .cast<double>()
                                               .norm();
                            assert(shift < radius_increase + 2. * config.maximum_move_distance_slow);
#endif // NDEBUG
                            if (support_element_radius(config, parent) !=
                                support_element_collision_radius(config, parent))
                            {
    ...
    Progress::messageProgress(
        Progress::Stage::SUPPORT, progress_total * m_progress_multiplier + m_progress_offset,
        TREE_PROGRESS_TOTAL); // It is just assumed that both smoothing loops together are one third of the time spent in this function. This was guessed. As the whole function is only 10%, and the smoothing is hard to predict a progress report in the loop may be not useful.
#endif
    ...
    for (int layer_idx = int(move_bounds.size()) - 2; layer_idx >= 0; --layer_idx)
    {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, processing_base_above - processing_base),
                          [&](const tbb::blocked_range<size_t> &range)
                          {
                              for (size_t processing_idx = range.begin(); processing_idx < range.end();
                                   ++processing_idx)
                              {
                                  DrawArea &draw_area = linear_data[processing_base + processing_idx];
                                  bool do_something = false;
                                  ...
                                  for (int32_t parent_idx : draw_area.element->parents)
                                  {
                                      ...
                                      Polygons result = offset(linear_data[processing_base_above + parent_idx].polygons,
                                                               max_outer_line_increase, jtMiter, 1.2);
                                      Point direction = draw_area.element->state.result_on_layer -
                                                        parent.state.result_on_layer;
                                      ...
                                      do_something = do_something || parent.state.marked ||
                                                     support_element_collision_radius(config, parent) !=
                                                         support_element_radius(config, parent);
                                  }
                                  if (do_something)
...
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
    progress_total += TREE_PROGRESS_SMOOTH_BRANCH_AREAS / 2;
    Progress::messageProgress(Progress::Stage::SUPPORT, progress_total * m_progress_multiplier + m_progress_offset,
                              TREE_PROGRESS_TOTAL);
#endif
}
 ...
 * \param inverse_tree_order[in] A mapping that returns the child of every influence area.
 */
static void drop_non_gracious_areas(const TreeModelVolumes &volumes, const std::vector<DrawArea> &linear_data,
                                    std::vector<Polygons> &support_layer_storage, std::function<void()> throw_on_cancel)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, linear_data.size()),
                      [&](const tbb::blocked_range<size_t> &range)
                      {
                          for (size_t idx = range.begin(); idx < range.end(); ++idx)
                          {
                              ...
                              if (const DrawArea &draw_element = linear_data[idx];
                                  !draw_element.element->state.to_model_gracious &&
                                  draw_element.child_element == nullptr)
                              {
                                  Polygons rest_support;
                                  const LayerIndex layer_idx_first = draw_element.element->state.layer_idx - 1;
                                  for (LayerIndex layer_idx = layer_idx_first;
                                       area(rest_support) > tiny_area_threshold && layer_idx >= 0; --layer_idx)
                                  {
                                      rest_support = diff_clipped(layer_idx == layer_idx_first ? draw_element.polygons
                                                                                               : rest_support,
...
static void finalize_interface_and_support_areas(
    ...
    const std::vector<Polygons> &overhangs, std::vector<Polygons> &support_layer_storage,
    ...
    std::function<void()> throw_on_cancel)
{
    ...
    InterfacePreference interface_pref =
        config.interface_preference; // InterfacePreference::InterfaceAreaOverwritesSupport;
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, support_layer_storage.size()),
        [&](const tbb::blocked_range<size_t> &range)
        {
...
//FIXME
                        ...
                        case InterfacePreference::InterfaceLinesOverwriteSupport:
                        {
                            ...
                            Polygons interface_lines = offset(
                                to_polylines(generate_support_infill_lines(support_roof->polygons, true, layer_idx,
                        ...
                        {
                            // Hatch the support roof interfaces, offset them by their line width and subtract them from support base.
                            Polygons tree_lines = union_(
                                offset(to_polylines(generate_support_infill_lines(base_layer_polygons, false, layer_idx,
                                                                                  config.support_line_distance, true)),
                ...
                if (config.support_bottom_layers > 0 && !base_layer_polygons.empty())
                {
                    ...
                    Polygons layer_outset = diff_clipped(config.support_bottom_offset > 0
                                                             ? offset(base_layer_polygons, config.support_bottom_offset,
                                                                      jtMiter, 1.2)
                                                             : base_layer_polygons,
                                                         volumes.getCollision(0, layer_idx, false));
                    ...
                    while (layers_below <= config.support_bottom_layers)
                    {
                        ...
                        const size_t sample_layer = static_cast<size_t>(
                            std::max(0, (static_cast<int>(layer_idx) - static_cast<int>(layers_below)) -
                                            static_cast<int>(config.z_distance_bottom_layers)));
                    ...
                    if (!floor_layer.empty())
                    {
                        ...
                        base_layer_polygons = diff_clipped(
                            base_layer_polygons, offset(support_bottom->polygons, scaled<float>(0.01), jtMiter,
                                                        1.2)); // Subtract the support floor from the normal support.
                    }
                }
 ...
 * \param storage[in,out] The storage where the support should be stored.
 */
static void draw_areas(PrintObject &print_object, const TreeModelVolumes &volumes, const TreeSupportSettings &config,
                       const std::vector<Polygons> &overhangs, std::vector<SupportElements> &move_bounds,
                       ...
                       std::function<void()> throw_on_cancel)
{
    ...
    // Single threaded combining all support areas to the right layers.
    {
        ...
        for (LayerIndex layer_idx = 0; layer_idx < LayerIndex(move_bounds.size()); ++layer_idx)
        {
            ...
            for (; end != linear_data.end() && end->element->state.layer_idx == layer_idx; ++end)
                ++(end->element->state.missing_roof_layers > end->element->state.distance_to_top ? cnt_roofs
                                                                                                 : cnt_layers);
            ...
            for (auto it = begin; it != end; ++it)
                std::move(std::begin(it->polygons), std::end(it->polygons),
                          std::back_inserter(it->element->state.missing_roof_layers > it->element->state.distance_to_top
                                                 ? this_roofs
                                                 : this_layers));
            begin = end;
        }
    ...
    finalize_interface_and_support_areas(print_object, volumes, config, overhangs, support_layer_storage,
                                         support_roof_storage, bottom_contacts, top_contacts, intermediate_layers,
                                         layer_storage, throw_on_cancel);
    ...
    BOOST_LOG_TRIVIAL(info) << "Time used for drawing subfuctions: generate_branch_areas: " << dur_gen_tips
                            << " ms "
                               "smooth_branch_areas: "
                            << dur_smooth
                            << " ms "
 ...
 */
static void generate_support_areas(Print &print, const BuildVolume &build_volume,
                                   const std::vector<size_t> &print_object_ids, std::function<void()> throw_on_cancel,
                                   const std::vector<Polygons> &additional_excluded_areas = {})
{
    ...
    std::vector<std::pair<TreeSupportSettings, std::vector<size_t>>> grouped_meshes = group_meshes(print,
                                                                                                   print_object_ids);
    ...
    for (std::pair<TreeSupportSettings, std::vector<size_t>> &processing : grouped_meshes)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Processing support tree mesh group " << counter + 1 << " of "
        ...
        TreeModelVolumes volumes{print_object,
                                 build_volume,
                                 config.maximum_move_distance,
                                 config.maximum_move_distance_slow,
                                 processing.second.front(),
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
                                 m_progress_multiplier,
                                 m_progress_offset,
#endif // SLIC3R_TREESUPPORTS_PROGRESS
        ...
        auto remove_undefined_layers =
            [&bottom_contacts, &top_contacts, &interface_layers, &base_interface_layers, &intermediate_layers]()
        {
            auto doit = [](SupportGeneratorLayersPtr &layers)
            {
                layers.erase(std::remove_if(layers.begin(), layers.end(),
                                            [](const SupportGeneratorLayer *ptr) { return ptr == nullptr; }),
                             layers.end());
            };
        ...
        InterfacePlacer interface_placer{print_object.slicing_parameters(), support_params, config,
                                         // Outputs
        ...
        if (has_support)
        {
            ...
            for (size_t layer_idx = 0; layer_idx < move_bounds.size(); ++layer_idx)
            {
                ...
                if (auto begin = move_bounds[layer_idx].begin(); begin != move_bounds[layer_idx].end())
                    SVG::export_expolygons(
                        ...
                        {{{union_ex(volumes.getWallRestriction(support_element_collision_radius(config, begin->state),
                                                               layer_idx, begin->state.use_min_xy_dist))},
            ...
            // generating organic supports for the painted regions. Use Tree style only if explicitly
            // set; otherwise use Organic (including when primary style is Snug/Grid in mixed mode).
            ...
            // Apply to: top_contacts, interface_layers, and base_interface_layers
            const coordf_t closing_radius = print_object.config().support_material_closing_radius.value;
            if (closing_radius > 0)
            {
                const auto scaled_closing_radius = scaled<float>(closing_radius);
                const double max_hole_area = M_PI * sqr(scaled_closing_radius);
                // Helper lambda to close holes in a vector of layers
                auto close_holes_in_layers = [max_hole_area](SupportGeneratorLayersPtr &layers)
                {
                        ...
                        if (layer && !layer->polygons.empty())
                        {
                            ...
                            for (ExPolygon &expoly : expolys)
                            {
                                expoly.holes.erase(std::remove_if(expoly.holes.begin(), expoly.holes.end(),
                                                                  [max_hole_area](const Polygon &hole)
                                                                  { return std::abs(hole.area()) < max_hole_area; }),
                                                   expoly.holes.end());
                            }
                ...
                // Close holes in top contact layer (topmost interface)
                close_holes_in_layers(top_contacts);
                // Close holes in lower interface layers (organic supports may skip insert_layer due to Z mismatch)
            ...
            // Re-trim interface layers against object geometry to restore proper XY gap clearance.
            {
                ...
                const float gap_xy_scaled = float(scale_(support_params.gap_xy));

                auto trim_layers_by_object =
                    [&print_object, &slicing_params, gap_xy_scaled](SupportGeneratorLayersPtr &layers)
                {
                    for (SupportGeneratorLayer *layer : layers)
                    {
                        if (layer == nullptr || layer->polygons.empty())
                            continue;
                        // Skip raft layers
                        if (layer->print_z < slicing_params.raft_contact_top_z + EPSILON)
                            continue;
                        // Collect object layer polygons that overlap with this support layer
                        Polygons polygons_trimming;
                        for (const Layer *object_layer : print_object.layers())
                        {
                            if (object_layer->bottom_z() > layer->print_z + slicing_params.gap_support_object - EPSILON)
                                break;
                            if (object_layer->print_z <
                                layer->bottom_print_z() - slicing_params.gap_object_support + EPSILON)
                                continue;
                            polygons_append(polygons_trimming,
                                            offset(object_layer->lslices, gap_xy_scaled, JoinType::Square, 0.));
                        }
                        if (!polygons_trimming.empty())
                            layer->polygons = diff(layer->polygons, polygons_trimming);
                    }
                };
            ...
            auto dur_pre_gen = 0.001 *
                               std::chrono::duration_cast<std::chrono::microseconds>(t_precalc - t_start).count();
            ...
            BOOST_LOG_TRIVIAL(info) << "Total time used creating Tree support for the currently grouped meshes: "
                                    << dur_total
                                    << " ms. "
                                       "Different subtasks:\nCalculating Avoidance: "
                                    << dur_pre_gen
                                    << " ms "
                                       "Creating inital influence areas: "
                                    << dur_gen
                                    << " ms "
                                       "Influence area creation: "
                                    << dur_path
                                    << "ms "
                                       "Placement of Points in InfluenceAreas: "
                                    << dur_place
                                    << "ms "
                                       "Drawing result as support "
                                    << dur_draw << " ms";
        ...
        SupportGeneratorLayersPtr raft_layers = generate_raft_base(print_object, support_params,
        ...
        // for the clipped region rather than trying to clip extrusion paths after the fact.
        if (!print_object.support_layers().empty())
        {
            const SupportLayer *snug_first_layer = print_object.support_layers().front();
            if (snug_first_layer && !snug_first_layer->support_islands.empty())
            {
                coordf_t first_layer_z = print_object.slicing_parameters().first_print_layer_height;
                Polygons snug_footprint = to_polygons(snug_first_layer->support_islands);

                // Lambda to UNION organic layer's polygons with snug footprint (merge into one base)
                auto union_layer = [&](SupportGeneratorLayer *layer)
                {
                    if (layer && std::abs(layer->print_z - first_layer_z) < EPSILON && !layer->polygons.empty())
                    {
                        // Union organic polygons with snug footprint to create combined base
                        layer->polygons = union_(layer->polygons, snug_footprint);
                        return true;
                    }
                    return false;
                };

                bool unioned_any = false;

                // Try raft_layers first
                for (size_t i = 0; i < raft_layers.size() && !unioned_any; ++i)
                {
                    if (union_layer(raft_layers[i]))
                        unioned_any = true;
                }

                // Try bottom_contacts
                for (size_t i = 0; i < bottom_contacts.size() && !unioned_any; ++i)
                {
                    if (union_layer(bottom_contacts[i]))
                        unioned_any = true;
                }

                // Try top_contacts
                for (size_t i = 0; i < top_contacts.size() && !unioned_any; ++i)
                {
                    if (union_layer(top_contacts[i]))
                        unioned_any = true;
                }

                // Try intermediate_layers
                for (size_t i = 0; i < intermediate_layers.size() && !unioned_any; ++i)
                {
                    if (union_layer(intermediate_layers[i]))
                        unioned_any = true;
                }

                if (unioned_any)
                {
                    // Clear Snug's first layer fills since Organic now has the combined fill
                    // This prevents duplicate fills in the merge step
                    SupportLayer *snug_layer = const_cast<SupportLayer *>(snug_first_layer);
                    for (ExtrusionEntity *ee : snug_layer->support_fills.entities)
                        delete ee;
                    snug_layer->support_fills.entities.clear();
                    snug_layer->support_islands.clear();
                }
            }
        }
                               ...
                               const std::vector<Polygons> &additional_excluded_areas)
{
    ...
    FFFTreeSupport::generate_support_areas(
        *print_object.print(),
        BuildVolume(Pointfs{Vec2d{-300., -300.}, Vec2d{-300., +300.}, Vec2d{+300., +300.}, Vec2d{+300., -300.}}, 0.),
        {idx}, throw_on_cancel, additional_excluded_areas);
}

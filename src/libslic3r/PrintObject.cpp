...
static thread_local Slic3r::PerfStageTimer pf_timer;
#define PF_TIMER_RESET() pf_timer.reset()
#define PF_TIMER_STAGE(name) pf_timer.stage(name)
...
// Perf timing globals defined in Fill.cpp
extern PerfAccumTimer g_mf_group_fills, g_mf_intersection, g_mf_fill_surface;
extern std::atomic<int> g_mf_layer_count;
...
PrintObject::PrintObject(Print *print, ModelObject *model_object, const Transform3d &trafo, PrintInstances &&instances)
    : PrintObjectBaseWithState(print, model_object), m_trafo(trafo)
{
    ...
    double z_diff = Geometry::rotation_diff_z(model_object->instances.front()->get_matrix(),
                                              instances.front().model_instance->get_matrix());
...
PrintBase::ApplyStatus PrintObject::set_instances(PrintInstances &&instances)
{
    ...
    bool equal = equal_length &&
                 std::equal(instances.begin(), instances.end(), m_instances.begin(),
...
void PrintObject::make_perimeters()
{
    ...
    const float total = g_progress_config.slicing.total_base();
    const float accum_start = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                              g_progress_config.slicing.process_sliced_regions +
                              g_progress_config.slicing.process_geometry + g_progress_config.slicing.build_layer_graph;
    const float weight_range = g_progress_config.slicing.perimeters;

    // Helper lambda to report incremental progress within this phase
    auto report_progress = [&](float progress_within_phase)
    {
        float accum = accum_start + (weight_range * progress_within_phase);
        m_print->set_status(static_cast<int>((accum / total) * 33.0f), _u8L("Generating perimeters"));
    };

    report_progress(0.0f); // Start of phase
    PF_TIMER_RESET();
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id)
    {
        ...
        if (!region.config().extra_perimeters || region.config().perimeters == 0 || region.config().fill_density == 0 ||
            this->layer_count() < 2)
            continue;
                ...
                for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
                {
                    ...
                    for (Surface &slice : layerm.m_slices.surfaces)
                    {
                        for (;;)
                        {
                            ...
                            const Polylines intersection = intersection_pl(to_polylines(upper_layerm_polygons),
                                                                           critical_area);
    ...
    report_progress(0.33f); // 33% - extra perimeters calculated
    PF_TIMER_STAGE("make_perimeters: extra perimeters");
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size()),
                      [this](const tbb::blocked_range<size_t> &range)
                      {
                          ...
                          for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
                          {
                              ...
                              m_layers[layer_idx]->clear_visibility_cache();
                          }
                      });
    m_print->throw_if_canceled();
    report_progress(1.0f); // 100% - perimeter generation complete
    PF_TIMER_STAGE("make_perimeters: generate perimeters");
    BOOST_LOG_TRIVIAL(debug) << "Generating perimeters in parallel - end";

    Feature::FuzzySkin::clear_visibility_cache();
...
void PrintObject::prepare_infill()
{
    ...
    const float accum_start = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                              g_progress_config.slicing.process_sliced_regions +
                              g_progress_config.slicing.process_geometry + g_progress_config.slicing.build_layer_graph +
                              g_progress_config.slicing.perimeters;
    const float accum_end = accum_start + g_progress_config.slicing.prepare_infill;
    const float weight_range = g_progress_config.slicing.prepare_infill;

    // Helper lambda to report incremental progress within this phase
    auto report_progress = [&](float progress_within_phase)
    {
        float accum = accum_start + (weight_range * progress_within_phase);
        m_print->set_status(static_cast<int>((accum / total) * 33.0f), _u8L("Preparing infill"));
    };
    ...
    m_print->throw_if_canceled();
    PF_TIMER_STAGE("prepare_infill: detect_surfaces_type");
    report_progress(0.25f); // 25% - surface types detected
    ...
    PF_TIMER_STAGE("prepare_infill: prepare_fill_surfaces");
    report_progress(0.5f); // 50% - fill surfaces prepared

    // Add solid fills to ensure the shell vertical thickness.
    this->discover_vertical_shells();
    m_print->throw_if_canceled();
    PF_TIMER_STAGE("prepare_infill: discover_vertical_shells");
    report_progress(0.75f); // 75% - vertical shells discovered
    ...
    m_print->throw_if_canceled();
    PF_TIMER_STAGE("prepare_infill: process_external_surfaces");
    report_progress(1.0f); // 100% - external surfaces processed, phase complete
    ...
    m_print->throw_if_canceled();
    PF_TIMER_STAGE("prepare_infill: discover_horizontal_shells");
    ...
    m_print->throw_if_canceled();
    PF_TIMER_STAGE("prepare_infill: bridge_over_infill");

    // combine fill surfaces to honor the "infill every N layers" option
    this->combine_infill();
    m_print->throw_if_canceled();
    PF_TIMER_STAGE("prepare_infill: combine_infill");
    ...
    PF_TIMER_STAGE("prepare_infill: separate_infill_above_bridges");
...
void PrintObject::clear_fills()
{
    ...
    // CRITICAL BUG FIX: m_adaptive_fill_octrees and m_lightning_generator are built based on
    // infill geometry/density and persist across reslices. When density changes (e.g., 30% → 50%),
...
void PrintObject::infill()
{
    ...
    if (this->set_started(posInfill))
    {
        ...
        const float accum_start = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                                  g_progress_config.slicing.process_sliced_regions +
                                  g_progress_config.slicing.process_geometry +
                                  g_progress_config.slicing.build_layer_graph + g_progress_config.slicing.perimeters +
                                  g_progress_config.slicing.prepare_infill;
        const float weight_range = g_progress_config.slicing.making_infill;

        // Helper lambda to report incremental progress within this phase
        auto report_progress = [&](float progress_within_phase)
        {
            float accum = accum_start + (weight_range * progress_within_phase);
            m_print->set_status(static_cast<int>((accum / total) * 33.0f), _u8L("Making infill"));
        };
        ...
        m_print->throw_if_canceled();
        report_progress(1.0f); // 100% - infill generation complete
        PF_TIMER_STAGE("infill: make_fills (parallel)");
        {
            perf_print("[TIMING]   make_fills breakdown (%d layers, thread-summed):\n"
                       "[TIMING]     group_fills:    %7.1fms\n"
                       "[TIMING]     intersection:   %7.1fms\n"
                       "[TIMING]     fill_surface:   %7.1fms\n",
                       g_mf_layer_count.load(), g_mf_group_fills.ms(), g_mf_intersection.ms(), g_mf_fill_surface.ms());
            g_mf_group_fills.reset();
            g_mf_intersection.reset();
            g_mf_fill_surface.reset();
            g_mf_layer_count = 0;
        }
...
void PrintObject::generate_support_spots()
{
    if (this->set_started(posSupportSpotsSearch))
    {
        ...
        const float total = g_progress_config.slicing.total_with_conditionals(true, false, false, false, false);
        const float accum_start = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                                  g_progress_config.slicing.process_sliced_regions +
                                  g_progress_config.slicing.process_geometry +
                                  g_progress_config.slicing.build_layer_graph + g_progress_config.slicing.perimeters +
                                  g_progress_config.slicing.prepare_infill + g_progress_config.slicing.making_infill;
        const float weight_range = g_progress_config.slicing.support_spots;

        // Helper lambda to report incremental progress within this phase
        auto report_progress = [&](float progress_within_phase)
        {
            float accum = accum_start + (weight_range * progress_within_phase);
            m_print->set_status(static_cast<int>((accum / total) * 33.0f), _u8L("Searching support spots"));
        };
        ...
        if (!this->shared_regions()->generated_support_points.has_value())
        {
            ...
            {
                po_transform = Geometry::translation_transform(Vec3d{0, 0, this->layers().front()->bottom_z()}) *
                               po_transform;
            }
            this->m_shared_regions->generated_support_points = {po_transform, supp_points, partial_objects};
            m_print->throw_if_canceled();
        }
        report_progress(1.0f); // 100% - support spot search complete
    ...
    if (this->set_started(posSupportMaterial))
    {
        ...
        if ((this->has_support() && m_layers.size() > 1) || (this->has_raft() && !m_layers.empty()))
        {
            ...
            const float accum_start = g_progress_config.slicing.prepare_layers +
                                      ...
                                      g_progress_config.slicing.process_sliced_regions +
                                      g_progress_config.slicing.process_geometry +
                                      g_progress_config.slicing.build_layer_graph +
                                      g_progress_config.slicing.perimeters + g_progress_config.slicing.prepare_infill +
                                      g_progress_config.slicing.making_infill;
            const float weight_range = g_progress_config.slicing.support_material;

            // Helper lambda to report incremental progress within this phase
            auto report_progress = [&](float progress_within_phase)
            {
                float accum = accum_start + (weight_range * progress_within_phase);
                m_print->set_status(static_cast<int>((accum / total) * 33.0f), _u8L("Generating support material"));
            };

            report_progress(0.0f); // Start of phase
            this->_generate_support_material();
            m_print->throw_if_canceled();
            report_progress(1.0f); // 100% - support material generation complete
        }
        else
        {
#if 0
    ...
    if (this->set_started(posEstimateCurledExtrusions))
    {
        if (this->print()->config().avoid_crossing_curled_overhangs ||
            std::any_of(this->print()->m_print_regions.begin(), this->print()->m_print_regions.end(),
                        ...
                        { return region->config().enable_dynamic_overhang_speeds.getBool(); }))
        {
            BOOST_LOG_TRIVIAL(debug) << "Estimating areas with curled extrusions - start";
            // printf("[%lld] SLICING - Estimating curled extrusions\n",
            //     std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
            // fflush(stdout);
            const float total = g_progress_config.slicing.total_with_conditionals(false, false, true, false, false);
            const float accum = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                                g_progress_config.slicing.process_sliced_regions +
                                g_progress_config.slicing.process_geometry +
                                g_progress_config.slicing.build_layer_graph + g_progress_config.slicing.perimeters +
                                g_progress_config.slicing.prepare_infill + g_progress_config.slicing.making_infill +
                                g_progress_config.slicing.curled_extrusions;
            m_print->set_status(static_cast<int>((accum / total) * 33.0f), _u8L("Estimating curled extrusions"));
...
void PrintObject::calculate_overhanging_perimeters()
{
    if (this->set_started(posCalculateOverhangingPerimeters))
    {
        ...
        // printf("[%lld] SLICING - Calculating overhanging perimeters\n",
        //     std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
        // fflush(stdout);
        const float total = g_progress_config.slicing.total_with_conditionals(false, false, false, true, false);
        const float accum = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                            g_progress_config.slicing.process_sliced_regions +
                            g_progress_config.slicing.process_geometry + g_progress_config.slicing.build_layer_graph +
                            g_progress_config.slicing.perimeters + g_progress_config.slicing.prepare_infill +
                            g_progress_config.slicing.making_infill + g_progress_config.slicing.overhanging_perims;
        m_print->set_status(static_cast<int>((accum / total) * 33.0f), _u8L("Calculating overhanging perimeters"));
        ...
        if (!regions_with_dynamic_speeds.empty())
        {
            ...
            tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size()),
                              [this, &curled_lines, &unscaled_polygons_lines,
                               &regions_with_dynamic_speeds](const tbb::blocked_range<size_t> &range)
                              {
                                  PRINT_OBJECT_TIME_LIMIT_MILLIS(PRINT_OBJECT_TIME_LIMIT_DEFAULT);
                                  for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
                                  {
                                      ...
                                      {
                                          if (regions_with_dynamic_speeds.find(layer_region->m_region) ==
                                              regions_with_dynamic_speeds.end())
                                          {
                                              continue;
                                          }
                                          size_t prev_layer_id = l->lower_layer ? l->lower_layer->id() : size_t(-1);
                                          layer_region->m_perimeters =
                                              ExtrusionProcessor::calculate_and_split_overhanging_extrusions(
                                                  &layer_region->m_perimeters, unscaled_polygons_lines[prev_layer_id],
                                                  curled_lines[l->id()]);
                                      }
                                  }
                              });
    ...
    const std::vector<std::pair<const Surface *, float>> &surfaces_w_bottom_z) const
{
    ...
    return std::make_pair(adaptive_line_spacing ? build_octree(mesh, overhangs.front(), adaptive_line_spacing, false)
                                                : OctreePtr(),
                          support_line_spacing ? build_octree(mesh, overhangs.front(), support_line_spacing, true)
                                               : OctreePtr());
}
...
FillLightning::GeneratorPtr PrintObject::prepare_lightning_infill_data()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id)
        if (const PrintRegionConfig &config = this->printing_region(region_id).config();
            config.fill_density > 0 && config.fill_pattern == ipLightning)
        {
    ...
    return has_lightning_infill ? FillLightning::build_generator(std::as_const(*this), lightning_density,
...
SupportLayerPtrs::iterator PrintObject::insert_support_layer(SupportLayerPtrs::iterator pos, size_t id,
                                                             size_t interface_id, coordf_t height, coordf_t print_z,
                                                             coordf_t slice_z)
{
                                                     ...
                                                     const std::vector<t_config_option_key> &opt_keys)
{
    ...
    {
        if (opt_key == "brim_width" || opt_key == "brim_separation" || opt_key == "brim_type")
        {
                 ...
                 opt_key == "serpentine_outer_loop")
        {
            ...
            if (opt_key == "perimeters" && this->is_fuzzy_skin_painted())
            ...
            steps.emplace_back(posPerimeters);
                 ...
                 opt_key == "interlocking_beam_layer_count" || opt_key == "interlocking_depth" ||
        ...
        else if (opt_key == "support_material" || opt_key == "support_material_auto")
        {
                ...
                !m_config.support_material_bridge_no_gap)
            {
                ...
                steps.emplace_back(posSlice);
                 ...
                 opt_key == "support_material_interface_extruder" || opt_key == "support_material_interface_spacing" ||
                 opt_key == "support_material_pattern" || opt_key == "support_material_style" ||
                 ...
                 opt_key == "support_tree_branch_distance" || opt_key == "support_tree_tip_diameter" ||
                 opt_key == "raft_expansion" || opt_key == "raft_first_layer_density" ||
                 opt_key == "raft_first_layer_expansion" || opt_key == "dont_support_bridges" ||
        ...
        else if (opt_key == "over_bridge_speed")
        {
            ...
            if (old_speed == nullptr || new_speed == nullptr || old_speed->value == 0 || new_speed->value == 0)
            {
                steps.emplace_back(posPrepareInfill);
            }
        ...
        else if (opt_key == "fuzzy_skin_painted_perimeters")
        {
            if (this->is_fuzzy_skin_painted())
                steps.emplace_back(posSlice);
            steps.emplace_back(posPerimeters);
        }
        else if (opt_key == "external_perimeter_extrusion_width" || opt_key == "perimeter_extruder" ||
                 opt_key == "fuzzy_skin" || opt_key == "fuzzy_skin_thickness" || opt_key == "fuzzy_skin_point_dist" ||
                 opt_key == "overhangs" || opt_key == "thin_walls" || opt_key == "thick_bridges")
        {
            steps.emplace_back(posPerimeters);
            steps.emplace_back(posSupportMaterial);
        }
        ...
        {
            // Bridging is enabled when: no soluble support OR soluble support with bridge_no_gap
        ...
        else if (opt_key == "perimeter_generator" || opt_key == "wall_transition_length" ||
                 opt_key == "wall_transition_filter_deviation" || opt_key == "wall_transition_angle" ||
                 opt_key == "wall_distribution_count" || opt_key == "min_feature_size" || opt_key == "min_bead_width")
        {
            steps.emplace_back(posSlice);
        }
        else if (opt_key == "seam_position" || opt_key == "scarf_seam_placement" ||
                 ...
                 opt_key == "scarf_seam_max_segment_length" || opt_key == "scarf_seam_on_inner_perimeters" ||
                 opt_key == "seam_preferred_direction" || opt_key == "seam_preferred_direction_jitter" ||
                 opt_key == "support_material_speed" || opt_key == "support_material_interface_speed" ||
                 opt_key == "bridge_speed" || opt_key == "external_perimeter_speed" ||
                 opt_key == "small_perimeter_speed" || opt_key == "solid_infill_speed" ||
                 opt_key == "first_layer_infill_speed" || opt_key == "top_solid_infill_speed")
        {
            invalidated |= m_print->invalidate_step(psGCodeExport);
        }
        else if (opt_key == "wipe_into_infill" || opt_key == "wipe_into_objects" || opt_key == "infill_speed" ||
...
bool PrintObject::invalidate_step(PrintObjectStep step)
{
    ...
    {
        invalidated |= this->invalidate_steps({posPrepareInfill, posInfill, posIroning, posSupportSpotsSearch,
                                               posEstimateCurledExtrusions, posCalculateOverhangingPerimeters});
    ...
    {
        // When fill_angle changes, posInfill is invalidated. We must also invalidate posPrepareInfill
        // to ensure m_fill_surfaces gets regenerated from slices. Without this, stale m_fill_surfaces
        // causes crashes when angle-dependent geometry no longer matches cached structures.
        invalidated |= this->invalidate_steps({posPrepareInfill, posIroning, posSupportSpotsSearch});
        invalidated |= m_print->invalidate_steps({psSkirtBrim});
    }
    else if (step == posSlice)
    {
        invalidated |= this->invalidate_steps({posPerimeters, posPrepareInfill, posInfill, posIroning,
    ...
    {
        invalidated |= m_print->invalidate_steps({
            psSkirtBrim,
        });
...
void PrintObject::detect_surfaces_type()
{
    ...
    size_t num_layers = spiral_vase
                            ? std::min(size_t(this->printing_region(0).config().bottom_solid_layers), m_layers.size())
                            : m_layers.size();
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id)
    {
            ...
            [this, region_id, interface_shells, &surfaces_new](const tbb::blocked_range<size_t> &range)
            {
                ...
                for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer)
                {
                    ...
                    if (lower_layer)
                    {
                        ...
                        surfaces_append(bottom,
                                        opening_ex(diff_ex(layerm->slices().surfaces, lower_layer->lslices,
                                                           ApplySafetyOffset::Yes),
                                                   offset),
                                        surface_type_bottom_other);
...
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        ...
                        expolygons_with_attributes.emplace_back(
                            std::make_pair(union_ex(top), SVG::ExPolygonAttributes("green")));
                        expolygons_with_attributes.emplace_back(
                            std::make_pair(union_ex(bottom), SVG::ExPolygonAttributes("brown")));
                        expolygons_with_attributes.emplace_back(std::make_pair(to_expolygons(layerm->slices().surfaces),
                                                                               SVG::ExPolygonAttributes("black")));
                        SVG::export_expolygons(debug_out_path("1_detect_surfaces_type_%d_region%d-layer_%f.svg", iRun++,
                                                              region_id, layer->print_z)
                                                   .c_str(),
        ...
        BOOST_LOG_TRIVIAL(debug) << "Detecting solid surfaces for region " << region_id
                                 << " - clipping in parallel - start";
...
void PrintObject::process_external_surfaces()
{
    ...
    if (has_voids && m_layers.size() > 1)
    {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size() - 1),
                          [this, &surfaces_covered, &layer_expansions_and_voids,
                              ...
                              for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
                              {
                                  ...
                                  m_layers[layer_idx]
                                      ...
                                      ->process_external_surfaces(
                                          ...
                                          (layer_idx == 0 || surfaces_covered.empty() ||
                                           surfaces_covered[layer_idx - 1].empty())
                                              ? nullptr
...
void PrintObject::discover_vertical_shells()
{
    ...
    size_t num_layers = spiral_vase
                            ? std::min(size_t(this->printing_region(0).config().bottom_solid_layers), m_layers.size())
                            : m_layers.size();
    std::vector<DiscoverVerticalShellsCacheEntry> cache_top_botom_regions(num_layers,
        ...
        tbb::parallel_for(
            ...
            [this, &cache_top_botom_regions](const tbb::blocked_range<size_t> &range)
            {
                ...
                for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer)
                {
                    ...
                    {
                        LayerRegion &layerm = *layer.m_regions[region_id];
                        float top_bottom_expansion = float(layerm.flow(frSolidInfill).scaled_spacing()) *
                        ...
                        append(cache.bottom_surfaces,
                               offset(layerm.slices().filter_by_types(surfaces_bottom), top_bottom_expansion));
                    ...
                    if (perimeter_offset > 0.)
                    {
                        ...
                        polygons_append(cache.holes, offset2(layer.lslices, 0.3f * perimeter_min_spacing,
                                                             -perimeter_offset - 0.3f * perimeter_min_spacing));
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                        {
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id)
    {
        ...
        if (!top_bottom_surfaces_all_regions)
        {
            ...
            tbb::parallel_for(tbb::blocked_range<size_t>(0, num_layers, grain_size),
                              [this, region_id, &cache_top_botom_regions](const tbb::blocked_range<size_t> &range)
                              {
                                  ...
                                  for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer)
                                  {
                                      ...
                                      //                        append(cache.top_surfaces, offset(layerm.fill_surfaces().filter_by_type(stTop), top_bottom_expansion));
                                      // Bottom surfaces.
                                      cache.bottom_surfaces = offset(layerm.slices().filter_by_types(surfaces_bottom),
                                                                     top_bottom_expansion);
                                      //                        append(cache.bottom_surfaces, offset(layerm.fill_surfaces().filter_by_types(surfaces_bottom), top_bottom_expansion));
                                      ...
                                      if (cache.holes.empty())
                                      {
                                          for (size_t region_id = 0; region_id < layer.regions().size(); ++region_id)
                                              polygons_append(cache.holes,
                                                              to_polygons(
                                                                  layer.regions()[region_id]->fill_expolygons()));
                                      }
                                  }
                              });
            m_print->throw_if_canceled();
            BOOST_LOG_TRIVIAL(debug) << "Discovering vertical shells for region " << region_id
                                     << " in parallel - end : cache top / bottom";
        }
...
// #ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    ...
                    if (int n_top_layers = region_config.top_solid_layers.value; n_top_layers > 0)
                    {
                        ...
                        if (!at_least_one_top_projected && i < int(cache_top_botom_regions.size()))
                        {
                            ...
                            Polygons anchor_area =
                                intersection(expand(cache_top_botom_regions[idx_layer].top_surfaces,
                        ...
                        if (one_more_layer_below_top_bottom_surfaces)
                            if (i < int(cache_top_botom_regions.size()) &&
                                (i <= itop ||
                                 m_layers[i]->bottom_z() - print_z < region_config.top_solid_min_thickness - EPSILON))
                    ...
                    if (int n_bottom_layers = region_config.bottom_solid_layers.value; n_bottom_layers > 0)
                    {
                        ...
                        for (; i >= 0 && (i > ibottom || bottom_z - m_layers[i]->bottom_z() <
                        ...
                        {
                            Polygons anchor_area =
                                intersection(expand(cache_top_botom_regions[idx_layer].bottom_surfaces,
                                                    layerm->flow(frExternalPerimeter).scaled_spacing()),
                                             to_polygons(m_layers[i]->lslices));
                            combine_shells(anchor_area);
                        }
                        ...
                        if (one_more_layer_below_top_bottom_surfaces)
                            if (i >= 0 && (i > ibottom || bottom_z - m_layers[i]->print_z <
                                                              region_config.bottom_solid_min_thickness - EPSILON))
                                combine_holes(cache_top_botom_regions[i].holes);
                    }
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        Slic3r::SVG svg(debug_out_path("discover_vertical_shells-perimeters-before-union-%d.svg",
                                                       debug_idx),
                                        get_extents(shell));
                        svg.draw(shell);
...
//                    shell = union_(shell, true);
                ...
                //    continue;
...
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        Slic3r::SVG svg(debug_out_path("discover_vertical_shells-perimeters-after-union-%d.svg",
                                                       debug_idx),
                                        get_extents(shell));
                        svg.draw(shell_ex);
...
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        Slic3r::SVG svg(debug_out_path("discover_vertical_shells-internal-wshell-%d.svg", debug_idx),
                                        get_extents(shell));
                        svg.draw(layerm->fill_surfaces().filter_by_type(stInternal), "yellow", 0.5);
                        svg.draw_outline(layerm->fill_surfaces().filter_by_type(stInternal), "black", "blue",
                                         scale_(0.05));
                    ...
                    {
                        Slic3r::SVG svg(debug_out_path("discover_vertical_shells-internalvoid-wshell-%d.svg",
                                                       debug_idx),
                                        get_extents(shell));
                        svg.draw(layerm->fill_surfaces().filter_by_type(stInternalVoid), "yellow", 0.5);
                        svg.draw_outline(layerm->fill_surfaces().filter_by_type(stInternalVoid), "black", "blue",
                                         scale_(0.05));
                        svg.draw(shell_ex, "blue", 0.5);
                        svg.draw_outline(shell_ex, "black", "blue", scale_(0.05));
                        svg.Close();
                    ...
                    {
                        Slic3r::SVG svg(debug_out_path("discover_vertical_shells-internalsolid-wshell-%d.svg",
                                                       debug_idx),
                                        get_extents(shell));
                        svg.draw(layerm->fill_surfaces().filter_by_type(stInternalSolid), "yellow", 0.5);
                        svg.draw_outline(layerm->fill_surfaces().filter_by_type(stInternalSolid), "black", "blue",
                                         scale_(0.05));
                        svg.draw(shell_ex, "blue", 0.5);
                        svg.draw_outline(shell_ex, "black", "blue", scale_(0.05));
                        svg.Close();
                    ...
                    // diff(polygonsInternal, holes) generates vertical shell rings that fill the perimeter-to-sparse gap.
                    // When interlocking perimeters are enabled, they fill this same gap with overlapping concentric shells.
                    // Both cannot coexist - vertical shell rings break the interlocking pattern with stray solid infill beads.
                    // Skip generating vertical rings when interlocking is enabled. Solid support from projected stTop/stBottom
                    // surfaces (added at line 1820) is still preserved.
                    // EXCEPTION: Don't skip if this layer has bridges - bridges need solid areas for anchoring!
                    const bool layer_has_bridges =
                        !layerm->fill_surfaces().filter_by_types({stBottomBridge, stInternalBridge}).empty();
                    if (!region_config.interlock_perimeters_enabled || layerm->num_interlocking_shells() == 0 ||
                        layer_has_bridges)
                    {
                        // Clipper2's nearbyint() rounding in intersection operations can cause 1-unit (1nm) coordinate
                        // drift between polygonsInternal and holes, even when they represent the same geometric region.
                        // This drift causes diff() to return spurious full-area polygons instead of empty.
                        // Fix: If areas match within tolerance, the diff should be empty - skip it entirely.
                        double internal_area = 0, holes_area = 0;
                        for (const Polygon &p : polygonsInternal)
                            internal_area += std::abs(p.area());
                        for (const Polygon &p : holes)
                            holes_area += std::abs(p.area());
                        // Tolerance: 0.01% of area - still 1,000,000x larger than observed Clipper2 drift,
                        // yet tight enough to catch any real geometric difference.
                        if (holes_area == 0 || std::abs(internal_area - holes_area) > internal_area * 0.0001)
                        {
                    ...
                    // ALWAYS append existing stInternalSolid surfaces to maintain the proper top/bottom solid layer structure.
                    // The 6-layer top solid structure (bridge + 5 solid + 1 top) requires these surfaces to propagate correctly.
                    // Previously this was skipped when interlocking was enabled to prevent a "feedback loop", but that broke
                    // the top solid layer detection, causing top surfaces to appear one layer too late.
                ...
                // make sure that this region does not contain parts narrower than the infill spacing width.
                    ...
                    ExPolygons regularized_shell;
                    {
                        ...
                        const float narrow_ensure_vertical_wall_thickness_region_radius = 0.5f * 0.65f *
                        ...
                        regularized_shell = shrink_ex(
                            offset2_ex(
                                ...
                                // Then close gaps narrower than 1.2 * line width, such gaps are difficult to fill in with sparse infill.
                                narrow_ensure_vertical_wall_thickness_region_radius +
                                    narrow_sparse_infill_region_radius,
                                JoinType::Square),
                            // Finally expand the infill a bit to remove tiny gaps between solid infill and the other regions.
                            narrow_sparse_infill_region_radius - tiny_overlap_radius, JoinType::Square);

                        Polygons object_volume;
                        Polygons internal_volume;
                        {
                            Polygons shrinked_bottom_slice = idx_layer > 0
                        ...
                        regularized_shell.erase(
                            std::remove_if(regularized_shell.begin(), regularized_shell.end(),
...
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        Slic3r::SVG svg(debug_out_path("discover_vertical_shells-regularized-%d.svg", debug_idx),
                                        get_extents(shell_before));
                        // Source shell.
                    ...
                    Slic3r::ExPolygons new_internal = diff_ex(layerm->fill_surfaces().filter_by_type(stInternal),
                                                              regularized_shell);
                    Slic3r::ExPolygons new_internal_void =
                        diff_ex(layerm->fill_surfaces().filter_by_type(stInternalVoid), regularized_shell);

#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        SVG::export_expolygons(debug_out_path("discover_vertical_shells-new_internal-%d.svg",
                                                              debug_idx),
                                               get_extents(shell), new_internal, "black", "blue", scale_(0.05));
                        SVG::export_expolygons(debug_out_path("discover_vertical_shells-new_internal_void-%d.svg",
                                                              debug_idx),
                                               get_extents(shell), new_internal_void, "black", "blue", scale_(0.05));
                        SVG::export_expolygons(debug_out_path("discover_vertical_shells-new_internal_solid-%d.svg",
                                                              debug_idx),
                    ...
                    // stInternalBridge surfaces are created by process_external_surfaces() and discover_horizontal_shells()
                    // before discover_vertical_shells() runs. They represent bridge spans that need to be preserved.
                    // Without this, bridge infill is deleted and Fill.cpp receives empty stInternalBridge surfaces.
                    layerm->m_fill_surfaces.keep_types({stTop, stBottom, stBottomBridge, stInternalBridge});
...
template<typename T>
void debug_draw(std::string name, const T &a, const T &b, const T &c, const T &d)
{
...
void PrintObject::bridge_over_infill()
{
    ...
    {
        CandidateSurface(const Surface *original_surface, int layer_index, Polygons new_polys,
                         const LayerRegion *region, double bridge_angle)
    ...
    {
        // Each layer's candidate gathering is independent - parallelize across layers.
        tbb::concurrent_vector<CandidateSurface> candidate_surfaces;
        tbb::parallel_for(
            tbb::blocked_range<size_t>(0, this->layers().size()),
            [this, &candidate_surfaces](const tbb::blocked_range<size_t> &range)
            {
                for (size_t lidx = range.begin(); lidx < range.end(); ++lidx)
                {
                    const Layer *layer = this->get_layer(lidx);
                    ...
                    for (const LayerRegion *region : layer->lower_layer->regions())
                    {
                        ...
                        {
                            if (surface.surface_type != stInternal ||
                                region->region().config().fill_density.value == 100)
                            {
                    ...
                    lower_layer_solids = shrink(lower_layer_solids,
                                                1 * spacing); // first remove thin regions that will not support anything
                    lower_layer_solids = expand(
                        lower_layer_solids,
                        (1 + 3) *
                    ...
                    for (const LayerRegion *region : layer->regions())
                    {
                        ...
                        for (const Surface *s : region_internal_solids)
                        {
                            ...
                            if (!unsupported.empty() &&
                                (!partially_supported || area(unsupported) > 3 * 3 * spacing * spacing))
                            {
                                Polygons worth_bridging = intersection(to_polygons(s->expolygon),
                                                                       expand(unsupported, 4 * spacing));
                                ...
                                for (const Polygon &p :
                                     diff(to_polygons(s->expolygon), expand(worth_bridging, spacing)))
                                {
                                    double area = p.area();
                                ...
                                worth_bridging = intersection(closing(worth_bridging, float(SCALED_EPSILON)),
                                                              s->expolygon);
                                candidate_surfaces.push_back(CandidateSurface(s, lidx, worth_bridging, region, 0));

#ifdef DEBUG_BRIDGE_OVER_INFILL
                                debug_draw(std::to_string(lidx) + "_candidate_surface_" +
                                               std::to_string(area(s->expolygon)),
                                           to_lines(region->layer()->lslices), to_lines(s->expolygon),
                                           to_lines(worth_bridging), to_lines(unsupported_area));
#endif
#ifdef DEBUG_BRIDGE_OVER_INFILL
                                debug_draw(
                                    std::to_string(lidx) + "_candidate_processing_" + std::to_string(area(unsupported)),
    ...
    PF_TIMER_STAGE("  bridge: S1 candidate gathering");
    ...
    if (has_lightning_infill)
    {
        ...
        // Serial: each layer reads lower_layer->fill_surfaces() while potentially modifying
        // its own fill_surfaces via backup/restore, creating a cross-layer data race if parallelized.
        // Lightning infill is rare and candidate layers are few, so serial cost is negligible.
        for (size_t lidx = 0; lidx < this->layers().size(); ++lidx)
        {
            ...
            for (LayerRegion *region : layer->regions())
            {
                // Bridge anchor solids (3mm expansion into sparse) create overlaps with interlocking perimeters.
                // When interlocking is enabled, convert these anchor areas to stInternal (sparse) instead of stInternalSolid,
                // so they get extracted for interlocking perimeter generation.
                const PrintRegionConfig &region_config = region->region().config();
                const bool use_interlocking = region_config.interlock_perimeters_enabled &&
                ...
                for (const auto &surface : surfaces_by_layer[lidx])
                {
                    if (surface.region != region)
                        continue;
                    ExPolygons expansion = intersection_ex(sparse_infill,
                                                           expand(surface.new_polys, scaled<float>(3.0)));
                    solid_infill.insert(solid_infill.end(), expansion.begin(), expansion.end());
                }

                solid_infill = union_safety_offset_ex(solid_infill);

                // This modification prevented subtracting bridge anchor areas from sparse infill, which
                // contributed to the fragmentation issue in layer 55. Now we always process anchor areas
    ...
    // SECTION to generate infill polylines
    {
        ...
        // Each layer's infill polyline generation is independent - parallelize across layers.
        tbb::parallel_for(
            tbb::blocked_range<size_t>(0, layers_to_generate_infill.size()),
            [this, &layers_to_generate_infill, &infill_lines](const tbb::blocked_range<size_t> &range)
            {
                for (size_t job_idx = range.begin(); job_idx < range.end(); ++job_idx)
                {
                    size_t lidx = layers_to_generate_infill[job_idx];
                    infill_lines.at(lidx) = this->get_layer(lidx)->generate_sparse_infill_polylines_for_anchoring(
                        this->m_adaptive_fill_octrees.first.get(), this->m_adaptive_fill_octrees.second.get(),
                        this->m_lightning_generator.get());
                }
            });
#ifdef DEBUG_BRIDGE_OVER_INFILL
        ...
        {
            debug_draw(std::to_string(il.first) + "_infill_lines", to_lines(get_layer(il.first)->lslices),
                       to_lines(il.second), {}, {});
        }
#endif
    }

    PF_TIMER_STAGE("  bridge: S2 lightning + S3 infill lines + S4 clustering");

    // cluster layers by depth needed for bridge-over-infill expansion.
    // Use nozzle diameter as the depth reference - this determines how deep into sparse infill
    // we scan for gaps that bridges can span across.
    std::vector<std::vector<size_t>> clustered_layers_for_threads;
    float target_flow_height_factor = 0.9f;
    float bridge_depth = float(*std::min_element(this->print()->config().nozzle_diameter.values.begin(),
                                                 this->print()->config().nozzle_diameter.values.end()));
    {
        ...
        // Each layer's AABB union is independent - parallelize across layers.
        tbb::parallel_for(
            tbb::blocked_range<size_t>(0, layers_with_candidates.size()),
            [&layers_with_candidates, &surfaces_by_layer,
             &layer_area_covered_by_candidates](const tbb::blocked_range<size_t> &range)
            {
                for (size_t job_idx = range.begin(); job_idx < range.end(); ++job_idx)
                {
        ...
        for (auto pair : surfaces_by_layer)
        {
            if (clustered_layers_for_threads.empty() ||
                this->get_layer(clustered_layers_for_threads.back().back())->print_z <
                    this->get_layer(pair.first)->print_z - bridge_depth * target_flow_height_factor - EPSILON ||
    ...
    auto determine_bridging_angle =
        [](const Polygons &bridged_area, const Lines &anchors, InfillPattern dominant_pattern)
    {
    ...
    // generated lines
    auto construct_anchored_polygon =
        [](Polygons bridged_area, Lines anchors, const Flow &bridging_flow, double bridging_angle)
    {
        ...
        double aligning_angle = -bridging_angle + PI * 0.5;
        {
            ...
            const size_t n_vlines = (bb_x.max.x() - bb_x.min.x() + bridging_flow.scaled_spacing() - 1) /
                                    bridging_flow.scaled_spacing();
            ...
            for (size_t i = 0; i < n_vlines; i++)
            {
                ...
                for (int intersection_idx = 0; intersection_idx < int(area_intersections.size()) - 1;
                     intersection_idx++)
                {
                    if (bridged_area_tree.outside((area_intersections[intersection_idx].first +
                ...
                {
                    auto maybe_below_anchor = std::upper_bound(anchors_intersections.rbegin(),
                                                               anchors_intersections.rend(), section.a,
                    ...
                    auto maybe_upper_anchor = std::upper_bound(anchors_intersections.begin(),
                                                               anchors_intersections.end(), section.b,
                                                               [](const Point &a, const std::pair<Point, size_t> &b)
                                                               { return a.y() < b.first.y(); });
            ...
            for (const auto &polygon_slice : polygon_sections)
            {
                ...
                {
                    auto candidates_begin = std::upper_bound(polygon_slice.begin(), polygon_slice.end(),
                                                             traced_poly.lows.back(),
                                                             [](const Point &low, const Line &seg)
                                                             { return seg.b.y() > low.y(); });
                    auto candidates_end = std::upper_bound(polygon_slice.begin(), polygon_slice.end(),
                                                           traced_poly.highs.back(),
                                                           [](const Point &high, const Line &seg)
                                                           { return seg.a.y() > high.y(); });
                    ...
                    for (auto candidate = candidates_begin; candidate != candidates_end && !segment_added; candidate++)
                    {
                        ...
                        {
                            traced_poly.lows.push_back(traced_poly.lows.back() +
                                                       Point(bridging_flow.scaled_spacing() / 2, coord_t(0)));
                            traced_poly.lows.push_back(candidate->a -
                                                       Point(bridging_flow.scaled_spacing() / 2, coord_t(0)));
                            traced_poly.lows.push_back(candidate->a);
                        }
                        ...
                        {
                            traced_poly.highs.push_back(traced_poly.highs.back() +
                                                        Point(bridging_flow.scaled_spacing() / 2, coord_t(0)));
                            traced_poly.highs.push_back(candidate->b -
                                                        Point(bridging_flow.scaled_spacing() / 2, coord_t(0)));
                            traced_poly.highs.push_back(candidate->b);
                    ...
                    if (!segment_added)
                    {
                        ...
                        new_poly.points.insert(new_poly.points.end(), traced_poly.highs.rbegin(),
                                               traced_poly.highs.rend());
                ...
                for (const auto &segment : polygon_slice)
                {
                    if (used_segments.find(&segment) == used_segments.end())
                    {
                        ...
                        new_tp.lows.push_back(segment.a - Point(bridging_flow.scaled_spacing() / 2, coord_t(0)));
                        new_tp.lows.push_back(segment.a);
                        new_tp.highs.push_back(segment.b - Point(bridging_flow.scaled_spacing() / 2, coord_t(0)));
    ...
    // Clusters are independent by construction (non-overlapping layer sets) - parallelize across clusters.
    // Within each cluster, layers are processed sequentially to prevent bridge stacking.
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, clustered_layers_for_threads.size()),
        [po = static_cast<const PrintObject *>(this), target_flow_height_factor, bridge_depth, &surfaces_by_layer,
         &clustered_layers_for_threads, gather_areas_w_depth, &infill_lines, determine_bridging_angle,
         construct_anchored_polygon](const tbb::blocked_range<size_t> &range)
        {
            for (size_t cluster_idx = range.begin(); cluster_idx < range.end(); ++cluster_idx)
            {
                for (size_t job_idx = 0; job_idx < clustered_layers_for_threads[cluster_idx].size(); job_idx++)
                {
                    ...
                    std::sort(surfaces_by_layer.at(lidx).begin(), surfaces_by_layer.at(lidx).end(),
                              [](const CandidateSurface &left, const CandidateSurface &right)
                              {
                                  ...
                                  return area(left.new_polys) < area(right.new_polys);
                              });
                    if (surfaces_by_layer.at(lidx).size() > 2)
                    {
                        Vec2d origin = get_extents(surfaces_by_layer.at(lidx).front().new_polys).max.cast<double>();
                        std::stable_sort(surfaces_by_layer.at(lidx).begin() + 1, surfaces_by_layer.at(lidx).end(),
                    ...
                    coordf_t spacing =
                        surfaces_by_layer.at(lidx).front().region->bridging_flow(frSolidInfill).scaled_spacing();
                    coordf_t target_flow_height = bridge_depth * target_flow_height_factor;
...

                    {
                        ...
                        if (job_idx > 0)
                        {
                            for (int lower_job_idx = job_idx - 1; lower_job_idx >= 0; lower_job_idx--)
                            {
                                ...
                                {
                                    for (const auto &c : surfaces_by_layer.at(lower_layer_idx))
                                    {
                                        filled_polyons_on_lower_layers.insert(filled_polyons_on_lower_layers.end(),
                                                                              c.new_polys.begin(), c.new_polys.end());
                                    }
                                }
                                else
                                {
                    ...
                    Polylines anchors = intersection_pl(infill_lines.at(lidx - 1), shrink(expansion_area, spacing));
                    ...
                    debug_draw(std::to_string(lidx) + "_" + std::to_string(cluster_idx) + "_" +
                                   std::to_string(job_idx) + "_" + "_total_area",
                               to_lines(total_fill_area), to_lines(expansion_area), to_lines(deep_infill_area),
                               to_lines(anchors));
#endif

                    std::vector<CandidateSurface> expanded_surfaces;
                    expanded_surfaces.reserve(surfaces_by_layer.at(lidx).size());
                    for (const CandidateSurface &candidate : surfaces_by_layer.at(lidx))
                    {
                        const Flow &flow = candidate.region->bridging_flow(frSolidInfill);
                        Polygons area_to_be_bridge = expand(candidate.new_polys, flow.scaled_spacing());
                        area_to_be_bridge = intersection(area_to_be_bridge, deep_infill_area);

                        area_to_be_bridge.erase(
                            std::remove_if(area_to_be_bridge.begin(), area_to_be_bridge.end(),
                        ...
                        Polygons bridging_area = construct_anchored_polygon(area_to_be_bridge,
                        ...
                        // Check collision with other expanded surfaces
                        {
                            ...
                            if (reconstruct)
                            {
                                bridging_area = construct_anchored_polygon(area_to_be_bridge, to_lines(boundary_plines),
                                                                           flow, bridging_angle);
                            }
                        }
...
#ifdef DEBUG_BRIDGE_OVER_INFILL
                        debug_draw(std::to_string(lidx) + "_" + std::to_string(cluster_idx) + "_" +
                                       std::to_string(job_idx) + "_" + "_expanded_bridging" + std::to_string(r),
                                   to_lines(layer->lslices), to_lines(boundary_plines), to_lines(candidate.new_polys),
                                   to_lines(bridging_area));
#endif

                        expanded_surfaces.push_back(CandidateSurface(candidate.original_surface, candidate.layer_index,
                                                                     bridging_area, candidate.region, bridging_angle));
                    }
                    surfaces_by_layer.at(lidx).swap(expanded_surfaces);
                    expanded_surfaces.clear();
                }
            } // for cluster_idx
        }); // tbb::parallel_for

    PF_TIMER_STAGE("  bridge: S5 main expansion (parallel)");
    BOOST_LOG_TRIVIAL(info) << "Bridge over infill - Directions and expanded surfaces computed" << log_memory_info();

    // Snapshot stInternalSolid surfaces per layer before parallel modification.
    // The parallel loop below modifies fill_surfaces; reading upper_layer->fill_surfaces()
    // concurrently would race with another thread modifying that layer.
    std::map<size_t, ExPolygons> solid_surfaces_by_layer;
    for (size_t lidx = 0; lidx < this->layers().size(); ++lidx)
    {
        if (surfaces_by_layer.find(lidx) == surfaces_by_layer.end())
            continue;
        ...
        if (layer->upper_layer)
        {
            ...
            if (!solids.empty())
                solid_surfaces_by_layer[lidx] = std::move(solids);
        }
    }

    // Each layer's final surface update is independent - parallelize across layers.
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, this->layers().size()),
        [this, &surfaces_by_layer, &solid_surfaces_by_layer](const tbb::blocked_range<size_t> &range)
        {
            for (size_t lidx = range.begin(); lidx < range.end(); ++lidx)
            {
                if (surfaces_by_layer.find(lidx) == surfaces_by_layer.end() &&
                    surfaces_by_layer.find(lidx + 1) == surfaces_by_layer.end())
                    continue;
                Layer *layer = this->get_layer(lidx);
                ...
                if (surfaces_by_layer.find(lidx) != surfaces_by_layer.end())
                {
                    ...
                    {
                        cut_from_infill.insert(cut_from_infill.end(), surface.new_polys.begin(),
                                               surface.new_polys.end());
                    }
                }
                ...
                if (surfaces_by_layer.find(lidx + 1) != surfaces_by_layer.end())
                {
                    for (const auto &surface : surfaces_by_layer.at(lidx + 1))
                    {
                        auto additional_area = diff(surface.new_polys,
                                                    shrink(surface.new_polys,
                                                           surface.region->flow(frSolidInfill).scaled_spacing()));
                        additional_ensuring_areas.insert(additional_ensuring_areas.end(), additional_area.begin(),
                                                         additional_area.end());
                    }
                }
                ...
                for (LayerRegion *region : layer->regions())
                {
                    ...
                    Polygons near_perimeters = to_polygons(
                        union_safety_offset_ex(to_polygons(region->fill_surfaces().surfaces)));
                    near_perimeters = diff(near_perimeters,
                                           shrink(near_perimeters, region->flow(frSolidInfill).scaled_spacing()));
                    ExPolygons additional_ensuring = intersection_ex(additional_ensuring_areas, near_perimeters);

                    // Bridge anchors (additional_ensuring) are solid infill added to support bridges above.
                    // When interlocking perimeters are enabled, they already fill sparse regions with dense shells.
                    // Adding bridge anchor solid on top creates overlap/conflict with interlocking perimeters.
                    // Solution: Don't add bridge anchor solid when interlocking is active - let interlocking provide support.
                    const PrintRegionConfig &region_config = region->region().config();
                    const bool use_interlocking = region_config.interlock_perimeters_enabled &&
                                                  region->num_interlocking_shells() > 0;
                    if (use_interlocking && !additional_ensuring.empty())
                    {
                        // Clear bridge anchors - interlocking perimeters will provide bridge support
                        additional_ensuring.clear();
                    }

                    // We'll collect the old and new bridge shapes to update stInternal holes
                    ExPolygons old_bridge_shapes;
                    ExPolygons new_bridge_shapes;
                    ...
                    if (surfaces_by_layer.find(lidx) != surfaces_by_layer.end())
                    {
                        ...
                        const PrintRegionConfig &region_config = region->region().config();
                        ExPolygons solid_above_for_matching;
                        if (region_config.interlock_perimeters_enabled && region->num_interlocking_shells() > 0)
                        {
                            auto it = solid_surfaces_by_layer.find(lidx);
                            if (it != solid_surfaces_by_layer.end())
                                solid_above_for_matching = it->second;
                        }
                        ...
                        for (const CandidateSurface &cs : surfaces_by_layer.at(lidx))
                        {
                            for (const Surface *surface : internal_solids)
                            {
                                if (cs.original_surface == surface)
                                {
                                    ...
                                    // If interlocking and we have solid above, intersect the bridge shape with it
                                    ExPolygons bridge_polys_original = union_ex(cs.new_polys);
                                    ExPolygons bridge_polys = bridge_polys_original;
                                    if (!solid_above_for_matching.empty())
                                    {
                                        ExPolygons matched_bridge = intersection_ex(bridge_polys,
                                                                                    solid_above_for_matching);
                                        if (!matched_bridge.empty())
                                        {
                                            // Track old and new bridge shapes for stInternal update
                                            append(old_bridge_shapes, bridge_polys_original);
                                            append(new_bridge_shapes, matched_bridge);
                                            bridge_polys = matched_bridge;
                                        }
                                    }

                                    for (const ExPolygon &ep : bridge_polys)
                                    {
                    ...
                    // When we changed the bridge shape to match solid above, stInternal still has holes matching the old bridge shape.
                    // We need to update stInternal to have holes matching the NEW bridge shape for clean interlocking edges.
                    if (!old_bridge_shapes.empty() && !new_bridge_shapes.empty())
                    {
                        // Collect updated stInternal surfaces (can't modify while iterating)
                        Surfaces updated_internal_surfaces;

                        for (Surface &surf : new_surfaces)
                        {
                            if (surf.surface_type == stInternal)
                            {
                                try
                                {
                                    // Convert single ExPolygon to ExPolygons for union_ex
                                    ExPolygons surf_as_expolygons;
                                    surf_as_expolygons.push_back(surf.expolygon);

                                    // Add back the old bridge area (fill in old holes)
                                    ExPolygons updated_internal = union_ex(surf_as_expolygons, old_bridge_shapes);
                                    // Subtract the new bridge area (create new clean holes)
                                    updated_internal = diff_ex(updated_internal, new_bridge_shapes);

                                    // Store updated surfaces
                                    if (!updated_internal.empty())
                                    {
                                        for (const ExPolygon &ep : updated_internal)
                                        {
                                            updated_internal_surfaces.emplace_back(stInternal, ep);
                                        }
                                    }
                                }
                                ...
                                {
                                    // If geometry operations fail, keep original surface
                                    updated_internal_surfaces.push_back(surf);
                                }
                            }
                            else
                            {
                                // Keep non-stInternal surfaces as-is
                                updated_internal_surfaces.push_back(surf);
                            }
                        }

                        // Replace new_surfaces with updated version
                        if (!updated_internal_surfaces.empty())
                        {
                            new_surfaces = std::move(updated_internal_surfaces);
                        }
                    }
                    ...
                    debug_draw("Aensuring_" + std::to_string(reinterpret_cast<uint64_t>(&region)),
                               to_polylines(additional_ensuring), to_polylines(near_perimeters),
                               to_polylines(to_polygons(internal_infills)), to_polylines(to_polygons(internal_solids)));
                    debug_draw("Aensuring_" + std::to_string(reinterpret_cast<uint64_t>(&region)) + "_new",
                               to_polylines(additional_ensuring), to_polylines(near_perimeters),
                               to_polylines(to_polygons(new_internal_infills)),
    ...
    PF_TIMER_STAGE("  bridge: S6 final surface update (parallel)");
...
PrintObjectConfig PrintObject::object_config_from_model_object(const PrintObjectConfig &default_object_config,
                                                               const ModelObject &object, size_t num_extruders)
{
...
static constexpr const std::initializer_list<const std::string_view> keys_extruders{"infill_extruder"sv,
                                                  ...
                                                  const ModelVolume &volume, size_t num_extruders)
{
    ...
    if (config.fuzzy_skin.value != FuzzySkinType::None &&
        (config.fuzzy_skin_point_dist.value < 0.01 || config.fuzzy_skin_thickness.value < 0.001))
...
void PrintObject::update_slicing_parameters()
{
    ...
    {
        m_slicing_params = SlicingParameters::create_from_config(this->print()->config(), m_config,
                                                                 this->model_object()->max_z(),
                                                                 this->object_extruders(),
                                                                 this->print()->shrinkage_compensation());
    }
}

SlicingParameters PrintObject::slicing_parameters(const DynamicPrintConfig &full_config,
                                                  const ModelObject &model_object, float object_max_z,
                                              ...
                                              std::vector<coordf_t> &layer_height_profile)
{
    ...
    if (!layer_height_profile.empty() &&
        ...
        ((layer_height_profile.size() & 1) != 0 ||
         ...
         std::abs(layer_height_profile[layer_height_profile.size() - 2] -
                  slicing_parameters.object_print_z_uncompensated_max + slicing_parameters.object_print_z_min) > 1e-3))
    {
...
//             smooth_outward(
//                 closing(upper_internal, closing_radius, JoinType::Square, 0.),
//                 scaled<coord_t>(0.1)),
...
void PrintObject::discover_horizontal_shells()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id)
    {
        for (size_t i = 0; i < m_layers.size(); ++i)
        {
            ...
            {
                m_print->throw_if_canceled();
                SurfaceType type = (idx_surface_type == 0)   ? stTop
                                   : (idx_surface_type == 1) ? stBottom
                                                             : stBottomBridge;
                int num_solid_layers = (type == stTop) ? region_config.top_solid_layers.value
                                                       : region_config.bottom_solid_layers.value;
                ...
                for (int n = (type == stTop) ? int(i) - 1 : int(i) + 1;
                     (type == stTop)
                         ...
                         : (n < int(m_layers.size()) && (n - int(i) < num_solid_layers ||
                                                         m_layers[n]->bottom_z() - bottom_z <
                                                             region_config.bottom_solid_min_thickness.value - EPSILON));
                     (type == stTop) ? --n : ++n)
                {
                    ...
                    if (new_internal_solid.empty())
                    {
                        ...
                        if (region_config.fill_density.value == 0 ||
                            region_config.ensure_vertical_shell_thickness.value ==
                    ...
                    // when spacing is added in Fill.pm
                    {
                        ...
                        if (!too_narrow.empty())
                        {
                            ...
                            polygons_append(
                                new_internal_solid,
                                intersection(expand(too_narrow, +margin),
                                             ...
                                             internal));
                            // Fix for contact surface detection
                    ...
                    ExPolygons internal = diff_ex(backup.filter_by_type(stInternal), polygons_internal,
                                                  ApplySafetyOffset::Yes);
                    // assign resulting internal surfaces to layer
...
void PrintObject::combine_infill()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id)
    {
        ...
        const double nozzle_diameter =
            std::min(this->print()->config().nozzle_diameter.get_at(region.config().infill_extruder.value - 1),
                     this->print()->config().nozzle_diameter.get_at(region.config().solid_infill_extruder.value - 1));

        const double automatic_infill_combination_max_layer_height =
            region.config().automatic_infill_combination_max_layer_height.get_abs_value(nozzle_diameter);
        const double max_combine_layer_height = automatic_infill_combination
                                                    ? std::min(automatic_infill_combination_max_layer_height,
                                                               nozzle_diameter)
                                                    : nozzle_diameter;
        ...
        std::vector<size_t> combine(m_layers.size(), 0);
        {
            ...
            for (size_t layer_idx = 0; layer_idx < m_layers.size(); ++layer_idx)
            {
                ...
                if (current_height + layer.height >= max_combine_layer_height + EPSILON ||
                    (!automatic_infill_combination && num_layers >= combine_infill_every_n_layers))
                {
        ...
        for (size_t layer_idx = 0; layer_idx < m_layers.size(); ++layer_idx)
        {
            ...
            if (!intersection.empty() && area_threshold > 0.)
                intersection.erase(std::remove_if(intersection.begin(), intersection.end(),
                                                  [area_threshold](const ExPolygon &expoly)
                                                  { return expoly.area() <= area_threshold; }),
                                   intersection.end());
            ...
            float clearance_offset = 0.5f * layerms.back()->flow(frPerimeter).scaled_width() +
                                     ...
                                     ((region.config().fill_pattern == ipRectilinear ||
                                       ...
                                       region.config().fill_pattern == ipHoneycomb)
                                          ? 1.5f
                                          : 0.5f) *
            ...
            for (LayerRegion *layerm : layerms)
            {
                ...
                {
                    // Save void surfaces.
                    layerm->m_fill_surfaces.append(intersection_ex(internal, intersection_with_clearance),
                                                   stInternalVoid);
                }
            }
        }
...
// This is used by both merge_duplicate_support_layers() and _generate_support_material()
static void clip_extrusion_entities(ExtrusionEntitiesPtr &entities, const ExPolygons &clip_regions)
{
    ...
    clip_recursive = [&clip_regions, &clip_recursive](ExtrusionEntitiesPtr &entities)
    {
        ...
        for (ExtrusionEntity *entity : entities)
        {
            ...
            {
                // Clip this path - subtract_expolygons returns parts OUTSIDE the clip region
                ExtrusionEntityCollection clipped;
                path->subtract_expolygons(clip_regions, &clipped);

                if (!clipped.empty())
                {
                    for (ExtrusionEntity *clipped_entity : clipped.entities)
                    {
                        new_entities.push_back(clipped_entity);
                    }
            ...
            else if (ExtrusionMultiPath *multipath = dynamic_cast<ExtrusionMultiPath *>(entity))
            {
                ...
                for (ExtrusionPath &subpath : multipath->paths)
                {
                    ...
                    for (ExtrusionEntity *clipped_entity : clipped.entities)
                    {
                        if (ExtrusionPath *clipped_path = dynamic_cast<ExtrusionPath *>(clipped_entity))
                        {
                            clipped_paths.push_back(std::move(*clipped_path));
                            delete clipped_path;
                        }
                        else
                        {
                ...
                {
                    new_entities.push_back(new ExtrusionMultiPath(std::move(clipped_paths)));
                }
                delete multipath;
            }
            ...
            else if (ExtrusionLoop *loop = dynamic_cast<ExtrusionLoop *>(entity))
            {
                ...
                if (!clipped_paths.empty())
                {
                    ...
                    {
                        new_entities.push_back(new ExtrusionPath(std::move(clipped_paths.front())));
                    }
                    else
                    {
                        new_entities.push_back(new ExtrusionMultiPath(std::move(clipped_paths)));
...
// After running both Snug/Grid and Organic generators, support layers may exist at the same
// print_z height. This function merges them by unioning geometry and clipping overlapping toolpaths.
void PrintObject::merge_duplicate_support_layers()
{
    if (m_support_layers.size() < 2)
        return;

    // Sort support layers by print_z using stable_sort to preserve original order
    // (Snug/Grid layers are generated first, so they should remain "base" when merging)
    ...
    // Find and merge layers with same print_z (within EPSILON)
    SupportLayerPtrs merged_layers;
    merged_layers.reserve(m_support_layers.size());
    ...
    while (i < m_support_layers.size())
    {
        ...
        // Find all layers with the same print_z
        while (j < m_support_layers.size() && std::abs(m_support_layers[j]->print_z - base_layer->print_z) < EPSILON)
        {
            ...
            // Before appending fills from duplicate layer, clip them by base layer's islands.
            // This prevents duplicate extrusions when both layers have fills in overlapping areas.
            if (!dup_layer->support_fills.empty())
            {
                if (!base_layer->support_islands.empty())
                {
                    // Clip dup_layer fills by base_layer islands to remove overlapping extrusions
                    clip_extrusion_entities(dup_layer->support_fills.entities, base_layer->support_islands);
                }
                // Only append if there's anything left after clipping
                if (!dup_layer->support_fills.empty())
                {
            ...
            // Recalculate bounding boxes for the unioned islands
            base_layer->support_islands_bboxes.clear();
            base_layer->support_islands_bboxes.reserve(base_layer->support_islands.size());
            for (const ExPolygon &expoly : base_layer->support_islands)
            {
                base_layer->support_islands_bboxes.emplace_back(get_extents(expoly).inflated(SCALED_EPSILON));
            }
        ...
        merged_layers.push_back(base_layer);
        i = j;
    }

    // Replace the support layers vector with merged result
    m_support_layers = std::move(merged_layers);

    // Reassign sequential IDs to all layers - critical for downstream code that
    // may use layer IDs to size arrays or index into data structures
...
void PrintObject::_generate_support_material()
{
    ...
    if (has_grid_paint && !has_snug_paint && !has_organic_paint)
    {
        ...
        const_cast<PrintObjectConfig &>(m_config).support_material_style.value = smsGrid;
    }
    else if (has_snug_paint && !has_grid_paint && !has_organic_paint)
    {
        // Only Snug painted - force Snug style for auto-detected overhangs too
        const_cast<PrintObjectConfig &>(m_config).support_material_style.value = smsSnug;
    }
    else if (has_snug_paint && has_grid_paint && !has_organic_paint)
    {
        // Both Snug and Grid painted - each enforcer region uses its native style
        // Use Snug for auto-detected overhangs (more conservative default)
        const_cast<PrintObjectConfig &>(m_config).support_material_style.value = smsSnug;
    }
    else if ((has_snug_paint || has_grid_paint) && has_organic_paint)
    {
        // Mixed Snug/Grid with Organic - force non-organic style for PrintObjectSupportMaterial
        // Organic is handled separately by TreeSupport after snug/grid generation
        // Use Grid if grid is painted, otherwise Snug (both have proper infill)
        const_cast<PrintObjectConfig &>(m_config).support_material_style.value = has_grid_paint ? smsGrid : smsSnug;
    }
    ...
    // AFTER style override, but added for clarity and defense-in-depth
    else if (has_organic_paint || (global_organic && !has_snug_paint && !has_grid_paint))
    {
        if (this->has_support())
        {
            fff_tree_support_generate(*this, std::function<void()>([this]() { this->throw_if_canceled(); }));
...
static void project_triangles_to_slabs(SpanOfConstPtrs<Layer> layers, const indexed_triangle_set &custom_facets,
                                       const Transform3f &tr, bool seam, std::vector<Polygons> &out)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, custom_facets.indices.size()),
                      [&custom_facets, &tr, tr_det_sign, seam, layers,
                       &projections_of_triangles](const tbb::blocked_range<size_t> &range)
                      {
                          for (size_t idx = range.begin(); idx < range.end(); ++idx)
                          {
                              ...
                              std::sort(facet.begin(), facet.end(),
                                        [](const Vec3f &pt1, const Vec3f &pt2) { return pt1.z() < pt2.z(); });
                              ...
                              size_t first_layer_id = projections_of_triangles[idx].first_layer_id = it -
                                                                                                     layers.begin();
...
void PrintObject::project_and_append_custom_facets(bool seam, TriangleStateType type, std::vector<Polygons> &out) const
{
            ...
            if (!custom_facets.indices.empty())
            {
                if (seam)
                    project_triangles_to_slabs(this->layers(), custom_facets,
                                               (this->trafo_centered() * mv->get_matrix()).cast<float>(), seam, out);
                else
                {
                    std::vector<Polygons> projected;
                    // Support blockers or enforcers. Project downward facing painted areas upwards to their respective slicing plane.
                    slice_mesh_slabs(custom_facets, zs_from_layers(this->layers()),
                                     this->trafo_centered() * mv->get_matrix(), nullptr, &projected, []() {});

                                               ...
                                               const std::vector<t_config_option_key> &opt_keys)
{
    ...
    static std::unordered_set<std::string> steps_gcode = {"auto_speed",
                                                          ...
                                                          "feature_flow_external_perimeter",
                                                          "feature_gcode_start_external_perimeter",
                                                          "feature_gcode_end_external_perimeter",
                                                          "feature_temp_perimeter",
                                                          "feature_flow_perimeter",
                                                          "feature_gcode_start_perimeter",
                                                          "feature_gcode_end_perimeter",
                                                          "feature_temp_overhang_perimeter",
                                                          "feature_flow_overhang_perimeter",
                                                          "feature_gcode_start_overhang_perimeter",
                                                          "feature_gcode_end_overhang_perimeter",
                                                          "feature_temp_infill",
                                                          "feature_flow_infill",
                                                          "feature_gcode_start_infill",
                                                          "feature_gcode_end_infill",
                                                          "feature_temp_solid_infill",
                                                          "feature_flow_solid_infill",
                                                          "feature_gcode_start_solid_infill",
                                                          "feature_gcode_end_solid_infill",
                                                          "feature_temp_top_solid_infill",
                                                          "feature_flow_top_solid_infill",
                                                          "feature_gcode_start_top_solid_infill",
                                                          "feature_gcode_end_top_solid_infill",
                                                          "feature_temp_bridge",
                                                          "feature_flow_bridge",
                                                          "feature_gcode_start_bridge",
                                                          "feature_gcode_end_bridge",
                                                          "feature_temp_support",
                                                          "feature_flow_support",
                                                          "feature_gcode_start_support",
                                                          "feature_gcode_end_support",
                                                          "feature_temp_support_interface",
                                                          "feature_flow_support_interface",
                                                          "feature_gcode_start_support_interface",
                                                          "feature_gcode_end_support_interface",
                                                          "feature_temp_wait",
    ...
    for (const t_config_option_key &opt_key : opt_keys)
    {
        ...
        else if (opt_key == "skirts" || opt_key == "skirt_height" || opt_key == "draft_shield" ||
                 opt_key == "skirt_distance" || opt_key == "min_skirt_length" || opt_key == "ooze_prevention")
        {
            steps.emplace_back(psSkirtBrim);
        }
        else if (opt_key == "first_layer_height" || opt_key == "nozzle_diameter" ||
                 opt_key == "resolution"
                 // Spiral Vase forces different kind of slicing than the normal model:
                 // In Spiral Vase mode, holes are closed and only the largest area contour is kept at each layer.
                 // Therefore toggling the Spiral Vase on / off requires complete reslicing.
                 || opt_key == "spiral_vase" || opt_key == "filament_shrinkage_compensation_x" ||
                 opt_key == "filament_shrinkage_compensation_y" || opt_key == "filament_shrinkage_compensation_z" ||
                 opt_key == "prefer_clockwise_movements")
        {
            osteps.emplace_back(posSlice);
        }
        else if (opt_key == "complete_objects" || opt_key == "filament_type" || opt_key == "first_layer_temperature" ||
                 ...
                 opt_key == "filament_minimal_purge_on_wipe_tower" || opt_key == "filament_cooling_initial_speed" ||
                 ...
                 opt_key == "wipe_tower_bridging" || opt_key == "wipe_tower_extra_spacing" ||
                 opt_key == "wipe_tower_extra_flow" || opt_key == "wipe_tower_no_sparse_layers" ||
                 opt_key == "wipe_tower_extruder" || opt_key == "wiping_volumes_matrix" ||
                 opt_key == "wiping_volumes_use_custom_matrix" || opt_key == "parking_pos_retraction" ||
                 opt_key == "cooling_tube_retraction" || opt_key == "cooling_tube_length" ||
                 opt_key == "extra_loading_move" || opt_key == "multimaterial_purging" || opt_key == "travel_speed" ||
                 opt_key == "travel_speed_z" || opt_key == "first_layer_speed" || opt_key == "z_offset")
        {
        ...
        else if (opt_key == "first_layer_extrusion_width" || opt_key == "min_layer_height" ||
                 opt_key == "max_layer_height" || opt_key == "gcode_resolution")
        {
...
{
    return (m_config.draft_shield == dsEnabled &&
            m_config.skirts > 0) /* || (m_config.ooze_prevention && this->extruders().size() > 1)*/;
}
...
std::string Print::validate(std::vector<std::string> *warnings) const
{
    ...
    if (warnings)
    {
        if (m_config.bed_temperature_extruder == 0)
        {
            for (size_t a = 0; a < extruders.size(); ++a)
            {
                ...
                {
                    if (std::abs(m_config.bed_temperature.get_at(extruders[a]) -
                                 m_config.bed_temperature.get_at(extruders[b])) > 15 ||
                        std::abs(m_config.first_layer_bed_temperature.get_at(extruders[a]) -
                                 m_config.first_layer_bed_temperature.get_at(extruders[b])) > 15)
                    {
    ...
    {
        return _u8L(
            "Avoid crossing perimeters option and avoid crossing curled overhangs option cannot be both enabled together.");
    }
    ...
    for (size_t print_object_idx = 0; print_object_idx < m_objects.size(); ++print_object_idx)
    {
        ...
        if (auto layers = generate_object_layers(print_object.slicing_parameters(),
            ...
            !layers.empty() && layers.back() > this->config().max_print_height + EPSILON)
        {
            ...
            if (shrinkage_compensation_z != 1. &&
                layers.back() > (this->config().max_print_height / shrinkage_compensation_z + EPSILON))
            {
                // The object exceeds the maximum build volume height because of shrinkage compensation.
                return format(
                    _u8L(
                        "While the object %1% itself fits the build volume, it exceeds the maximum build volume height because of material shrinkage compensation."),
            ...
            else if (0.5 * (layers[layers.size() - 2] + layers.back()) > this->config().max_print_height + EPSILON)
            {
                ...
                return format(_u8L("The object %1% exceeds the maximum build volume height."),
                              print_object.model_object()->name);
            }
            else
            {
    ...
    if (this->has_wipe_tower() && !m_objects.empty())
    {
        ...
        for (const auto &extruder_idx : extruders)
        {
            ...
            if (allow_nozzle_diameter_differ_warning &&
                (nozzle_diam - EPSILON > first_nozzle_diam || nozzle_diam + EPSILON < first_nozzle_diam))
            {
        ...
        if (m_config.gcode_flavor != gcfRapid && m_config.gcode_flavor != gcfRepRapSprinter &&
            m_config.gcode_flavor != gcfRepRapFirmware && m_config.gcode_flavor != gcfRepetier &&
            m_config.gcode_flavor != gcfMarlinLegacy && m_config.gcode_flavor != gcfMarlinFirmware &&
            m_config.gcode_flavor != gcfKlipper)
            return _u8L(
                "The Wipe Tower is currently only supported for the oozeBot Rapid, Marlin, Klipper, RepRap/Sprinter, RepRapFirmware and Repetier G-code flavors.");
        ...
        if (m_objects.size() > 1)
        {
            ...
            for (size_t i = 1; i < m_objects.size(); ++i)
            {
                ...
                if (std::abs(slicing_params.first_print_layer_height - slicing_params0.first_print_layer_height) >
                        EPSILON ||
                ...
                if (slicing_params0.gap_object_support != slicing_params.gap_object_support ||
                    ...
                    return _u8L(
                        "The Wipe Tower is only supported for multiple objects if they are printed with the same support_material_contact_distance");
                ...
                for (size_t idx_object = 0; idx_object < m_objects.size(); ++idx_object)
                {
                           ...
                           i < layer_height_profiles[tallest_object_idx].size())
                    {
                        if (i % 2 == 0 &&
                            ...
                            break;
                        if (std::abs(layer_height_profiles[idx_object][i] -
                                     layer_height_profiles[tallest_object_idx][i]) > eps)
                            return _u8L(
...

    {
                                                              ...
                                                              std::string &err_msg) -> bool
        {
            ...
            {
                err_msg = (boost::format(_u8L("Excessive %1%=%2% mm to be printable with a nozzle diameter %3% mm")) %
                           opt_key % extrusion_width_max % max_nozzle_diameter)
                              .str();
                return false;
            }
            return true;
        ...
        for (PrintObject *object : m_objects)
        {
            ...
            {
                if (warnings != nullptr &&
                    (object->config().support_material_extruder == 0 ||
                     object->config().support_material_interface_extruder == 0) &&
                    ...
                    object->config().support_material_style != smsBaobab)
                {
                    ...
                    if (object->config().support_material_contact_distance != stcgNoGap)
                    {
                        ...
                        if (object->config().support_material_extruder != 0 ||
                            object->config().support_material_interface_extruder != 0)
                            return _u8L(
                                "The Wipe Tower currently supports the non-soluble supports only if they are printed with the current extruder without triggering a tool change. "
            ...
            if (!object->has_support() && warnings)
            {
                for (const ModelVolume *mv : object->model_object()->volumes)
                {
                    bool has_enforcers = mv->is_support_enforcer() ||
                                         (mv->is_model_part() &&
                                          mv->supported_facets.has_facets(*mv, TriangleStateType::ENFORCER));
                    if (has_enforcers)
                    {
    ...
    }
    {
        ...
        if (!check_custom_parameters(m_config.custom_parameters_print, m_config.custom_parameters_printer,
                                     m_config.custom_parameters_filament.values, &error_out))
...
void Print::auto_assign_extruders(ModelObject *model_object) const
{
    ...
    for (size_t volume_id = 0; volume_id < model_object->volumes.size(); ++volume_id)
    {
        ...
        if ((volume->is_model_part() || volume->is_modifier()) && !volume->material_id().empty() &&
...
void Print::process()
{
    ...
    if (this->set_started(psSkirtBrim))
    {
        ...
        const float accum = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                            ...
                            g_progress_config.slicing.making_infill + g_progress_config.slicing.skirt_brim;
        // printf("[%lld] SLICING - Generating skirt and brim\n",
        //     std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
        // fflush(stdout);
        const_cast<Print *>(this)->set_status(static_cast<int>((accum / total) * 33.0f),
                                              _u8L("Generating skirt and brim"));
    ...
    if (conflictRes.has_value())
        BOOST_LOG_TRIVIAL(error) << boost::format("gcode path conflicts found between %1% and %2%") %
                                        conflictRes->_objName1 % conflictRes->_objName2;

    m_sequential_collision_detected = config().complete_objects ? check_seq_conflict(model(), config()) : std::nullopt;

    // printf("[%lld] SLICING COMPLETE\n",
    //     std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
    // fflush(stdout);
                                ...
                                ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    {
        // Empty path means memory-only mode (preview), don't resolve to actual file
        path = "";
        BOOST_LOG_TRIVIAL(info) << "Memory-only mode: skipping file path resolution";
    }
    else
    {
        path = this->output_filepath(path_template);
    }
    std::string message;
    ...
    // printf("[%lld] GCODE GENERATION STARTED\n",
    //     std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
    // fflush(stdout);
    this->set_status(33, message);
...
void Print::_make_skirt()
{
    ...
    for (size_t i = n_skirts, extruder_idx = 0; i > 0; --i)
    {
        ...
        Polygon loop;
        {
            Polygons loops = offset(convex_hull, distance, JoinType::Round, float(scale_(0.1)));
    ...
    for (Polygon &poly :
         offset(convex_hull, distance + 0.5f * float(scale_(spacing)), JoinType::Round, float(scale_(0.1))))
...
Points Print::first_layer_wipe_tower_corners() const
{
    ...
    if (has_wipe_tower() && !m_wipe_tower_data.tool_changes.empty())
    {
        ...
        const auto [cone_R, cone_x_scale] = WipeTower::get_wipe_tower_cone_base(m_config.wipe_tower_width,
                                                                                m_wipe_tower_data.height,
                                                                                m_wipe_tower_data.depth,
                                                                                m_config.wipe_tower_cone_angle);
...
void Print::alert_when_supports_needed()
{
    if (this->set_started(psAlertWhenSupportsNeeded))
    {
        ...
        const float total = g_progress_config.slicing.total_with_conditionals(false, false, false, false, true);
        const float accum = g_progress_config.slicing.prepare_layers + g_progress_config.slicing.slice_volumes +
                            g_progress_config.slicing.process_sliced_regions +
                            g_progress_config.slicing.process_geometry + g_progress_config.slicing.build_layer_graph +
                            g_progress_config.slicing.perimeters + g_progress_config.slicing.prepare_infill +
                            g_progress_config.slicing.making_infill + g_progress_config.slicing.supports_alert;
        // printf("[%lld] SLICING - Alert if supports needed\n",
        //     std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
        // fflush(stdout);
        const_cast<Print *>(this)->set_status(static_cast<int>((accum / total) * 33.0f),
                                              _u8L("Alert if supports needed"));
        ...
        auto elements_to_translated_list =
            [](const std::vector<std::string> &translated_elements, std::string expansion_rule)
        {
        ...
        for (const PrintObject *object : m_objects)
        {
                ...
                checked_model_objects.find(object->model_object()) == checked_model_objects.end())
            {
                ...
                {
                    SupportSpotsGenerator::SupportPoints supp_points =
        ...
        std::map<std::pair<SupportSpotsGenerator::SupportPointCause, bool>, std::vector<const PrintObject *>>
            po_by_support_issues;
        ...
        if (objects_isssues.size() > po_by_support_issues.size())
        {
            ...
            {
                auto &pair = message_elements.emplace_back(issue_to_alert_message(issue.first.first,
                                                                                  issue.first.second),
        ...
        auto message = Slic3r::format(_u8L("Detected print stability issues:\n%1%"),
                                      elements_to_translated_list(lines, multiline_list_rule));
...
{
    return !m_config.spiral_vase.value && m_config.wipe_tower.value && m_config.nozzle_diameter.values.size() > 1;
}
...
bool is_toolchange_required(const bool first_layer, const unsigned last_extruder_id, const unsigned extruder_id,
...
void Print::_make_wipe_tower()
{
    ...
    // Fix for print validation issue
    {
        ...
        if (idx_begin != size_t(-1))
        {
            ...
            for (; it_layer != it_end && (*it_layer)->print_z - EPSILON < wipe_tower_new_layer_print_z_first;
                 ++it_layer)
                ;
            ...
            for (size_t i = idx_begin; i < idx_end; ++i)
            {
                ...
                double height = lt.print_z -
                                (i == 0 ? 0. : m_wipe_tower_data.tool_ordering.layer_tools()[i - 1].print_z);
                //FIXME the support layer ID is set to -1, as Vojtech hopes it is not being used anyway.
                it_layer = m_objects.front()->insert_support_layer(it_layer, -1, 0, height, lt.print_z,
                                                                   lt.print_z - 0.5 * height);
                ++it_layer;
            }
        }
    }
    ...
    WipeTower wipe_tower(model().wipe_tower().position.cast<float>(), model().wipe_tower().rotation, m_config,
    ...
    m_wipe_tower_data.priming = Slic3r::make_unique<std::vector<WipeTower::ToolChangeResult>>(
        wipe_tower.prime((float) this->skirt_first_layer_height(), m_wipe_tower_data.tool_ordering.all_extruders(),
                         false));
    ...
    // to pass to wipe_tower (so that it can use it for planning the layout of the tower)
    {
        ...
        { // for all layers
            ...
            for (const auto extruder_id : layer_tools.extruders)
            {
                ...
                if (is_toolchange_required(first_layer, last_extruder_id, extruder_id, current_extruder_id))
                {
                    float volume_to_wipe =
                        wipe_volumes[current_extruder_id][extruder_id]; // total volume to wipe after this toolchange
                    // Not all of that can be used for infill purging:
                    volume_to_wipe -= (float) m_config.filament_minimal_purge_on_wipe_tower.get_at(extruder_id);

                    // try to assign some infills/objects for the wiping:
                    volume_to_wipe = layer_tools.wiping_extrusions_nonconst().mark_wiping_extrusions(
                        *this, layer_tools, current_extruder_id, extruder_id, volume_to_wipe);
            ...
            if (&layer_tools == &m_wipe_tower_data.tool_ordering.back() ||
                (&layer_tools + 1)->wipe_tower_partitions == 0)
                break;
        }
    }
    ...
    if (m_wipe_tower_data.tool_ordering.back().wipe_tower_partitions > 0)
    {
        ...
        if (wipe_tower.layer_finished())
        {
            ...
            wipe_tower.set_layer(float(m_wipe_tower_data.tool_ordering.back().print_z + layer_height),
    ...
    else
    {
        ...
        wipe_tower.set_layer(float(m_wipe_tower_data.tool_ordering.back().print_z), float(layer_height), 0, false,
                             true);
    }
...
bool Print::has_same_shrinkage_compensations() const
{
    ...
    const double filament_shrinkage_compensation_x = m_config.filament_shrinkage_compensation_x.get_at(
        extruders.front());
    const double filament_shrinkage_compensation_y = m_config.filament_shrinkage_compensation_y.get_at(
        extruders.front());
    const double filament_shrinkage_compensation_z = m_config.filament_shrinkage_compensation_z.get_at(
        extruders.front());
...
Vec3d Print::shrinkage_compensation() const
{
    ...
    const double x_compensation_percent = std::clamp(m_config.filament_shrinkage_compensation_x.get_at(first_extruder),
                                                     -99., 99.);
    const double y_compensation_percent = std::clamp(m_config.filament_shrinkage_compensation_y.get_at(first_extruder),
                                                     -99., 99.);
    const double z_compensation_percent = std::clamp(m_config.filament_shrinkage_compensation_z.get_at(first_extruder),
                                                     -99., 99.);
    const double x_compensation = 100. / (100. - x_compensation_percent);
    const double y_compensation = 100. / (100. - y_compensation_percent);
...
const std::string PrintStatistics::TotalFilamentUsedWipeTowerValueMask =
    "; total filament used for wipe tower [g] = %.2lf\n";
...
DynamicConfig PrintStatistics::placeholders()
{
    ...
    for (const std::string &key :
         {"print_time", "normal_print_time", "silent_print_time", "used_filament", "extruded_volume", "total_cost",
          "total_weight", "total_toolchanges", "total_wipe_tower_cost", "total_wipe_tower_filament", "initial_tool",
          "initial_extruder", "initial_filament_type", "printing_filament_types", "num_printing_extruders"})
...
PrintRegion *PrintObjectRegions::FuzzySkinPaintedRegion::parent_print_object_region(
    const LayerRangeRegions &layer_range) const
{

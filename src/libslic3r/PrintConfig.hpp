...
enum class FuzzySkinType
{
    None,
    // No holes options
    External,      // External perimeters (no holes)
    ExternalPlus1, // External perimeters +1 (no holes)
    ExternalPlus2, // External perimeters +2 (no holes)
    ExternalPlus3, // External perimeters +3 (no holes)
    All,           // All perimeters (no holes)
    // With holes options
    ExternalWithHoles,      // External perimeters (incl holes)
    ExternalPlus1WithHoles, // External perimeters +1 (incl holes)
    ExternalPlus2WithHoles, // External perimeters +2 (incl holes)
    ExternalPlus3WithHoles, // External perimeters +3 (incl holes)
    AllWalls,               // All perimeters (incl holes)
};

enum class FuzzySkinNoiseType
{
    Classic,     // Original random noise (default)
    Perlin,      // Coherent Perlin noise
    Billow,      // Softer, cloud-like noise
    RidgedMulti, // Sharp, jagged features
    Voronoi,     // Cell-based patchwork
};

enum class FuzzySkinMode
{
    Displacement, // Offset path perpendicular to surface (default)
    Extrusion,    // Vary extrusion width instead of path
    Combined,     // Both displacement and extrusion
};

enum class FuzzySkinPointPlacement
{
    Standard,       // Original algorithm - faster, may cut corners at large point distances
    ShapeFollowing, // Preserves corner vertices for better accuracy at larger point distances
};

enum class FuzzySkinPaintedPerimeters
{
    External,      // Only outermost perimeter (perimeter 0)
    ExternalPlus1, // External + 1 inner perimeter (perimeters 0-1)
    ExternalPlus2, // External + 2 inner perimeters (perimeters 0-2)
    ExternalPlus3, // External + 3 inner perimeters (perimeters 0-3)
    All,           // All perimeters
};

enum class FuzzySkinVisibilityDetection
{
    fsvdPrecise,  // 1mm sampling - highest accuracy
    fsvdStandard, // 2mm sampling - good balance (default)
    fsvdRelaxed,  // 4mm sampling - faster
    fsvdMinimal,  // 8mm sampling - fastest
};

enum class TopSurfaceVisibilityDetection
{
    tsvdPrecise,  // 1mm sampling - highest accuracy
    tsvdStandard, // 2mm sampling - good balance (default)
    tsvdRelaxed,  // 4mm sampling - faster
    tsvdMinimal,  // 8mm sampling - fastest
};
...
enum class FanSpinupResponseType
{
    ...
    fsrtBlower,      // Moderate response (typical blowers) - exponent 0.5
    fsrtHighInertia, // Slow response (high-inertia blowers) - exponent 0.4
};
...
enum SupportBottomContactGap
{
    sbcgNoGap,     // No gap (0)
    sbcgHalfLayer, // Half layer height gap
    sbcgFullLayer, // Full layer height gap
};

enum SupportTopContactGap
{
    stcgNoGap,     // No gap (0) - soluble mode
    stcgHalfLayer, // Half layer height gap (requires interface layers)
    stcgFullLayer, // Full layer height gap
    stcgCustom,    // Custom gap value (uses support_material_contact_distance_custom)
};
...
enum BrimType
{
    ...
    btInnerOnly,
    btOuterAndInner,
    btEar,     // Mouse ears - only at sharp corners
    btPainted, // Manually painted brim areas
};
...
enum class PerimeterGeneratorType
{
    ...
    // for maximum dimensional accuracy
    Athena
};
...
CONFIG_OPTION_ENUM_DECLARE_STATIC_MAPS(FuzzySkinNoiseType)
CONFIG_OPTION_ENUM_DECLARE_STATIC_MAPS(FuzzySkinMode)
CONFIG_OPTION_ENUM_DECLARE_STATIC_MAPS(FuzzySkinPointPlacement)
CONFIG_OPTION_ENUM_DECLARE_STATIC_MAPS(FuzzySkinPaintedPerimeters)
CONFIG_OPTION_ENUM_DECLARE_STATIC_MAPS(FuzzySkinVisibilityDetection)
...
CONFIG_OPTION_ENUM_DECLARE_STATIC_MAPS(SupportBottomContactGap)
CONFIG_OPTION_ENUM_DECLARE_STATIC_MAPS(SupportTopContactGap)
...
public:
    ...
    static DynamicPrintConfig full_print_config_with(const t_config_option_key &opt_key, const std::string &str,
                                                     bool append = false)
    {
...
protected:
    ...
    public:
        ...
        ConfigOption *optptr(const std::string &name, T *owner) const
        {
            ...
            return (it == m_map_name_to_offset.end()) ? nullptr
                                                      : reinterpret_cast<ConfigOption *>((char *) owner + it->second);
        }

        const ConfigOption *optptr(const std::string &name, const T *owner) const
        {
            const auto it = m_map_name_to_offset.find(name);
            return (it == m_map_name_to_offset.end())
                       ? nullptr
                       : reinterpret_cast<const ConfigOption *>((const char *) owner + it->second);
        }
...
public:                                                                                                                                \
    ...
    {                                                                                                                                  \
        return s_cache_##CLASS_NAME.keys();                                                                                            \
    }                                                                                                                                  \
    const t_config_option_keys &keys_ref() const override                                                                              \
    {                                                                                                                                  \
        return s_cache_##CLASS_NAME.keys();                                                                                            \
    }                                                                                                                                  \
...
#define PRINT_CONFIG_CLASS_ELEMENT_INITIALIZATION(r, data, elem) \
    PRINT_CONFIG_CLASS_ELEMENT_INITIALIZATION2(BOOST_PP_TUPLE_ELEM(1, elem))
...
#define PRINT_CONFIG_CLASS_DERIVED_INITIALIZER(CLASSES_PARENTS_TUPLE, VALUE)    \
    BOOST_PP_SEQ_FOR_EACH_I(PRINT_CONFIG_CLASS_DERIVED_INITIALIZER_ITEM, VALUE, \
                            BOOST_PP_TUPLE_TO_SEQ(CLASSES_PARENTS_TUPLE))
...
#define PRINT_CONFIG_CLASS_DERIVED_HASH(r, data, elem) \
    boost::hash_combine(seed, static_cast<const elem *>(this)->hash());
...
#define PRINT_CONFIG_CLASS_DERIVED_DEFINE1(CLASS_NAME, CLASSES_PARENTS_TUPLE, PARAMETER_DEFINITION,                  \
                                           PARAMETER_REGISTRATION, PARAMETER_HASHES, PARAMETER_EQUALS)               \
...
#define PRINT_CONFIG_CLASS_DERIVED_DEFINE(CLASS_NAME, CLASSES_PARENTS_TUPLE, PARAMETER_DEFINITION_SEQ) \
    PRINT_CONFIG_CLASS_DERIVED_DEFINE1(                                                                \
        CLASS_NAME, CLASSES_PARENTS_TUPLE,                                                             \
        BOOST_PP_SEQ_FOR_EACH(PRINT_CONFIG_CLASS_ELEMENT_DEFINITION, _, PARAMETER_DEFINITION_SEQ),     \
        BOOST_PP_SEQ_FOR_EACH(PRINT_CONFIG_CLASS_ELEMENT_INITIALIZATION, _, PARAMETER_DEFINITION_SEQ), \
        BOOST_PP_SEQ_FOR_EACH(PRINT_CONFIG_CLASS_ELEMENT_HASH, _, PARAMETER_DEFINITION_SEQ),           \
        BOOST_PP_SEQ_FOR_EACH(PRINT_CONFIG_CLASS_ELEMENT_EQUAL, _, PARAMETER_DEFINITION_SEQ))
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionBool, interface_shells))((ConfigOptionFloat, layer_height))((ConfigOptionFloat,
        ...
        (ConfigOptionFloat, mmu_segmented_region_interlocking_depth))((ConfigOptionFloat, raft_contact_distance))(
        (ConfigOptionFloat, raft_expansion))((ConfigOptionPercent, raft_first_layer_density))(
        (ConfigOptionFloat, raft_first_layer_expansion))((ConfigOptionInt, raft_layers))(
        (ConfigOptionEnum<SeamPosition>, seam_position))((ConfigOptionBool, staggered_inner_seams))
    //  ((ConfigOptionFloat,               seam_preferred_direction))
    //  ((ConfigOptionFloat,               seam_preferred_direction_jitter))
    ((ConfigOptionFloat, slice_closing_radius))((ConfigOptionEnum<SlicingMode>, slicing_mode))(
        (ConfigOptionEnum<PerimeterGeneratorType>, perimeter_generator))((ConfigOptionFloatOrPercent,
                                                                          wall_transition_length))(
        (ConfigOptionFloatOrPercent, wall_transition_filter_deviation))((ConfigOptionFloat, wall_transition_angle))(
        (ConfigOptionInt, wall_distribution_count))((ConfigOptionFloatOrPercent, min_feature_size))(
        (ConfigOptionFloatOrPercent, min_bead_width))((ConfigOptionBool, support_material))
    ...
    ((ConfigOptionFloat, support_material_angle))((ConfigOptionBool, support_material_buildplate_only))(
        ...
        (ConfigOptionPercent, support_material_bottom_contact_extrusion_width))((ConfigOptionInt,
                                                                                 support_material_enforce_layers))(
        (ConfigOptionInt, support_material_extruder))((ConfigOptionFloatOrPercent, support_material_extrusion_width))(
        (ConfigOptionFloatOrPercent,
         support_material_interface_extrusion_width))((ConfigOptionBool, support_material_interface_contact_loops))(
        (ConfigOptionInt, support_material_interface_extruder))((ConfigOptionInt, support_material_interface_layers))(
        (ConfigOptionInt, support_material_bottom_interface_layers))
    // Spacing between interface lines (the hatching distance). Set zero to get a solid interface.
    ((ConfigOptionFloat, support_material_interface_spacing))(
        (ConfigOptionFloatOrPercent, support_material_interface_speed))((ConfigOptionEnum<SupportMaterialPattern>,
                                                                         support_material_pattern))(
        (ConfigOptionBool, support_material_bridge_no_gap))((ConfigOptionEnum<SupportMaterialInterfacePattern>,
                                                             support_material_interface_pattern))
    // Morphological closing of support areas. Only used for "sung" supports.
    ((ConfigOptionFloat, support_material_closing_radius))((ConfigOptionFloat, support_material_min_area))
    // Spacing between support material lines (the hatching distance).
    ((ConfigOptionFloat, support_material_spacing))((ConfigOptionFloat, support_material_speed))(
        (ConfigOptionEnum<SupportMaterialStyle>, support_material_style))((ConfigOptionBool,
    ...
    ((ConfigOptionFloat, support_tree_angle))((ConfigOptionFloat, support_tree_angle_slow))(
        ...
        (ConfigOptionFloat, support_tree_branch_diameter_double_wall))((ConfigOptionPercent, support_tree_top_rate))(
        (ConfigOptionFloat, support_tree_branch_distance))((ConfigOptionFloat, support_tree_tip_diameter))
    // The rest
    ((ConfigOptionBool, thick_bridges))((ConfigOptionFloat, xy_size_compensation))((ConfigOptionBool,
                                                                                    wipe_into_objects))
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionBool, automatic_infill_combination))(
        ...
        (ConfigOptionInt, fuzzy_skin_octaves))((ConfigOptionFloat, fuzzy_skin_persistence))(
    ...
    ((ConfigOptionBool, overhangs))((ConfigOptionInt, perimeter_extruder))(
        (ConfigOptionFloatOrPercent, perimeter_extrusion_width))((ConfigOptionFloat, perimeter_speed))
    // Total number of perimeters.
    ((ConfigOptionInt, perimeters))((ConfigOptionFloatOrPercent, small_perimeter_speed))(
        (ConfigOptionFloat, solid_infill_below_area))((ConfigOptionInt, solid_infill_extruder))(
    ...
    ((ConfigOptionEnum<SeamNotchType>, seam_type))((ConfigOptionFloat, seam_notch_width))((ConfigOptionFloat,
        ...
        ((ConfigOptionEnum<ScarfSeamPlacement>, scarf_seam_placement))((ConfigOptionBool, scarf_seam_only_on_smooth))(
            ...
            (ConfigOptionFloat, scarf_seam_length))((ConfigOptionFloat, scarf_seam_max_segment_length))(
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    // M203 X... Y... Z... E... [mm/sec]
    ((ConfigOptionFloats, machine_max_feedrate_x))((ConfigOptionFloats, machine_max_feedrate_y))(
        (ConfigOptionFloats, machine_max_feedrate_z))((ConfigOptionFloats, machine_max_feedrate_e))

    // M204 P... R... T...[mm/sec^2]
    ((ConfigOptionFloats, machine_max_acceleration_extruding))((
        ConfigOptionFloats, machine_max_acceleration_retracting))((ConfigOptionFloats, machine_max_acceleration_travel))
    ...
    ((ConfigOptionString, machine_rrf_m566)) // Jerk limits + P parameter
    ((ConfigOptionString, machine_rrf_m201)) // Max acceleration per axis
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionEnum<ArcFittingType>, arc_fitting))((ConfigOptionBool, autoemit_temperature_commands))(
        (ConfigOptionInt, bed_temperature_extruder))((ConfigOptionString, before_layer_gcode))((ConfigOptionString,
                                                                                                between_objects_gcode))(
        (ConfigOptionBool, binary_gcode))((ConfigOptionFloats, deretract_speed))((ConfigOptionString, end_gcode))(
        (ConfigOptionStrings, end_filament_gcode))((ConfigOptionString, extrusion_axis))(
        ...
        (ConfigOptionFloats, filament_toolchange_delay))((ConfigOptionFloats, filament_unload_time))(
        (ConfigOptionInts, filament_cooling_moves))((ConfigOptionFloats, filament_cooling_initial_speed))(
        (ConfigOptionFloats, filament_minimal_purge_on_wipe_tower))((ConfigOptionFloats, filament_cooling_final_speed))(
        (ConfigOptionPercents, filament_purge_multiplier))((ConfigOptionStrings, filament_ramming_parameters))(
        ...
        (ConfigOptionFloats, filament_stamping_distance))((ConfigOptionFloatsOrPercentsNullable,
                                                           filament_seam_gap_distance))(
        (ConfigOptionPercents, filament_shrinkage_compensation_x))((ConfigOptionPercents,
                                                                    filament_shrinkage_compensation_y))(
        (ConfigOptionPercents, filament_shrinkage_compensation_z))((ConfigOptionBool, gcode_comments))(
        (ConfigOptionEnum<GCodeFlavor>, gcode_flavor))((ConfigOptionEnum<LabelObjectsStyle>, gcode_label_objects))
    ...
    ((ConfigOptionStrings, gcode_substitutions))((ConfigOptionString, layer_gcode))((ConfigOptionBool, auto_speed))(
        ...
        (ConfigOptionBools, travel_lift_before_obstacle))((ConfigOptionBools, nozzle_high_flow))((ConfigOptionPercents,
                                                                                                  retract_before_wipe))(
        (ConfigOptionBools, wipe_extend))((ConfigOptionFloats, wipe_length))((ConfigOptionFloats, retract_length))(
        (ConfigOptionFloats, retract_length_toolchange))((ConfigOptionFloats, retract_lift))(
        (ConfigOptionFloats, retract_lift_above))((ConfigOptionFloats, retract_lift_below))(
        (ConfigOptionFloats, retract_restart_extra))((ConfigOptionFloats, retract_restart_extra_toolchange))(
        (ConfigOptionFloats, retract_speed))((ConfigOptionFloatOrPercent, seam_gap_distance))(
        (ConfigOptionString, custom_parameters_printer))((ConfigOptionString, start_gcode))((ConfigOptionStrings,
                                                                                             start_filament_gcode))(
        (ConfigOptionStrings, custom_parameters_filament))((ConfigOptionBool, single_extruder_multi_material))(
        (ConfigOptionBool, single_extruder_multi_material_priming))((ConfigOptionBool, wipe_tower_no_sparse_layers))(
        (ConfigOptionString, toolchange_gcode))((ConfigOptionFloat, travel_speed))((ConfigOptionFloat, travel_speed_z))(
        (ConfigOptionBool, use_firmware_retraction))((ConfigOptionBool, use_relative_e_distances))(
        (ConfigOptionBool, use_volumetric_e))((ConfigOptionBool, variable_layer_height))(
        ...
        (ConfigOptionBool, remaining_times))((ConfigOptionBool, silent_mode))((ConfigOptionFloat, extra_loading_move))(
        (ConfigOptionFloat, multimaterial_purging))((ConfigOptionString, color_change_gcode))(
        (ConfigOptionString, pause_print_gcode))((ConfigOptionString, template_custom_gcode)))

static inline std::string get_extrusion_axis(const GCodeConfig &cfg)
{
    return ((cfg.gcode_flavor.value == gcfMach3) || (cfg.gcode_flavor.value == gcfMachinekit)) ? "A"
           : (cfg.gcode_flavor.value == gcfNoExtrusion)                                        ? ""
                                                        : cfg.extrusion_axis.value;
}

PRINT_CONFIG_CLASS_DERIVED_DEFINE(
    PrintConfig, (MachineEnvelopeConfig, GCodeConfig),

    ((ConfigOptionBool, automatic_extrusion_widths))((ConfigOptionBool, avoid_crossing_curled_overhangs))(
        ...
        (ConfigOptionInts, overhang_fan_speed_0))((ConfigOptionInts, overhang_fan_speed_1))(
        (ConfigOptionInts, overhang_fan_speed_2))((ConfigOptionInts, overhang_fan_speed_3))(
        (ConfigOptionInts, chamber_temperature))((ConfigOptionInts, chamber_minimal_temperature))(
        (ConfigOptionBool, complete_objects))((ConfigOptionFloats, colorprint_heights))((ConfigOptionBools, cooling))(
        (ConfigOptionEnums<CoolingSlowdownLogicType>, cooling_slowdown_logic))(
        (ConfigOptionFloats, cooling_perimeter_transition_distance))((ConfigOptionFloat, default_acceleration))(
        (ConfigOptionInts, disable_fan_first_layers))((ConfigOptionEnum<DraftShield>, draft_shield))(
        (ConfigOptionFloat, duplicate_distance))((ConfigOptionFloat, external_perimeter_acceleration))(
        (ConfigOptionFloat, extruder_clearance_height))((ConfigOptionFloat, extruder_clearance_radius))(
        (ConfigOptionStrings, extruder_colour))((ConfigOptionPoints, extruder_offset))(
        ...
        (ConfigOptionFloat, travel_acceleration))((ConfigOptionFloat, travel_short_distance_acceleration))(
        (ConfigOptionBools, wipe))((ConfigOptionBool, wipe_tower))((ConfigOptionFloat, wipe_tower_acceleration))(
        (ConfigOptionFloat, wipe_tower_width))((ConfigOptionFloat, wipe_tower_per_color_wipe))(
...
PRINT_CONFIG_CLASS_DEFINE(SLAPrintConfig, ((ConfigOptionString, output_filename_format)))
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionFloat, initial_layer_height))((ConfigOptionFloat, bottle_cost))((ConfigOptionFloat, bottle_volume))(
        ...
        (ConfigOptionFloat, material_correction_z))((ConfigOptionEnum<SLAMaterialSpeed>,
                                                     material_print_speed))((ConfigOptionInt, zcorrection_layers))
        ...
        ((ConfigOptionFloatNullable, material_ow_support_pillar_diameter))(
            ...
            (ConfigOptionFloatNullable, material_ow_support_head_width))((ConfigOptionFloatNullable,
            ...
            (ConfigOptionFloatNullable, material_ow_absolute_correction))((ConfigOptionFloat, area_fill))

    //tilt params
    ((ConfigOptionFloats, delay_before_exposure))((ConfigOptionFloats, delay_after_exposure))(
        (ConfigOptionFloats, tower_hop_height))((ConfigOptionEnums<TowerSpeeds>, tower_speed))(
        (ConfigOptionBools, use_tilt))((ConfigOptionEnums<TiltSpeeds>, tilt_down_initial_speed))(
        (ConfigOptionInts, tilt_down_offset_steps))((ConfigOptionFloats, tilt_down_offset_delay))(
        (ConfigOptionEnums<TiltSpeeds>, tilt_down_finish_speed))((ConfigOptionInts, tilt_down_cycles))(
        (ConfigOptionFloats, tilt_down_delay))((ConfigOptionEnums<TiltSpeeds>, tilt_up_initial_speed))(
        (ConfigOptionInts, tilt_up_offset_steps))((ConfigOptionFloats, tilt_up_offset_delay))(
        (ConfigOptionEnums<TiltSpeeds>, tilt_up_finish_speed))((ConfigOptionInts, tilt_up_cycles))((ConfigOptionFloats,
                                                                                                    tilt_up_delay)))

PRINT_CONFIG_CLASS_DEFINE(
    SLAPrinterConfig,

    ((ConfigOptionEnum<PrinterTechnology>, printer_technology))((ConfigOptionPoints, bed_shape))(
        (ConfigOptionFloat, max_print_height))((ConfigOptionFloat, display_width))((ConfigOptionFloat, display_height))(
        (ConfigOptionInt, display_pixels_x))((ConfigOptionInt, display_pixels_y))(
        (ConfigOptionEnum<SLADisplayOrientation>, display_orientation))((ConfigOptionBool, display_mirror_x))(
        (ConfigOptionBool, display_mirror_y))((ConfigOptionFloats, relative_correction))(
        (ConfigOptionFloat, relative_correction_x))((ConfigOptionFloat, relative_correction_y))(
        (ConfigOptionFloat, relative_correction_z))((ConfigOptionFloat, absolute_correction))(
        (ConfigOptionFloat, elefant_foot_compensation))((ConfigOptionFloat, elefant_foot_min_width))(
        (ConfigOptionFloat, gamma_correction))((ConfigOptionFloat, fast_tilt_time))(
        (ConfigOptionFloat, slow_tilt_time))((ConfigOptionFloat, high_viscosity_tilt_time))
    //    ((ConfigOptionFloat,                      area_fill))
    ((ConfigOptionFloat, min_exposure_time))((ConfigOptionFloat, max_exposure_time))(
        (ConfigOptionFloat, min_initial_exposure_time))((ConfigOptionFloat, max_initial_exposure_time))(
        (ConfigOptionString, sla_archive_format))((ConfigOptionFloat, sla_output_precision))((ConfigOptionString,
                                                                                              printer_model)))

PRINT_CONFIG_CLASS_DERIVED_DEFINE0(SLAFullPrintConfig,
...
public:
    ...
    {
        this->apply(other.get(), ignore_nonexistent);
    }
    ...
    {
        m_data.set_deserialize_strict(opt_key, str, append);
        this->touch();
    }
